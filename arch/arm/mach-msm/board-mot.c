/* linux/arch/arm/mach-msm/board-mot.c
 *
...
/* 
    Qualcomm's display framework uses all the stuff below
    Google's uses stuff from panel-mot.c.
*/
#ifdef CONFIG_FB_MSM_MDDI_TPO_NV_HVGA
...
{               
    ...
    if (on) 
    {
        ...
        msleep(10); // TPO_NV requires 10ms delay after assert
        gpio_set_value(LCD_RST_N_SIGNAL, on);
    }
    else
    {
        gpio_set_value(LCD_RST_N_SIGNAL, on);
        msleep(1); // slight delay after deassert
...
static int msm_fb_detect_panel(const char *name)
{
    ...
    if (!strcmp(name, "mddi_tpo_nv_hvga"))
        ret = 0;
    else
        ret = -ENODEV;
...
static struct msm_i2c_platform_data msm_i2c_pdata = {
    .clk_freq = 100000,
//  .lock_val = 0, //SMEM_I2C_MUTEX, /* We don't need mutex, because AMSS doesn't use i2c */
	.rmutex = NULL,
	.pri_clk = 60,
	.pri_dat = 61,
	.aux_clk = 60,	// we use the same pins for second adapter, just in different mode
	.aux_dat = 61,
...
static struct android_usb_platform_data android_usb_pdata = {
    .vendor_id    = 0x22b8,
    .product_id    = 0xF000,
    .adb_product_id    = 0x2d61,
    .version    = 0x0100,
    .product_name    = "Motorola HSUSB Device",
    .manufacturer_name = "Motorola ",
    .nluns = 1,
};
...
static void sdcc_gpio_init(void)
{
    ...
    if (gpio_request(19, "host_wlan_irq "))
        pr_err("failed to request gpio host_wlan_irq\n");
}

// static unsigned int sdcc_vreg_enabled = 0;
static unsigned int wlan_vreg_enabled = 0;
/* WLAN chip is hardwired. So we do not want it to pe powered on on the host power up probe*/
//static unsigned int wlan_host_power_up_done = 0;
...
static unsigned sdcc_cfg_data[][7] = {
    ...
    GPIO_CFG(19, 0, GPIO_INPUT,  GPIO_PULL_UP, GPIO_2MA),    // HOST_IRQ_WAKE
    },
};

static unsigned long sdcc_vreg_sts, sdcc_gpio_sts;
...
static void msm_sdcc_setup_gpio(int dev_id, unsigned int enable)
{
    ...
    if (!(test_bit(dev_id, &sdcc_gpio_sts)^enable))
        return;

    if (enable)
        set_bit(dev_id, &sdcc_gpio_sts);
    else
        clear_bit(dev_id, &sdcc_gpio_sts);
...
static uint32_t mot_sdcc_sd_power(struct device *dv, unsigned int vdd)
{
    int rc = 0;
    ...
    if (vdd == 0) {
        ...
        clear_bit(pdev->id, &sdcc_vreg_sts);
...
static int mot_sdcc_wlan_power(int dev_id, int on)
{
    msm_sdcc_setup_gpio(dev_id, on);

    if ((on && wlan_vreg_enabled) || (!on && !wlan_vreg_enabled))
        return 0;

    printk(KERN_ERR "%s: WLAN power control on %d \n", __func__, on);

    if (on)
    {
        gpio_request(WLAN_REG_ON_SIGNAL, "wlan_reg_on");         
        gpio_direction_output(WLAN_REG_ON_SIGNAL, 1);        // turn on WLAN internal VREG
        msleep_interruptible(100);                // BCM4325 powerup requirement

        gpio_request(WLAN_RESET_N_SIGNAL, "wlan_reset_n");     
        gpio_direction_output(WLAN_RESET_N_SIGNAL, 1);        // take WLAN out of reset
        msleep_interruptible(100);                // BCM4325 powerup requirement

        wlan_vreg_enabled = 1;
        printk(KERN_ERR "%s: WLAN power ON\n", __func__);
    }
    else
    {
        gpio_request(WLAN_RESET_N_SIGNAL, "wlan_reset_n");     
                     gpio_direction_output(WLAN_RESET_N_SIGNAL, 0);
        ...
        gpio_request(WLAN_REG_ON_SIGNAL, "wlan_reg_on");         
            ...
            wlan_vreg_enabled = 0;
        printk(KERN_ERR "%s: WLAN power OFF\n", __func__);
    }    
    return 0;
}
 ...
 */
static unsigned int mot_sdcc_sd_status(struct device *dev)
{
    unsigned val;
    /*
     * SD_DETECT is GPIO 17 on Morrison
     */
    val = gpio_get_value(SD_DETECT_N_SIGNAL);
    return (1 - val);
}

static unsigned int mot_sdcc_wifi_status(struct device *dev)
{
    /*
...
static struct mmc_platform_data mot_sdcc1_data = {
    .ocr_mask        = MMC_VDD_28_29,
    .translate_vdd        = mot_sdcc_sd_power,
    .status            = mot_sdcc_sd_status,
    .status_irq        = MSM_GPIO_TO_INT(SD_DETECT_N_SIGNAL),
    .irq_flags      = IRQF_TRIGGER_RISING|IRQF_TRIGGER_FALLING,
};

static struct mmc_platform_data mot_sdcc2_data = {
    .ocr_mask        = MMC_VDD_28_29,
    .setup_power    = mot_sdcc_wlan_power,
    .status            = mot_sdcc_wifi_status,
    .register_status_notify    = mot_sdcc_wifi_status_notify_register
};


static void __init mot_init_mmc(void)
{
    ...
    sdcc_gpio_init();
    msm_add_sdcc(1, &mot_sdcc1_data);
    msm_add_sdcc(2, &mot_sdcc2_data);
...
{
    if (!(machine_is_morrison() && mot_hw_rev < 0x2D))
...
static unsigned bt_config_power_on[] = {
    ...
    //    GPIO_CFG(90, 0, GPIO_INPUT,  GPIO_PULL_DOWN, GPIO_2MA),    /* HOST_WAKE */
...
static int bluetooth_power(int on)
{
    ...
    if (on) {
        for (pin = 0; pin < ARRAY_SIZE(bt_config_power_on); pin++) 
          {
            if((rc = gpio_request(0x3ff & (bt_config_power_on[pin] >> 4), "BT PIN")))
              {
            printk(KERN_ERR
        ...
        gpio_request(BT_REG_ON_SIGNAL, "bt_reg_on");     // turn off internal BT VREG
...
static void __init msm_add_batt_device(void)
{
    int ret;

    ret = platform_device_register(&msm_batt_device);
    if (ret)
...
void morrison_setup(void)
{
    ...
    if(!model_set) {
        ...
        usb_mass_storage_pdata.product = "MB200";
#endif
    }
...
//	    gpio_direction_output(LCD_RST_N_SIGNAL, 1);         // enable display and backlight ctlr
...
#ifdef CONFIG_USB_FUNCTION_MASS_STORAGE
        usb_mass_storage_pdata.product = "MB300";
#endif
    }
...
#ifdef CONFIG_USB_FUNCTION_MASS_STORAGE
        usb_mass_storage_pdata.product = "Zeppelin";
#endif
    }
 ...
 */
/* static */ void __init msm_mot_allocate_memory_regions(void)
{
    void *addr;
    unsigned long size;
    ...
    if (!android_pmem_pdata.start)
    {
        ...
        printk(KERN_INFO "allocating %lu bytes at %p (%lx physical) for pmem\n", 
            size, addr, __pa(addr));
    }

    size = MSM_PMEM_CAMERA_SIZE;
    if (!android_pmem_camera_pdata.start)
    {
        addr = alloc_bootmem(size);
        android_pmem_camera_pdata.start = __pa(addr);
        android_pmem_camera_pdata.size = size;
        printk(KERN_INFO "allocating %lu bytes at %p (%lx physical) for camera pmem\n", 
            size, addr, __pa(addr));
    }
    ...
    if (!android_pmem_adsp_pdata.start)
    {
        ...
        printk(KERN_INFO "allocating %lu bytes at %p (%lx physical) for adsp pmem\n", 
            size, addr, __pa(addr));
    }

    size = MSM_PMEM_GPU1_SIZE;
    if (!android_pmem_gpu1_pdata.start)
    {
        ...
        printk(KERN_INFO "allocating %lu bytes at %p (%lx physical) for gpu1 pmem\n", 
                size, addr, __pa(addr));
      }
    ...
    */
    ...
    for (t=tags; t->hdr.size; t = tag_next(t)) {
        ...
        if (t->hdr.tag == ATAG_REVISION) {
            ...
            printk("%s: mot_hw_rev=%x\n", __func__, mot_hw_rev);
        }
        if (t->hdr.tag == ATAG_MEM) {
            smi_64m = t->u.mem.start;
            ddr_mono = t->u.mem.size;
            printk("%s: smi_64m=%d, ddr_mono=%d\n", __func__, smi_64m, ddr_mono);
        }
    }

    /* 
     * Initially assume monolithic package 1x256MB, and 64Mb SMI package
    ...
    /* 
     * Describe bank 1, but don't tell Linux about it 
     * We just use it for math below
     */
    mi->bank[1].start = 0x20000000;
    mi->bank[1].size  = (128)*1024*1024;

    /* 
     * Let Linux manage buffers, except framebuffer (sits in the SMI)
     * This will leave 5Mb of SMI unused. Part of that goes for RAM CONSOLE
     * Putting framebuffer into SMI should improve performance a bit
     */
    msm_fb_resources[0].start = SMI_MEM_OFFSET;
    msm_fb_resources[0].end = msm_fb_resources[0].start + MSM_FB_SIZE - 1;

#ifdef CONFIG_ANDROID_RAM_CONSOLE
    ramconsole_resource[0].start = msm_fb_resources[0].end + 1;
    ramconsole_resource[0].end = ramconsole_resource[0].start + MSM_RAMCONSOLE_SIZE - 1;
#endif

    android_pmem_pdata.start = 0;
    android_pmem_adsp_pdata.start = 0;
    android_pmem_gpu1_pdata.start = 0;
    android_pmem_camera_pdata.start = 0;

    if (!smi_64m)
    {
        /* 
         * If we're on mono memory, AMSS must move to the top of EBI (19MB)
         * that needs to be subtracted from the bank 0 size.
         * Framebuffer also has to move to EBI, but let Linux manage it.
         * If we're on dual bank memory - then put it into bank 1
         * and manage manually
         */
        if (ddr_mono)
        {
            mi->bank[0].size  -= SCL_EBI1_MODEM_CODE_SIZE;
            msm_fb_resources[0].start = 0;                    // auto-allocate
#ifdef CONFIG_ANDROID_RAM_CONSOLE
            ramconsole_resource[0].start = 0;                // auto-allocate
#endif
        }
        else
        {
            mi->bank[1].size  -= SCL_EBI1_MODEM_CODE_SIZE;    // leave space for modem

            mi->bank[1].size  -= MSM_FB_SIZE*2;
            msm_fb_resources[0].start = mi->bank[1].start + mi->bank[1].size;
            msm_fb_resources[0].end = msm_fb_resources[0].start + MSM_FB_SIZE - 1;

#ifdef CONFIG_ANDROID_RAM_CONSOLE
            mi->bank[1].size  -= MSM_RAMCONSOLE_SIZE*2;
            ramconsole_resource[0].start = mi->bank[1].start + mi->bank[1].size;
            ramconsole_resource[0].end = ramconsole_resource[0].start + MSM_RAMCONSOLE_SIZE - 1;
#endif
        }
    }
    ...
    {
        /* 
         * Dual-die DDR, 2x128MB, 128MB gap between banks
         * Linux can only use 128M, unless SPARSEMEM is enabled (which is not)
         * EBI bank 0 size has to be halved regardless of SMI size.
         */
        mi->bank[0].size  -= 128*1024*1024;

        /*  
         * The second bank can hold AMSS and all pmem buffers, 
         * Put buffers at the top, that depends on SMI (calculated above)
         */
        mi->bank[1].size -= MSM_PMEM_MDP_SIZE;
        android_pmem_pdata.start = mi->bank[1].start + mi->bank[1].size;

        mi->bank[1].size -= MSM_PMEM_CAMERA_SIZE;
        android_pmem_camera_pdata.start = mi->bank[1].start + mi->bank[1].size;

        mi->bank[1].size -= MSM_PMEM_ADSP_SIZE;
		if (machine_is_motus())
			mi->bank[1].size -= MSM_PMEM_ADSP_FLIP_SIZE;
        android_pmem_adsp_pdata.start = mi->bank[1].start + mi->bank[1].size;

        mi->bank[1].size -= MSM_PMEM_GPU1_SIZE;
        android_pmem_gpu1_pdata.start = mi->bank[1].start + mi->bank[1].size;
    }
    ...
    printk("%s: android_pmem_camera_pdata.start=%lx\n", __func__, android_pmem_camera_pdata.start);
    printk("%s: android_pmem_gpu1_pdata.start=%lx\n", __func__, android_pmem_gpu1_pdata.start);
}
...
static void __init board_model_setup(char *model)
{
	char *str = NULL;
...
#ifdef CONFIG_USB_FUNCTION_MASS_STORAGE
	usb_mass_storage_pdata.product = str;
#endif
...
static void __init board_i2c_setup(char *speed)
{
	char *str = NULL;
...
static void __init board_keypad_setup(char *keypad_type)
{
	static char   keypad_driver_name[64];
	...
	if(keypad_type_len && (keypad_type_len < 8))
		...
		return;
#if 0
	if((strcmp(keypad_type,"QWERTY") != 0) && (strcmp(keypad_type,"qwerty") != 0)) {
		/* not a qwerty keypad...add a name extension from cmdline */
		sprintf(keypad_driver_name, "%s_%s", mot_adp5588_device.name, keypad_type);
		mot_adp5588_device.name = keypad_driver_name;
	}
#endif
