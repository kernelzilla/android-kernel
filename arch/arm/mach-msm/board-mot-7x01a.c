...
static struct usb_composition usb_func_composition[] = {
	{
		/* 00011111 ums/modem/ether/adb/diag */
		.product_id	= 0x2d61,
		.functions	= 0x1F,
		.config		= "Motorola Config 38",
	},
	{
		/* 00011101 ums/modem/ether/diag */
		.product_id	= 0x2d63,
		.functions	= 0x1D,
		.config		= "Motorola Config 39",
	},
	{
		/* 00001111 modem/ether/adb/diag */
		.product_id	= 0x2d64,
		.functions	= 0x0F,
		.config		= "Motorola Config 40",
	},
	{
		/* 00001101 modem/ether/diag */
		.product_id	= 0x2d65,
		.functions	= 0x0D,
...
static void __init mot_init(void)
{
	...
	if (mot_hw_rev >= 0x30)
		/* P3A runs spurious interrupts without pullup */
	...
	if (!vendor1->bare_board) {
		...
		/* enable keypad */
		gpio_direction_output(QWERTY_RST_N_SIGNAL, 1);
		/* enable touchpad */
		gpio_request(TOUCH_RST_N_SIGNAL, "touch_rst_n");
		gpio_direction_output(TOUCH_RST_N_SIGNAL, 1);

		size = ARRAY_SIZE(i2c_morrison_devices);
		if (mot_hw_rev < 0x20)        /* chop off e-compass & couloumb counter on anything older than P2 */
			size -= 2;
