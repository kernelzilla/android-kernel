...
static struct gpio_event_direct_entry sapphire_keypad_nav_map[] = {
	{
		.gpio = SAPPHIRE_POWER_KEY,
		.code = KEY_END,
		.no_suspend = 1,
	},
	{
		.gpio = SAPPHIRE_VOLUME_UP,
		.code = KEY_VOLUMEUP
	},
	{
		.gpio = SAPPHIRE_VOLUME_DOWN,
		.code = KEY_VOLUMEDOWN
	},
};
...
int sapphire_gpio_event_power(const struct gpio_event_platform_data *pdata, bool on)
{
       return 0;
}
...
static struct gpio_event_platform_data sapphire_keypad_data = {
	...
	.power = sapphire_gpio_event_power,
};
