...
struct msm_mic_rpc_ids {
	unsigned long prog;
	unsigned long vers;
	unsigned      pm_mic_en; 
	unsigned      pm_mic_is_en;
	unsigned      pm_mic_set_volt;
	unsigned      pm_mic_get_volt;

};

static struct hs_dev	  *the_hs  ;

static struct msm_rpc_endpoint *mic_ep;

struct msm_mic_rpc_ids mic_rpc_ids;

int msm_mic_init_rpc_ids(void)
{
	if (machine_is_zeppelin() || machine_is_motus() || machine_is_morrison() || machine_is_lajolla()) 
	{
		mic_rpc_ids.prog			= 0x30000061;
		mic_rpc_ids.vers			= 0x00010000;
		mic_rpc_ids.pm_mic_en			= 27;
		mic_rpc_ids.pm_mic_is_en		= 28;
		mic_rpc_ids.pm_mic_set_volt		= 29;
		mic_rpc_ids.pm_mic_get_volt		= 30;
		
	}  else {
		printk(KERN_ERR "%s: not supported this target\n", __func__);
		return -ENODATA;
	}
	return 0;
}
...
int msm_pm_mic_en(unsigned char  enable_disable)
{
	...
	if (IS_ERR(mic_ep)) {
		printk(KERN_ERR "%s: msm_pm_mic_en rpc failed before call,"
			"rc = %ld\n", __func__, PTR_ERR(mic_ep));
		return -EAGAIN;
	}

	if (machine_is_morrison() && mot_hw_rev < 0x2D) // check the actual factory build id
	{
	    req.enable_disable=cpu_to_be32((uint32_t )enable_disable); 
		rc = msm_rpc_call(mic_ep, mic_rpc_ids.pm_mic_en,
			&req, sizeof(req), 5 * HZ);
...
int msm_mic_rpc_connect(void)
{

	if (mic_ep) {
		printk(KERN_INFO "%s: mic_ep already connected\n", __func__);
		return 0;
	}

	/* Initialize rpc ids */
	if (msm_mic_init_rpc_ids()) {
		printk(KERN_ERR "%s: mic rpc ids initialization failed\n"
			, __func__);
		return -ENODATA;
	}

	mic_ep = msm_rpc_connect(mic_rpc_ids.prog, mic_rpc_ids.vers, 0);

	if (IS_ERR(mic_ep)) {
		printk(KERN_ERR "%s: connect failed with current  mic_ep %p ,PROG %lx , VERS = %lx, \n",
				__func__, mic_ep,mic_rpc_ids.prog,mic_rpc_ids.vers);
	return -1;
	} else
		printk(KERN_INFO "%s: mic rpc connect success\n", __func__);
	return 0;
}
...
static int do_set_state(struct hs_dev *hs, uint8_t new_state)
{
	...
	{
        hs->state = new_state;
//	switch_set_state(&hs->sdev, new_state);
	if( new_state )  /* headset is ON. cheak mic */
	{       
 	hs->mic = do_check_mic();  /* there is the mic and may be the BTN */	
	}else {
	 if( hs->mic )	 	      msm_pm_mic_en(0);
         if( the_hs->btn_in_test )    enable_disable_btn_irq(hs,0);
         hs->mic =0;
	 the_hs->btn_in_test=0;	
	}
	
	switch_set_state(&hs->sdev,
	                 get_state_value(hs->state,hs->mic,hs->tv_out ));

	}else if( hs->state ) {
        /* if state did not changed and ON then retest the mic it can the disconnected if HS is hlf removed ( it is very noisy :) */
    	 if( hs->mic )	
          { 
 	     mic_state  = do_check_mic();  /* there is the mic and may be the BTN */
             if( hs->mic != mic_state )
              {
            	hs->mic = mic_state;
		switch_set_state(&hs->sdev,
...
static void hs_work_func( struct work_struct *work)
{
 ...
 //       printk(KERN_INFO "HEADSET: hs_work_fun  state %s , mic %d \n", the_hs->detect_state,hs->mic);
	...
	local_irq_restore(irq_flags);
        ...
        if( !hs->mic ){  
          if( hs->mic = do_check_mic())    /*retest  mic*/
...
static int __init hs_probe(struct platform_device *pdev)
{
	...
	if (rc < 0)
      ...
      /*connect to PMIC mic RPC */
	if (machine_is_morrison() && mot_hw_rev < 0x2B)
              msm_mic_rpc_connect();
/*
        if( !gpio_get_value(HEADSET_DETECT_GPIO)) 
	{
	disable_irq(the_hs->irq);
            do_set_state(the_hs,!gpio_get_value(HEADSET_DETECT_GPIO));
	enable_irq(the_hs->irq);
	}
*/
//	switch_set_state(&the_hs->sdev, the_hs->state);

 //       msm_pm_mic_en(the_hs->state);
