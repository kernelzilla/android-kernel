...
#define DRIVER_NAME "h2w"
#else
#define DRIVER_NAME "hs"
#endif

#ifndef CONFIG_MACH_CIENA
#define HEADSET_DETECT_GPIO   	   49
#define HEADSET_SEND_END_0_GPIO     28
#define HEADSET_SEND_END_1_GPIO     112

#define HEADSET_DETECT_DEBOUNCE_TIMER   500000000    /*  500 ms */
#else
#define HEADSET_DETECT_GPIO   	   30
#define HEADSET_SEND_END_0_GPIO     28
#define HEADSET_SEND_END_1_GPIO     112

#define HEADSET_DETECT_DEBOUNCE_TIMER   100000000    /*  500 ms */
#endif
...
#define HEADSET_MIC_BIAS_DELAY          50    

static uint8_t   prev_val1 = 0; 
static uint8_t   prev_val2 = 0;
...
int msm_pm_mic_en(unsigned char  enable_disable)
{
   ...
   if (!the_hs->vreg)
	...
	if(enable_disable )
	{
	   if (rc = vreg_set_level(the_hs->vreg, 2600))
      {
         printk(KERN_ERR "%s: vreg(synth) set level to 2600 failed (%d)\n", __func__, rc);
         return rc;
      } 
      if ((rc = vreg_enable(the_hs->vreg)))
		   printk(KERN_ERR "%s: vreg(gp4) enable failed (%d)\n", __func__, rc);
	}
	else
	{
	   if ((rc = vreg_disable(the_hs->vreg)))
		printk(KERN_ERR "%s: vreg(gp4) disable failed (%d)\n", __func__, rc);
	}
	if (rc < 0) 
	{
...
static int  do_check_mic(void)
{
   ...
   if (hs_detect == 0)  /* headset is detected */
   {
	   msm_pm_mic_en(1);
   ...
   {
      /* No headset , turn off mic bias and disable BTN*/
	   msm_pm_mic_en(0);	
...
void enable_disable_btn_irq( struct hs_dev *hs,int state )
{
	...
	if( state ) {
		...
		set_irq_wake(the_hs->irq_btn_0, 1);
		set_irq_wake(the_hs->irq_btn_1, 1);
	} else {
		disable_irq(hs->irq_btn_0);
		disable_irq(hs->irq_btn_1);
		set_irq_wake(the_hs->irq_btn_0, 0);
		set_irq_wake(the_hs->irq_btn_1, 0);
	}
...
//	switch_set_state(&hs->sdev, new_state);
	if( new_state )  /* headset is ON. check mic */
	{       
 	hs->mic = do_check_mic();  /* there is the mic and may be the BTN */	
	}
   else     /* no headset is detected */
   {      
      msm_pm_mic_en(0);
      if( the_hs->btn_in_test )    
         enable_disable_btn_irq(hs,0);
...
static enum hrtimer_restart hs_btn_event_timer_func(struct hrtimer *data)
{
    ...
    if (hs_detect == 0)  /* headset is detected */
    {
      /* take a reading of SEND_END_0 & SEND_END_1  */
      val1 =gpio_get_value(HEADSET_SEND_END_0_GPIO);
	   val2 =gpio_get_value(HEADSET_SEND_END_1_GPIO);
      printk(KERN_INFO "val1 = %d, val2 = %d, hs_dtect = %d @ hs_btn_event_timer()\n", val1, val2, hs_detect );
	   ...
	   if(  hs->btn_counter < HEADSET_BTN_DETECT_REPEATS )
      {
	   ...
	   else
	   {
         if (val1 == val2)
         {
            printk(KERN_INFO "hs detect, button press(hs,1) @hs_btn_event_timer()\n" );
	         do_button_event(hs, 1);  // Button pressed	 
         }
         else
         {
            printk(KERN_INFO "hs detect, button release(hs,0) @hs_btn_event_timer()\n" );
   ...
   {
      /* HS was removed during BTN test */
      the_hs->btn_in_test =0;
	   hs->btn_counter=0;
   ...
   /* RE ENABLE button detect interrupt  */
	hs->btn_counter=0;
	hs->btn_off_counter=0;
   /* Save the debounced key state for the next debounce */
   prev_val1  = val1;
   prev_val2  = val2;
...
static int __init hs_probe(struct platform_device *pdev)
{
	...
	rc = set_irq_wake(the_hs->irq_btn_0, 0);
	if (rc < 0)
		goto err_request_detect_irq;
	...
	rc = set_irq_wake(the_hs->irq_btn_1, 0);
	if (rc < 0)
		goto err_request_detect_irq;

         /* DISABLE BTN INT & turn off mic bias if  Headset is not plugged in */
