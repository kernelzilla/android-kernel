...
#if !defined(CONFIG_MACH_PITTSBURGH) && !defined(CONFIG_MACH_SOCIAL)
#include "timed_vibrator.h"
#endif /*CONFIG_MACH_PITTSBURGH && CONFIG_MACH_SOCIAL */
#include "board-mot.h"
#include "pm.h"
...
#endif /*CONFIG_MACH_PITTSBURGH || CONFIG_MACH_SOCIAL*/
#define MOT_CHANGE_SOCIAL_POWER
#ifdef CONFIG_MACH_SOCIAL
extern int adp8870_get_msg_led_status();
#define MSG_LED_OFF 0
#endif /* CONFIG_MACH_SOCIAL */

/* Social camera defs: */
#ifdef CONFIG_MACH_SOCIAL
#define TEMP_76XX_ANDROID   1
#endif /* CONFIG_MACH_SOCIAL */
...
#endif /*CONFIG_KIONIX_KXTF9_ACCELEROMETER*/
...
#endif /*#CONFIG_SENSORS_LIS331DLH*/
...
#ifdef CONFIG_MACH_SOCIAL
static struct platform_device mot_isl29011_device = {
        .name = "isl29011",
        .id  = 0,
};
/* Added for Social AT42QT602240*/
...
#ifdef CONFIG_MACH_SOCIAL
	.product_name   = "Motorola MB512",
#else /* CONFIG_MACH_SOCIAL */
...
static struct usb_composition usb_func_composition[] = {
	...
	},
	{
		.product_id     = MOT_PID4,
		.functions      = 0x2f,
		/* 00101111 adb/ums/ether/modem/diag */
		.config         = "Motorola Config 35",
		/* Motorola Android Composite Device */
	},
	{
		.product_id     = MOT_PID5,
		.functions      = 0x29,     /* 00101001 adb/ums/diag */
		.config         = "Motorola Config 36",
                                 /* Motorola Mass Storage Class */
	},
	{
		.product_id     = MOT_PID6,
		.functions      = 0x31,     /* 00110001 adb/mtp/diag */
...
static void msm_hsusb_get_serial(void)
{
#ifdef CONFIG_USB_DEV_SERIAL_NUM
		unsigned int serial_len = 0;
		unsigned char *serial = NULL;
		int i = 0;
		serial = smem_get_entry(SMEM_WM_UUID, &serial_len);

		if (serial && serial_len)
		{
            /* Current serial number string size is 16,
               Convert the latest 8 bytes UUID into string */
            if(serial_len > (MAX_USB_SERIAL_NUM - 1) / 2){
                serial += serial_len - (MAX_USB_SERIAL_NUM - 1) / 2;
                serial_len = (MAX_USB_SERIAL_NUM - 1) / 2;
            }
			for (i = 0; i < serial_len; i++)
				sprintf(&device_serial[i*2], "%02X", serial[i]);
		}
		else
#endif
			strcpy(device_serial, MOT_SERIAL_NUM);
}
...
#ifdef CONFIG_MACH_SOCIAL /* Social product specific changes */ 
	"gp5"
#else 
	"gp3"
#endif
...
static void msm_fb_mddi_power_save(int on)
{
	...
	int i;
#ifdef CONFIG_MACH_SOCIAL
        int msg_led_status = 0;
	msg_led_status = adp8870_get_msg_led_status();
	/* When msg leds is active, we couldn't let display to turn off GPIO since the GPIO is shared with msg led.*/
	if ((!on) && (msg_led_status == MSG_LED_OFF))
	{
...
#ifdef CONFIG_MACH_SOCIAL
	{
                I2C_BOARD_INFO("isl29011",ISL29011_I2C_ADDR),
                .irq = MSM_GPIO_TO_INT(ISL29011_INT_N_SIGNAL),
                .platform_data = &mot_isl29011_device
        },
	 /* Added for Social AT42QT602240*/
        {
...
{
#ifndef MOT_CHANGE_SOCIAL_POWER
...
#else
	/* Samsung 3MP sensor on Social/Ciena requires that powerdown go high before the
           MCLK is cut, otherwise it leaks PCLK when shut down. */
        if (!cam_pwrdwn_gpio_state)
	{
	  gpio_request(0, "cam_pwrdwn");
	  gpio_direction_output(0, 1);
...
static unsigned sdcc_cfg_data[][7] = {
	...
	/* SDC1 configs */
	{
	GPIO_CFG(51, 1, GPIO_OUTPUT, GPIO_NO_PULL, GPIO_6MA),
	GPIO_CFG(52, 1, GPIO_OUTPUT, GPIO_NO_PULL, GPIO_6MA),
	GPIO_CFG(53, 1, GPIO_OUTPUT, GPIO_NO_PULL, GPIO_6MA),
	GPIO_CFG(54, 1, GPIO_OUTPUT, GPIO_NO_PULL, GPIO_6MA),
	GPIO_CFG(55, 1, GPIO_OUTPUT, GPIO_NO_PULL, GPIO_8MA),
	GPIO_CFG(56, 1, GPIO_OUTPUT, GPIO_NO_PULL, GPIO_10MA),
	GPIO_CFG(56, 1, GPIO_OUTPUT, GPIO_NO_PULL, GPIO_10MA), /* 7th item in array */
	},
	/* SDC2 configs */
	{
...
static void __init mot_7x27_init_mmc(void)
{
	...
	msm_add_sdcc(1, &mot_7x27_sdcc1_data, MSM_GPIO_TO_INT(SD_DETECT_N_SIGNAL), IRQF_TRIGGER_RISING|IRQF_TRIGGER_FALLING); // SD slot
...
MACHINE_START(SOCIAL, MOT_HW_REV)
#ifdef CONFIG_MSM_DEBUG_UART
        .phys_io        = MSM_DEBUG_UART_PHYS,
        .io_pg_offst    = ((MSM_DEBUG_UART_BASE) >> 18) & 0xfffc,
#endif
        .fixup      = mot_7x27_fixup,
        .boot_params    = 0x13600100,
        .map_io         = mot_7x27_map_io,
        .init_irq       = mot_7x27_init_irq,
        .init_machine   = mot_7x27_init,
        .timer          = &msm_timer,
