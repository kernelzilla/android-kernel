...
int register_gpio_chip(struct gpio_chip *new_gpio_chip)
{
	...
	if ((m = (unsigned long long)(new_gpio_chip->end + 1 - new_gpio_chip->start) * sizeof(new_gpio_chip->state[0])) > 0xffffffff) {
		printk(KERN_ERR "register_gpio_chip: integer overflow caused by arguments\n");
		return -EINVAL;
	}
	...
	if (chip_array_end_index >= gpio_chip_array_size) {
		...
		if ((m = (unsigned long long)new_gpio_chip_array_size * sizeof(new_gpio_chip_array[0])) > 0xffffffff) {
			printk(KERN_ERR "register_gpio_chip: integer overflow caused by arguments\n");
			err = -EINVAL;
			goto failed;
		}
