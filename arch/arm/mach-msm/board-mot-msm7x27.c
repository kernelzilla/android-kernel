...
#ifdef CONFIG_LEDS_RT9378
#include <linux/leds-rt9378.h>
#endif /* CONFIG_LEDS_RT9378 */
...
#endif
#ifdef CONFIG_KIONIX_KXTF9_ACCELEROMETER
#include <linux/kxtf9.h>
#endif
#ifdef CONFIG_INPUT_SFH7743
#include <linux/sfh7743.h>
#endif
#ifdef CONFIG_MACH_CIENA
#include "timed_vibrator.h"
#include "board-mot.h"
#endif

/*Add for CIENA GPIO Buttons by a20590@mot */
int ciena_gpio_power(const struct gpio_event_platform_data *pdata, bool on)
{

        return 0;
};

static struct gpio_event_direct_entry ciena_gpio_buttons[] = {
        { 37, KEY_SEND },
        { 38, KEY_END },
        { 39, KEY_VOLUMEUP },
        { 40, KEY_VOLUMEDOWN },
...
static struct gpio_event_input_info ciena_gpio_button_info = {
        .info.func = gpio_event_input_func,
        .flags = GPIOEDF_PRINT_KEYS | GPIOEDF_PRINT_KEY_DEBOUNCE,
        .poll_time.tv.nsec = 40 * NSEC_PER_MSEC,
        .type = EV_KEY,
        .keymap = ciena_gpio_buttons,
        .keymap_size = ARRAY_SIZE(ciena_gpio_buttons)
};

static struct gpio_event_info *ciena_gpio_event_info[] = {
        &ciena_gpio_button_info.info
};

static struct gpio_event_platform_data ciena_gpio_event_data = {
        .name = "ciena-gpio-event",
        .info = ciena_gpio_event_info,
        .info_count = ARRAY_SIZE(ciena_gpio_event_info),
        .power = ciena_gpio_power,
};

static struct platform_device ciena_gpio_device = {
        .name = GPIO_EVENT_DEV_NAME,
        .id = 2,
        .dev = {
                .platform_data = &ciena_gpio_event_data,
        },
};


#ifdef CONFIG_LEDS_PM7540_NOTIF
#include <proc_comm.h>
#endif /* CONFIG_LEDS_PM7540_NOTIF */
...
static struct resource mxt224_resources[]  =  {
       {
               .start  = MXT224_I2C_ADDR,
               .end    = MXT224_BL_I2C_ADDR,
               .flags  = IORESOURCE_MEM,
       },
};
static struct platform_device mot_mxt224_device = {
       .name           = "mXT224",
       .id             = 0,
       .num_resources  = ARRAY_SIZE(mxt224_resources),
       .resource       = mxt224_resources,
};
#endif /* CONFIG_TOUCHSCREEN_MXT224 */

#ifdef CONFIG_KIONIX_KXTF9_ACCELEROMETER
#define KXTF_INT_N          18
#define KXTF9_INT_GPIO      18
#define KXTF9_I2C_ADDR      0x0F

struct kxtf9_platform_data accelerometer_kxtf9_data = {
	.min_interval	= 2,
	.poll_interval	= 200,
...
struct platform_device kxtf9_platform_device = {
	...
	.id = -1,
	.dev = {
		.platform_data = &accelerometer_kxtf9_data,
	},
};
#endif
...
static struct msm_hsusb_platform_data msm_hsusb_pdata = {
	...
	.vendor_id          = MOT_VID,
        .product_id         = MOT_PID,
	.product_name       = "Motorola CIENA",
	.serial_number      = "Ciena",
	.manufacturer_name  = "Motorola Incorporated",
	.compositions	= usb_func_composition,
	.num_compositions = ARRAY_SIZE(usb_func_composition),
...
static unsigned bt_config_power_on[] = {
	GPIO_CFG(113, 0, GPIO_OUTPUT, GPIO_NO_PULL, GPIO_2MA),	/* WAKE */
	...
	GPIO_CFG(114, 0, GPIO_INPUT,  GPIO_NO_PULL, GPIO_2MA),	/* HOST_WAKE */
};
static unsigned bt_config_power_off[] = {
	GPIO_CFG(113, 0, GPIO_INPUT, GPIO_PULL_DOWN, GPIO_2MA),	/* WAKE */
	...
	GPIO_CFG(114, 0, GPIO_INPUT, GPIO_PULL_DOWN, GPIO_2MA),	/* HOST_WAKE */
};
...
static struct rt9378_platform_data rt9378_data = {
    .name = "rt9378",
    .gpio_en = 89,
};

static struct platform_device mot_rt9378_device = {
    .name = "rt9378",
    .id = -1,
    .dev = {
        .platform_data = &rt9378_data,
    },
};
...
#ifdef CONFIG_LEDS_PM7540_NOTIF
static struct platform_device pm7540_notif_led_device = {
    .name = "pm7540-notif-leds"
};
#endif /* CONFIG_LEDS_PM7540_NOTIF */

#ifdef CONFIG_LEDS_PM7540_SOC
static struct platform_device pm7540_soc_led_device = {
    .name = "pm7540-soc-leds",
};
#endif /* CONFIG_LEDS_PM7540_SOC */
...
#ifdef CONFIG_KIONIX_KXTF9_ACCELEROMETER
    {
      I2C_BOARD_INFO("kxtf9", KXTF9_I2C_ADDR),
      .irq = MSM_GPIO_TO_INT(KXTF_INT_N),
      .platform_data = &accelerometer_kxtf9_data,
    },
#endif
#ifdef CONFIG_VB6801
...
#endif
#ifdef CONFIG_TOUCHSCREEN_MXT224
        /* I2C for Touch AT42QT602240*/
        {
                I2C_BOARD_INFO("mXT224", MXT224_I2C_ADDR),
                .irq = MSM_GPIO_TO_INT(TOUCH_INT_N),
                .platform_data = &mot_mxt224_device
        },
#endif /* CONFIG_TOUCHSCREEN_MXT224 */
};
...
static int cam_pwrdwn_gpio_state = 0;

static void config_camera_on_gpios(void)
{
        /* release the cam_pwrdwn GPIO*/
        if (cam_pwrdwn_gpio_state)
        {
                gpio_free(0);
                cam_pwrdwn_gpio_state = 0;
        }
...
static void config_camera_off_gpios(void)
{
	config_gpio_table(camera_off_gpio_table,
        ...
        /* drive the cam_pwrdwn GPIO high */
        if (!cam_pwrdwn_gpio_state)
        {
                gpio_request(0, "cam_pwrdwn");
                gpio_direction_output(0, 1);
                cam_pwrdwn_gpio_state = 1;
        }
}
...
static struct msm_camera_sensor_info msm_camera_sensor_mot_camera_data = {
...
static struct platform_device msm_camera_sensor_mot_camera = {
        .name      = "msm_camera_mot_camera",
        .dev       = {
                .platform_data = &msm_camera_sensor_mot_camera_data,
        },
};
#endif
...
#endif
#ifdef CONFIG_LEDS_PM7540_NOTIF
    &pm7540_notif_led_device,
#endif
#ifdef CONFIG_LEDS_PM7540_SOC
    &pm7540_soc_led_device,
#endif
   &msm_camera_sensor_mot_camera,
};

static struct msm_panel_common_pdata mdp_pdata = {
	.gpio = 97,
};
#ifdef CONFIG_FB_MSM_MDDI_TPO_QVGA

#define LCD_RST_N_SIGNAL 128
#define LCD_LED_SIGNAL 89
#define LCD_RST_DELAY 5
...
static void msm_lcd_backlight(void)
{
    printk(KERN_INFO "lcd backlight\n");
    gpio_set_value(LCD_LED_SIGNAL, 1);
    return;
}
...
static uint32_t msm_sdcc1_setup_power(struct device *dv, unsigned int vdd)
{
	int rc = 0;
...
static struct mmc_platform_data msm7x2x_sdcc1_data = {
	.ocr_mask	= MMC_VDD_28_29,
	.translate_vdd	= msm_sdcc1_setup_power,
	.mmc_bus_width  = MMC_CAP_4_BIT_DATA,
};

static struct mmc_platform_data msm7x2x_sdcc2_data = {
	.ocr_mask	= MMC_VDD_28_29,
...
static void __init msm7x2x_init_mmc(void)
{
	...
	sdcc_gpio_init();
#ifdef CONFIG_MMC_MSM_SDC1_SUPPORT
	msm_add_sdcc(1, &msm7x2x_sdcc1_data, 0, 0);
#endif
...
{
#ifdef CONFIG_MMC_MSM_SDC2_SUPPORT
    msm_add_sdcc(2, &msm7x2x_sdcc2_data, 0, 0);
#endif
}
...
static void __init msm_device_i2c_init(void)
{
	...
	/* will be delete for Sesame
	if (gpio_request(91, "i2c_sec_clk"))
		pr_err("failed to request gpio i2c_sec_clk\n");
	if (gpio_request(92, "i2c_sec_dat"))
...
void msm_blink_messaging_led(uint16_t onoff, uint16_t level,
    uint16_t on_ms, uint16_t off_ms)
{
    ...
    ret = msm_proc_comm(PROC_NOTIF_LED, &param1, &param2);
    if (ret) {
        printk (KERN_ERR "%s: msm_proc_comm error: %d\n",
            __FUNCTION__, ret);
    }
}
EXPORT_SYMBOL(msm_blink_messaging_led);
#endif
