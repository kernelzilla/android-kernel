...
#include "board-mot.h" /* For MSM_SERVER_SYNC_COUNT */
...
static int process_control_msg(union rr_control_msg *msg, int len)
{
	...
	case RPCROUTER_CTRL_CMD_NEW_SERVER:
		...
		printk(KERN_INFO "o NEW_SERVER id=%d:%08x prog=%08x:%08x\n",
...
#if defined(CONFIG_KERNEL_MOTOROLA)
	printk(KERN_INFO "%s: Waiting for first rpc registration\n",
			__func__);
	wait_event_interruptible(newserver_wait,
			(msm_server_exists >= MSM_SERVER_SYNC_COUNT));
#endif /* defined(CONFIG_KERNEL_MOTOROLA) */
	return 0;
