...
#if defined( CONFIG_KERNEL_MOTOROLA) && !defined(CONFIG_MACH_CIENA)
int power_key_enable( uint8_t value)
{

	if(value)
	{
             hs->enable_power_key=1;
	}
	else
	{
            hs->enable_power_key=0;
	}
	return 1;
}

EXPORT_SYMBOL(power_key_enable);
#endif
#endif
/*
