...
{
   memcpy(batt_info, smem_alloc(SMEM_BATTERY_INFO, sizeof(mot_battery_info)), sizeof(mot_battery_info));
}
...
void set_mot_battery_temp(int val)
{
   ...
   if (!temp) {
      batt_info = (mot_battery_info*) smem_alloc(SMEM_BATTERY_INFO, sizeof(mot_battery_info));      
