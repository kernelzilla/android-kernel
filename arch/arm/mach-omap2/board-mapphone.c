...
char *bp_model = "CDMA";
static int mapphone_panel_attached = 1;
...
static void mapphone_touch_init(void)
{
	...
	if ((touch_node = of_find_node_by_path(DT_PATH_TOUCH))) {
		...
		if ((touch_prop = of_get_property(touch_node, DT_PROP_TOUCH_T9, &len))) {
			mapphone_ts_platform_data.multi_touch_cfg[0]
				= *(struct qtm_touch_multi_cfg *)touch_prop;
			if (len > sizeof(struct qtm_touch_multi_cfg))
				mapphone_ts_platform_data.multi_touch_cfg[1]
					= *(((struct qtm_touch_multi_cfg *)touch_prop) + 1);
		}
		...
		if ((touch_prop = of_get_property(touch_node, DT_PROP_TOUCH_T20, &len))) {
			mapphone_ts_platform_data.grip_suppression_cfg[0]
				= *(struct  qtm_proci_grip_suppression_cfg*)touch_prop;
			if (len > sizeof(struct qtm_proci_grip_suppression_cfg))
				mapphone_ts_platform_data.grip_suppression_cfg[1]
					= *(((struct  qtm_proci_grip_suppression_cfg*)touch_prop) + 1);
		}
...
static void mapphone_als_init(void)
{
	...
	if (als_node != NULL) {
		...
		als_val = of_get_property(als_node, DT_PROP_SCALE_FACTOR,
									&len);
		if (als_val && len)
			omap3430_als_light_data.scale_factor = *als_val;
		else
			pr_err("%s: Cann't get scale factor\n", __func__);

		als_val = of_get_property(als_node, DT_PROP_BRIGHTNESS_MOD_PERC,
									&len);

		if (als_val && len)
			omap3430_als_light_data.brightness_mod_perc = *als_val;
		else
			pr_err("%s: Can't get brightness mod perc\n", __func__);

		res_val = of_get_property(als_node, DT_PROP_ALS_RESISTOR_VAL,
									&len);
		if (res_val && len) {
			for (i = 0; i < 16; i++)
				omap3430_als_light_data.als_resistor_val[i] =
				*((u16 *)res_val + i);
		} else
			pr_err("%s: Cann't get als resistor val\n", __func__);

		of_node_put(als_node);
	}

	if (!mapphone_panel_attached) {
		omap3430_als_light_data.power_up_gen_config = 0x1A;
		omap3430_als_light_data.gen_config          = 0x1A;
		omap3430_als_light_data.manual_current      = 0x1A;
	}
...
static struct lm3530_platform_data omap3430_als_light_data = {
	...
	.scale_factor = 0,
	.brightness_mod_perc = 0,
	.als_resistor_val = {1, 9260, 4630, 3090, 2310, 1850, 1540, 1320, 1160,
					1030, 925, 842, 772, 712, 661, 617},
};
...
static struct i2c_board_info __initdata
...
#if CONFIG_KEYBOARD_ADP5588
	{
		...
		.irq = OMAP_GPIO_IRQ(ADP5588_INT_GPIO),
	},
#endif
...
void mapphone_no_panel_attached(void)
{
	mapphone_panel_attached = 0;
}
