...
static int mapphone_hdtv_pwr_en_gpio;	/* 0 by default */
...
static int mapphone_panel_enable(struct omap_dss_device *dssdev)
{
	...
	regulator_enable(display_regulator);
	msleep(1);
	gpio_request(mapphone_lcd_device.reset_gpio, "display reset");
...
static int mapphone_dt_get_dsi_panel_info(void)
{
	...
	panel_prop = of_get_property(panel_node, "dsi_ddr_clk_hz", NULL);
	if (panel_prop != NULL)
		mapphone_lcd_device.phy.dsi.ddr_clk_hz = *(u32 *)panel_prop;

	panel_prop = of_get_property(panel_node, "dsi_lp_clk_hz", NULL);
	if (panel_prop != NULL)
		mapphone_lcd_device.phy.dsi.lp_clk_hz = *(u32 *)panel_prop;
	...
	PANELDBG(" gpio_reset= %d panel_id= 0x%lx lp_clk= %ld ddr_clk= %ld\n",
		mapphone_lcd_device.reset_gpio,
		mapphone_lcd_device.panel.panel_id,
		mapphone_lcd_device.phy.dsi.lp_clk_hz,
		mapphone_lcd_device.phy.dsi.ddr_clk_hz);
...
static int mapphone_dt_get_tda19989_info(void)
{
	...
	mapphone_hdtv_pwr_en_gpio = mapphone_tda19989_data.pwr_en_gpio;
...
void panel_print(void)
{
		...
		PANELDBG(" DT: lp_clk= %ld ddr_clk= %ld\n",
			mapphone_lcd_device.phy.dsi.lp_clk_hz,
			mapphone_lcd_device.phy.dsi.ddr_clk_hz);
#if 0
...
void __init mapphone_panel_init(void)
{
	...
	if (mapphone_feature_hdmi) {
		...
		ret = gpio_request(mapphone_hdtv_pwr_en_gpio,
							"HDMI-power-enable");
		if (ret) {
			printk(KERN_ERR "Failed hdtv pwr en gpio request\n");
			goto failed_mux_en;
		}
		gpio_direction_output(mapphone_hdtv_pwr_en_gpio, 0);
		gpio_set_value(mapphone_hdtv_pwr_en_gpio, 0);
		gpio_free(mapphone_hdtv_pwr_en_gpio);
