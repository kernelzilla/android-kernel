...
static inline u8 get_vsel_for_opp(struct omap_sr *sr, int opp)
{
	...
	if (sr->last_opp != 0 && (sr->last_opp == (opp+1))) {
			...
			u32 target_opp_no, u8 target_vsel, u8 current_vsel)
{
	...
	vpconfig_value |=
	    ((target_opp_no <
	      sr->opp_boundary) ? sr_errgain_lowopp : sr_errgain_highopp) <<
			     ...
			     u8 target_vsel, u8 current_vsel)
{
	...
	struct omap_sr_vp *vp = &sr->vp;
	temp = prm_read_mod_reg(OMAP3430_GR_MOD, vp->prm_vpx_voltage_offset);
	if (current_vsel > target_vsel && temp <= target_vsel)
 ...
 */
#if defined(SR_TURBO) || defined(SR_HIGHSPEED)
void ComputeNadj(u32 *Sen_RN, u32 *Sen_Gain, u32 SenGainfuse,
		 u32 SenRNfuse, u32 Delta)
{
	u32 Nadj, R, G;
