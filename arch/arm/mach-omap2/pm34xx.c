...
void show_prcm_reg(void)
{
	printk(KERN_INFO "CM_CLKSEL1_PLL_MPU=0X%08X, \
		CM_CLKSEL2_PLL_MPU=0X%08X\n",
		omap_readl(0x48004940), omap_readl(0x48004944));
	printk(KERN_INFO "CM_CLKSEL1_PLL_IVA2=0X%08X, \
		CM_CLKSEL2_PLL_IVA2=0X%08X\n",
		omap_readl(0x48004040), omap_readl(0x48004044));
	printk(KERN_INFO "CM_CLKSEL1_PLL=0X%08X, \
		CM_CLKSEL2_PLL=0X%08X, CM_CLKSEL3_PLL=0X%08X, \
		CM_CLKSEL4_PLL=0X%08X, CM_CLKSEL5_PLL=0X%08X\n",
		omap_readl(0x48004D40), omap_readl(0x48004D44),
		omap_readl(0x48004D48), omap_readl(0x48004D4C),
		omap_readl(0x48004D50));
	printk(KERN_INFO "PRM_VP1_VOLTAGE=0x%08x, \
		PRM_VP2_VOLTAGE=0x%08x \n",
		omap_readl(0x483072c0), omap_readl(0x483072e0));

}
void show_die_id(void)
{
	printk(KERN_INFO "CONTROL.CONTROL_IDCODE=0X%08X\n",
		omap_readl(0x4830a204));
	printk(KERN_INFO "CONTROL.CONTORL_PRODTION_ID=\
		0X%08x:0X%08x:0X%08x:0X%08x:\n",
		omap_readl(0x4830a208), omap_readl(0x4830a208 + 0x4),
		omap_readl(0x4830a208 + 0x8), omap_readl(0x4830a208 + 0xc));
	printk(KERN_INFO "CONTROL.CONTORL_DIE_ID=\
		0X%08x:0X%08x:0X%08x:0X%08x:\n",
		omap_readl(0x4830a218), omap_readl(0x4830a218 + 0x4),
		omap_readl(0x4830a218 + 0x8), omap_readl(0x4830a218 + 0xc));
}
...
static irqreturn_t prcm_interrupt_handler (int irq, void *dev_id)
{
	...
	do {
		...
		/* only clear the interrupts which are enabled */
...
void omap_sram_idle(void)
{
	...
	*/
	...
	if ((core_next_state < PWRDM_POWER_ON ||
		mpu_next_state < PWRDM_POWER_ON) && !get_deepsleep_mode()) {
...
int set_pwrdm_state(struct powerdomain *pwrdm, u32 state)
{
	...
	/*
	 * FIXME
	 * For omap3430, DSS, PER and CORE OFF mode is not supported.
	 *
	 * Temporarily disabled OFF mode for omap3630 until ES1.1 is applied.
	 */
	if (cpu_is_omap34xx())
...
#define _WKST_REG(mod, wkst_off, grp_sel_off, value)( \
{                                    \
	value = prm_read_mod_reg(mod, wkst_off); \
	value &= prm_read_mod_reg(mod, grp_sel_off); \
}                                    \
)

#define _DUMP_REG(mod, value, buf, len)( \
{                                    \
	if (value) \
		len += sprintf(buf+len, "%s(0x%x) ", #mod, value); \
}                                    \
)

static void dump_mod_wkst_reg(void)
{
	u32 v = 0;
	int len = 0;
	char wkst_str[160];

	/* Only dump info that wake up from suspend LPM status*/
	if (suspend_state != PM_SUSPEND_MEM)
		return;

	_WKST_REG(WKUP_MOD, PM_WKST, OMAP3430_PM_MPUGRPSEL, v);
	_DUMP_REG(WKUP, v, wkst_str, len);

	_WKST_REG(CORE_MOD, PM_WKST1, OMAP3430_PM_MPUGRPSEL, v);
	_DUMP_REG(CORE1, v, wkst_str, len);

	_WKST_REG(CORE_MOD, OMAP3430ES2_PM_WKST3, OMAP3430ES2_PM_MPUGRPSEL3, v);
	_DUMP_REG(CORE3, v, wkst_str, len);

	_WKST_REG(OMAP3430_PER_MOD, PM_WKST, OMAP3430_PM_MPUGRPSEL, v);
	_DUMP_REG(PER, v, wkst_str, len);

	_WKST_REG(OMAP3430ES2_USBHOST_MOD, PM_WKST, OMAP3430_PM_MPUGRPSEL, v);
	_DUMP_REG(USBHOST, v, wkst_str, len);

	v = prm_read_mod_reg(OCP_MOD, OMAP2_PRM_IRQSTATUS_MPU_OFFSET);
	_DUMP_REG(PRM_IRQSTATUS, v, wkst_str, len);

	v = __raw_readl(IO_ADDRESS(OMAP34XX_IC_BASE + (0x0098)));
	_DUMP_REG(INTC_IRQ0, v, wkst_str, len);

	v = __raw_readl(IO_ADDRESS(OMAP34XX_IC_BASE + (0x00b8)));
	_DUMP_REG(INTC_IRQ1, v, wkst_str, len);

	v = __raw_readl(IO_ADDRESS(OMAP34XX_IC_BASE + (0x00d8)));
	_DUMP_REG(INTC_IRQ2, v, wkst_str, len);

	/* print the real wkup sources */
	printk(KERN_INFO "WAKEUP BY: %s\n", wkst_str);
}
...
static void __init prcm_setup_regs(void)
{
	...
	prm_write_mod_reg(0xffffffff, OCP_MOD, OMAP2_PRM_IRQSTATUS_MPU_OFFSET);
		...
		struct kobj_attribute *attr, char *buf)
{
	...
	for (i = 1; i <= num_mpu_opps; i++)
		if (mpu_opps[i].rate)
			tbuf += sprintf(tbuf, "mpu %d:\t0x%02x\t\t0x%02x\t0x%08x\t0x%08x\n", i,
					mpu_opps[i].vsel,
	...
	for (i = 1; i <= num_l3_opps; i++)
		if (l3_opps[i].rate)
			tbuf += sprintf(tbuf, "l3 %d:\t0x%02x\t\t0x%02x\t0x%08x\t0x%08x\n", i,
					l3_opps[i].vsel,
