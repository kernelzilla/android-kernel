...
static char *hsmmc_get_pwr_supply(void)
{
	char *hsmmc_regulator = "vwlan2";
#ifdef CONFIG_ARM_OF
	const void *pwr_supply_prop;
	struct device_node *hsmmc_node =
		of_find_node_by_path("/System@0/SDHC@0/SDHCSLOT@0");

	if (hsmmc_node) {
		pwr_supply_prop = of_get_property(hsmmc_node, \
						"pwr_supply", NULL);
		if (pwr_supply_prop) {
			if (*(u32 *)pwr_supply_prop == 0x0)
				hsmmc_regulator = "vsdio";
			else if (*(u32 *)pwr_supply_prop == 0x1)
				hsmmc_regulator = "vwlan2";
		}
		of_node_put(hsmmc_node);
	}
#endif
	return hsmmc_regulator;
}
...
static int hsmmc_late_init(struct device *dev)
{
	...
	if (ret < 0)
		goto err2;
	hsmmc_regulator = regulator_get(NULL, hsmmc_get_pwr_supply());
