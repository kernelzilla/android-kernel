...
extern int iIpcW;
extern int iLogR;
extern unsigned long aIpcR[IPC_DBG_ARRAY_SIZE];
extern unsigned long aIpcW[IPC_DBG_ARRAY_SIZE];
extern unsigned long aLogR[IPC_DBG_ARRAY_SIZE];
#define LOG_IPC_ACTIVITY(array, index, event) \
	do { \
		array[index++] = event; \
		if (index >= IPC_DBG_ARRAY_SIZE) \
			index = 0; \
	} while (0)
#else
#define LOG_IPC_ACTIVITY(array, index, event)
