...
static unsigned char tbuf[2000];
static int tbuf_len;
				...
				struct sk_buff *skb)
{
	...
	if (hdr == (HIDP_TRANS_DATA | HIDP_DATA_RTYPE_INPUT)) {
		if ((skb->len+1) == L2CAP_DEFAULT_MTU) {
			...
			BT_DBG("start acuumatling data");
			for (loop = 0; loop < skb->len; loop++)
				tbuf[tbuf_len++] = skb->data[loop];
			BT_DBG("start acuumatling data %d", tbuf_len);
			kfree_skb(skb);
			return;
		}
		hidp_set_timer(session);
		...
		BT_DBG("start acuumatling data %d", tbuf_len);
		if ((skb->len+1) < L2CAP_DEFAULT_MTU) {
			/* Pkt ends here.Process it */
			hidp_set_timer(session);
			if (session->hid) {
				hid_input_report(session->hid,
					HID_INPUT_REPORT, tbuf, tbuf_len , 1);
				BT_DBG("report len %d", skb->len);
			}
			tbuf_len = 0;
			memset(&tbuf, 0, 2000);
