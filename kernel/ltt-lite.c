 ...
 * for logging long time. The minimal size of the file is 2MB. If this value equal zero,
 * means there is no maximal size limit.
 * 
 * @example
...
#define DPRINT(args...) printk(KERN_DEBUG args)
#else
#define DPRINT(args...)
#endif
...
/* Using 32 chars to display Android Application process names */
#define ANDROID_TASK_NAME_LEN 32
...
struct process_table_log {
	...
	char comm[ANDROID_TASK_NAME_LEN];//TASK_COMM_LEN];
};
...
static unsigned long ltt_lite_buf_size = 0;
static unsigned long ltt_lite_privateram_size;
static unsigned long ltt_lite_half_buf_size = 0;
static unsigned long ltt_lite_file_size = 0;
...
static unsigned long early_buf_pos = 0;
/* indicate if ltt lite is enabled */
static bool ltt_lite_is_enabled = false;

static bool use_private_ram = false;
static long *pevent_count;
static char *pevent_table; 
#define PEVENT_COUNT_MAX 1000
static bool use_private_ram_file = false;
...
static ltt_lite_mode_t ltt_lite_mode = LTT_LITE_MODE_DEFAULT; 
...
static syscall_group_t syscall_mask_group_id = SYSCALL_GROUP_ALL;
...
static unsigned short ltt_lite_syscall_mask[SYSCALL_GROUP_IPC + 1][MASK_ARRAY_LEN]
= {
...
static bool early_enabled_mode = false;
static bool early_tracing_is_allowed = false;
/* variable used to record pid filter rule number (0~3) */
static unsigned short ltt_lite_pid_filter_num = 0;
...
static unsigned short ltt_lite_int_filter_num = 0;
...
static char* get_privateram_address(void)
{
	if (ltt_lite_res.start && !ltt_lite_privateram_virtaddr){
		ltt_lite_privateram_virtaddr = (char*)ioremap(ltt_lite_res.start, ltt_lite_privateram_size);
	}
 ...
 * Free reserved pages
 */
static void free_reserved_pages(void* mem, struct page **pagearray, int pagecount)
{
	int j;
	if (mem)
		vunmap(mem);
...
static int process_name_table_init(void)
{
	...
	memset(&process_log.header, 0, sizeof(record_header_t));
	...
	while ((p = next_task(p)) != &init_task) {
		...
		do {
			...
			if (t->pid == t->tgid)
		       	...
		       	process_log.ppid = t->tgid;
			memcpy(process_log.comm, t->comm, ANDROID_TASK_NAME_LEN);
...
static void wq_init(struct work_struct* ignore)
{
	 ...
	 * low priority lttliter may not be scheduled in a long period,
	 * thus ltt-lite may lose records.
	 */
	set_user_nice(current, -20);
	if ((retval = sched_setscheduler(current, SCHED_FIFO, &param)))
		printk(KERN_ERR "LTT-LITE: setscheduler(): %d.\n", retval);

	set_fs(get_ds());
	if ((log_file_fd = sys_open((const char __user *)
					log_file, O_RDWR | O_CREAT | O_TRUNC,
					0600)) < 0) {
	...
	if (!ltt_lite_buf) {
		if (!use_private_ram_file) {
			...
			if (!buf) {
				printk(KERN_ERR "cannot get pages for ltt_lite_buf\n");
			}
			else {
				ltt_lite_buf_size = PAGE_SIZE * (1 << LTT_LITE_PAGE_ORDER) - sizeof(buf_info_t);
			}
		} else {
			...
			ltt_lite_buf_size = ltt_lite_privateram_size - sizeof(buf_info_t);
		}
		if (!buf) {
		...
		buf_info = (buf_info_t *)buf;
		ltt_lite_buf = buf + sizeof(buf_info_t);
		ltt_lite_half_buf_size = ltt_lite_buf_size / 2;
		memset(buf_info, 0, sizeof(buf_info_t));
	...
	if (early_enabled_mode) {
		...
		pid = kernel_thread(write_early_buf, NULL, 0);
		if (pid < 0) {
			printk(KERN_ERR "LTT-LITE: create kernel thread failed\n");
		}
	}
...
static void ltt_lite_write_privateram(void) 
{
	...
	if ((log_file_fd = sys_open((const char __user *)log_file, 
	...
	buf = get_privateram_address();
	if (buf == NULL) {
		printk(KERN_ALERT "Error in mapping LTTLITE partition offset\n");
		if (sys_close(log_file_fd))
			printk(KERN_ERR	"%s: close error\n", __FUNCTION__);
		return;
	}
	...
	if (buf_info->size != ltt_lite_buf_size) {
		printk(KERN_ALERT "Buf info not correct! %ld\n", buf_info->size);
	}
...
static void ltt_lite_append_log()
{
	...
	if ((buf_info->size != ltt_lite_buf_size) ||
			(buf_info->is_top && (buf_info->buf_pos > ltt_lite_half_buf_size)) ||
			(!buf_info->is_top && (buf_info->buf_pos <= ltt_lite_half_buf_size))) {
		printk(KERN_ALERT "Buf info not correct, will not append!%lu, %lu\n", 
	...
	if (buf_info->log_file_pos != -1) {
		result = sys_lseek(log_file_fd, buf_info->log_file_pos, SEEK_SET);
		if (result < 0) {
	...
	if (buf_info->is_top) {
		if (buf_info->buf_bottom_full) {
			result = sys_write(log_file_fd, ltt_lite_buf +										ltt_lite_half_buf_size, ltt_lite_half_buf_size);
			if (result < 0) {
				printk(KERN_ERR "%s: write error - %d\n", __FUNCTION__, result);
				goto err_out;
			}
		}
		result = sys_write(log_file_fd, ltt_lite_buf, buf_info->buf_pos);
	}
	else {
		...
		result = sys_write(log_file_fd, ltt_lite_buf + 
				ltt_lite_half_buf_size, buf_info->buf_pos -
				ltt_lite_half_buf_size);
	}
	if (result < 0) {
		printk(KERN_ERR "%s: write error - %d\n", __FUNCTION__, result);
	...
	memset(&lreport.header, 0, sizeof(record_header_t));
	...
	memcpy(&lreport.reports,
			...
			REPORT_ITEM_LEN);
	result = sys_write(log_file_fd, (char*)&lreport, sizeof(lreport));
	if (result < 0) {
	...
	memset(buf_info, 0, sizeof(buf_info_t));
err_out:
	if ((result = sys_close(log_file_fd)) < 0)
		printk(KERN_ERR "%s: close error\n", __FUNCTION__);
	log_file_fd = -1;
	release_privateram_address(buf);
	ltt_lite_buf = NULL;
...
void ltt_lite_early_init(void)
{
	...
	ltt_lite_early_buf = (char *)alloc_reserved_pages(&early_page_array, 
			ltt_lite_early_page_num);
...
static void wq_close_file(struct work_struct* ignore)
{
	...
	if (!ltt_lite_buf)
		return;
	if (!use_private_ram_file) {
		free_reserved_pages(buf_info, page_array, 1 << LTT_LITE_PAGE_ORDER);
	} else {
...
static int init_private_ram(void)
{
	...
	pevent_table = (char *)(buf + sizeof(buf_info_t));
	ltt_lite_buf = (char *)(buf + sizeof(buf_info_t) +
		PEVENT_COUNT_MAX * sizeof(struct process_table_log));
...
/*
 * proc_init_char_command - deal with char command to /proc/lttlite/init interface 
 */
		...
		const char *buffer, size_t count, loff_t *data)
{
	...
	buf = simple_strtol(string, NULL, 0);

	mode = LTT_LITE_MODE_SCHD | LTT_LITE_MODE_SYSC | LTT_LITE_MODE_MEM | LTT_LITE_MODE_TRAP |
		LTT_LITE_MODE_INT | LTT_LITE_MODE_SOFTIRQ | LTT_LITE_MODE_SIG | LTT_LITE_MODE_TMR;
	if (buf < 0 || buf > mode)
		return count;
	...
	/* start profiling */
		...
		if (use_private_ram) {
			...
			if (!ltt_lite_wq) {
				...
				ltt_lite_wq = create_singlethread_workqueue("lttliter");
				BUG_ON(!ltt_lite_wq);
		...
		if (ltt_lite_file_size) {
			...
			if (use_private_ram_file || use_private_ram){
				if ((ltt_lite_file_size * SZ_1M) < (ltt_lite_privateram_size / 2)) {
					ltt_lite_file_size = ltt_lite_privateram_size/2/SZ_1M + 1;
				}
			}
		}
	}
...
static struct file_operations proc_ltt_lite_init_operations = {
 ...
 */
static int ltt_lite_proc_config_open(struct inode *inode, struct file *file)
{
		...
		char __user *buf, size_t count, loff_t *ppos)
{
	...
	config_buf = (char *)kmalloc(OUT_STR_LEN, GFP_KERNEL);
	if (!config_buf) {
	...
	len = snprintf(config_buf, OUT_STR_LEN,
			"filesz:%luM\npriram: %d\nramfil: %d\nmemint: %d\nsysftg: %d\nlogfil: %s\npidflt: ",
			ltt_lite_file_size, use_private_ram, use_private_ram_file, 
			mem_profile_interval, syscall_mask_group_id, log_file);
 ...
 */
static int split(char *string, char **index_array, char pattern, int max_param)
{
	char *ptr;
	int count;
...
static int string2pid(char *string)
{
	...
	if (strlen(string) > PID_NUM_LEN)
		return -1;
	pidnum = simple_strtol(string, &ptr, 0);
	if (*ptr)
		return -1;
 ...
 */
static void do_cmd_filter(char * params, unsigned short* array, unsigned short* filter_num)
{
	char *char_sub_pos;
	int param_num = 0;
	int input_pid1 = -1;
	...
	param_num = split(params, substr_index, ' ', LTT_LITE_PID_FILTER_MAX);
	if (!param_num) {
	...
	printk (KERN_NOTICE "Ltt-lite pid filter not set.\n");
}
...
static void do_cmd(char * cmd_line, int cmd_type)
{
	...
	switch (cmd_type) {
		...
		case CMD_SYSCALL_FILTER_GROUP:
			...
			break;
		case CMD_PID_FILTER:
			do_cmd_filter(cmd_line, (unsigned short *)ltt_lite_pid_filter,
				&ltt_lite_pid_filter_num);
			break;
		case CMD_INT_FILTER:
			do_cmd_filter(cmd_line, (unsigned short *)ltt_lite_int_filter,
		...
		case CMD_FILE_SIZE:
			param_num = split(cmd_line, &char_sub_pos, ' ', 1);
                        /* do not change buffer during logging */
                        if (param_num) {
                                file_size = simple_strtoull(char_sub_pos, NULL, 0);
				if (file_size == 0) {
				...
				if (use_private_ram_file || use_private_ram){
					if ((file_size * SZ_1M) < (ltt_lite_privateram_size / 2)) {
						file_size = ltt_lite_privateram_size/2/SZ_1M + 1;
					}
				}
...
static struct file_operations proc_ltt_lite_config_operations = {
...
static void log_mem_status(unsigned long ignore)
{
	...
	while ((p = next_task(p)) != &init_task) {
		...
		if (!p->mm)
			continue;
		memset((void *)&mem_log, 0, sizeof(struct process_mem_log));
		mem_log.pid = p->pid;
		mem_log.start_time_sec = p->start_time.tv_sec;
		mem_log.start_time_usec = p->start_time.tv_nsec / 1000;
		mm = get_task_mm(p);
		mem_log.vmsize = mm->total_vm - mm->reserved_vm;
		mem_log.vmlck = mm->locked_vm;
		mem_log.vmrss = get_mm_rss(mm);
		mem_log.vmshr = mem_log.vmrss - get_mm_counter(mm, anon_rss);
		mem_log.vmdata = mm->total_vm - mm->shared_vm - mm->stack_vm;
...
int __init ltt_lite_init(void)
{
	...
	init_entry = create_proc_entry("init", 0666, 
			proc_lttlite_entry);
	if (init_entry == NULL) {
...
static int write_early_buf(void *ignore)
{
	...
	strncat(early_log_file, ".early", LTT_LITE_CMD_LEN - strlen(log_file));
	
	if ((early_log_file_fd = sys_open((const char __user *)early_log_file, 
					O_RDWR | O_CREAT | O_TRUNC, 0600)) < 0) {
		printk(KERN_ERR "cannot open %s, %ld\n", early_log_file,
				early_log_file_fd);
		early_log_file_fd = -1;
	...
	lreport.header.ssize = sizeof(struct ltt_lite_report_event);
	memcpy(&lreport.reports,
			&early_unlogged_count[LTT_LITE_EV_SYSCALL_ENTRY], REPORT_ITEM_LEN);
	result += sys_write(early_log_file_fd, (char *)&lreport,
			sizeof(lreport));
	if (result < 0)
		printk(KERN_ERR "%s: write error\n", __FUNCTION__);
...
static void wq_log_top_half(struct work_struct* ignore)
{
	...
	UNUSED_PARAM(ignore);
	if (ltt_lite_file_size) {
		DPRINT(KERN_DEBUG "bottom: buf_info->log_file_pos %lu\n", buf_info->log_file_pos);
		if ((buf_info->log_file_pos + ltt_lite_half_buf_size) >= ltt_lite_file_size * SZ_1M) {
...
static void wq_log_bottom_half(struct work_struct* ignore)
{
	...
	if (ltt_lite_file_size) {
		DPRINT(KERN_DEBUG "bottom: buf_info->log_file_pos %lu\n", 
				buf_info->log_file_pos);
		if ((buf_info->log_file_pos + ltt_lite_half_buf_size - buf_empty_size) 
	...
	if (result < 0)
		printk(KERN_ERR "%s: write error\n", __FUNCTION__);
	if (ltt_lite_file_size) {
		buf_info->log_file_pos += (ltt_lite_half_buf_size - buf_empty_size);
	}
	buf_info->buf_bottom_full = 0;
}
...
static void commit_log(void *addr, int size, unsigned short type)
{
	...
	if (early_enabled_mode) {
		if ((early_buf_pos + size) > ((PAGE_SIZE * ltt_lite_early_page_num) - 1)) {
		...
		if (*pevent_count <= PEVENT_COUNT_MAX &&
			...
			memcpy(pevent_table + *pevent_count *
				sizeof(struct process_table_log), addr, size);
			(*pevent_count) ++;
		}
		else {
	 ...
	 */
	if ((buf_info->buf_pos > ltt_lite_half_buf_size) && buf_info->is_top) {
		buf_info->is_top = 0;
...
void ltt_lite_ev_process_exit( void )
{
	...
	if (current->pid == current->tgid)
	    process_log.ppid = current->parent->pid;
           ...
           process_log.ppid = current->tgid;
	memcpy(process_log.comm, current->comm, ANDROID_TASK_NAME_LEN);
	commit_log(&process_log, sizeof(process_log), LTT_LITE_EV_PROCESS);
}
...
static inline int check_in_filter(unsigned short target, 
		unsigned short array[LTT_LITE_PID_FILTER_MAX][2], 
		unsigned short num)
{
	int i;
 ...
 */
void ltt_lite_ev_schedchange(ltt_schedchange* schedchange)
{
	struct ltt_lite_schedlog sched_event;

#ifdef CONFIG_MOT_FEAT_LTT_LITE_PSS
	/* increase context switch counter of current task */
	((struct log_ps_buffer*)&current->log_ps)->ctxt++;
#endif
	...
	if (!check_in_filter(schedchange->ipid, ltt_lite_pid_filter,ltt_lite_pid_filter_num) 
		&& !check_in_filter(schedchange->opid, ltt_lite_pid_filter, ltt_lite_pid_filter_num))
		return;

	memcpy(&sched_event.opid, schedchange, sizeof(ltt_schedchange));
	commit_log(&sched_event, sizeof(sched_event), LTT_LITE_EV_SCHEDULE);
}

static inline int if_log_syscall(unsigned short syscall_pid, short syscallid)
{
	if (!(ltt_lite_is_enabled | early_enabled_mode) ||
	...
	if (!check_in_filter(syscall_pid, ltt_lite_pid_filter, ltt_lite_pid_filter_num))
		return 0;
...
#ifdef CONFIG_MOT_FEAT_LTT_LITE_PSS
	/* log all sys calls */
	if (sign == LTT_LITE_EVENT_ENTER) {
		((struct log_ps_buffer*)&current->log_ps)->syscalls++;

		/* log file group sys calls */
		if (syscall_mask_group_id != SYSCALL_GROUP_ALL && syscallid >= 0) {
			/* use scno / 16 to get the index in the mask array,
			   use scno % 16 to get the bit in the mask variable */
			if ((ltt_lite_syscall_mask[1]
					[(int)(syscallid / BITS_IN_SHORT)]
					& (1 << syscallid % BITS_IN_SHORT)) != 0)
				((struct log_ps_buffer*)&current->log_ps)->file_syscalls++;
		}
	}
#endif
...
void ltt_lite_ev_log_process(int type, struct task_struct* p)
{
...
#ifdef CONFIG_MOT_FEAT_LTT_LITE_PSS
	/* initilize log_ps filed in task struct */
	if(type == LTT_LITE_EV_PROCESS_FORK)
		memset((void *)&p->log_ps, 0, sizeof(p->log_ps));
#endif
	...
	if (p->pid == p->tgid)
	    process_log.ppid = p->parent->pid;
           ...
           process_log.ppid = p->tgid;
	memcpy(process_log.comm, p->comm, ANDROID_TASK_NAME_LEN);
	commit_log(&process_log, sizeof(process_log), LTT_LITE_EV_PROCESS);
}
...
void ltt_lite_ev_trap_entry(unsigned short trapid, unsigned long address)
{
	...
	if (!(ltt_lite_is_enabled | early_enabled_mode) ||
		...
		return;
	pid = current->pid;
	if (!check_in_filter(pid, ltt_lite_pid_filter, ltt_lite_pid_filter_num))
		return;
...
void ltt_lite_int_entry(unsigned short intid, char kmod)
{
	...
	if (!(ltt_lite_is_enabled | early_enabled_mode) ||
		...
		return;
	if (!check_in_filter(intid, ltt_lite_int_filter, ltt_lite_int_filter_num))
                return;
...
void ltt_lite_log_softirq(unsigned short type, unsigned int sub_type, unsigned int data)
{
	...
	soft_irq_log.sub_type = sub_type,
	soft_irq_log.data = data;
...
{
	char userinfo[sizeof(record_header_t) + LTT_LITE_MAX_LOG_STRING_SIZE];
	...
	if (size < LTT_LITE_MAX_LOG_STRING_SIZE) {
		memcpy(userinfo + sizeof(record_header_t), string, size);
		commit_log(userinfo, sizeof(record_header_t) + size, LTT_LITE_EV_STRING);
	} else {
		memcpy(userinfo + sizeof(record_header_t), string, LTT_LITE_MAX_LOG_STRING_SIZE);
...
void ltt_lite_syscall_param(int scno, char* string, int size)
{
	 ...
	 */
	if (!if_log_syscall(syscall_pid, syscallid) || (early_enabled_mode && (scno != __NR_lttlite)))
		return;

	ltt_lite_log_string(string, size);
}

void ltt_lite_ev_sig(unsigned short s_pid, unsigned short r_pid, unsigned short sig)
{
...
void ltt_lite_ev_handle_sig(unsigned short pid, unsigned short sig, unsigned long handler)
{
...
void ltt_lite_run_timer(unsigned short timer_type, unsigned long fn, unsigned long data)
{
...
asmlinkage long sys_lttlite(unsigned int cmd, unsigned long arg)
{
	switch (cmd) {
		case LLEVENT: {
			...
			snprintf(ltt_string, LTT_LITE_MAX_LOG_STRING_SIZE, "User event:pid=%d,eventid=%d\n", current->pid, (unsigned int)arg);
			ltt_lite_syscall_param(__NR_lttlite, ltt_string, strlen(ltt_string));
		...
		case LLINFO: {
			...
			if ( (len = strncpy_from_user(ltt_string, (char *)arg, LTT_LITE_MAX_LOG_STRING_SIZE - 1)) < 0 ) {
				return -EFAULT;
			}

			ltt_string[LTT_LITE_MAX_LOG_STRING_SIZE-1] = '\0';
