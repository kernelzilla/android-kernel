...
sizeof_func_data(struct bb *bb, struct gcov_fn_info *func)
{
	...
	result = /* tag */ 4 + /* length */ 4 + /* ident */ 4 +
		/* lineno_checksum */ 4 + /* cfg_checksum */ 4
		+ /* name */ sizeof_str(func->name);
...
store_string_data(const char *str, char *buf, size_t count, loff_t offset)
{
	...
	while (count > 0) {
		if (offset < 4) {
			/* Length */
			store_int32((1 + ((str_len + 4) >> 2)), data);
			len = 4 - offset;
			from = data + offset;
		} else if (offset < 4 + str_len) {
			/* String */
	...
	while (count > 0) {
		if (offset < 20) {
			if (offset < 4) {
				...
				/* Tag length in words */
				store_int32(GCOV_TAG_FUNCTION_LENGTH + \
					(name_len >> 2), data);
				len = 4 - (offset - 4);
				from = data + (offset - 4);
			} else if (offset < 12) {
				...
				store_int32(
				(uint32_t) func_ptr->lineno_checksum, data);
				len = 4 - (offset - 12);
				from = data + (offset - 12);
			} else {
				/* Function lineno_checksum */
				store_int32(
				(uint32_t) func_ptr->cfg_checksum, data);
				len = 4 - (offset - 16);
				from = data + (offset - 16);
			}
			/* Do the actual store */
