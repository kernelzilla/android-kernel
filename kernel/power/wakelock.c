			       ...
			       int count, int *eof, void *data)
{
	...
	int x;
	int type;
	int max = (count > PAGE_SIZE) ? PAGE_SIZE : count;
...
//		p += print_lock_stat(p, lock);
		len = print_lock_stat(tmpbuf, lock);
		if((p-page) + len >= max)
		{
			for(x=0; ((p-page) + x) < max; x++) p[x] = tmpbuf[x];
			goto listdone;
		}
		else
		{
			for(x=0; x < len; x++) p[x] = tmpbuf[x];
			p += len;
		}
	}
...
/// \bug I don't pay much attention to the case where off > PAGE_SIZE  .... maybe I should.
