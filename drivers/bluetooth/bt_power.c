...
static int bt_stat_read_proc(char *buffer, char **start, off_t offset, int size, int *eof, void *data)
{
  ...
  unsigned int cp;
  char str[64*13];    str[0]=0;
  char line[64];
  int len=0;

  cp=0;

  cp |= gpio_get_value(93); cp <<= 1;
  cp |= gpio_get_value(92); cp <<= 1;
  cp |= gpio_get_value(87); cp <<= 1;
  ...
  cp |= gpio_get_value(91); cp <<= 1;
  cp |= gpio_get_value(90);

  if(cp != op)
    {
      op = cp;

      sprintf(line,"%d",   gpio_get_value(93)); strcat(str,line);
      sprintf(line,"%d",   gpio_get_value(92)); strcat(str,line);
      sprintf(line,"%d ",  gpio_get_value(87)); strcat(str,line);
      ...
      sprintf(line,"%d",   gpio_get_value(91)); strcat(str,line);
      sprintf(line,"%d\n", gpio_get_value(90)); strcat(str,line);
...
static int bt_status_read_proc(char *buffer, char **start, off_t offset, int size, int *eof, void *data)
{
  ...
  sprintf(line,"%d = BT_RESET\n",   gpio_get_value(93)); strcat(str,line);
  sprintf(line,"%d = BT_VREG\n",    gpio_get_value(92)); strcat(str,line);
  sprintf(line,"%d = WLAN_VREG\n\n",gpio_get_value(87)); strcat(str,line);
  ...
  sprintf(line,"%d = WAKE\n",       gpio_get_value(91)); strcat(str,line);
  ...
  if('1' == *proc_buf)
      ...
      if(!power_is_on)
	{
	  ...
	  if ((rc = vreg_enable(vreg)))
	    {printk(KERN_ERR "BLUETOOTH: %s: vreg(gp3) enable failed (%d)\n", __func__, rc);}
      ...
      if(power_is_on)
	{
	  ...
	  if(!gpio_get_value(WLAN_REG_ON_SIGNAL))
		...
		{printk(KERN_ERR "BLUETOOTH: %s: vreg(gp3) disable failed (%d)\n", __func__, rc);}
