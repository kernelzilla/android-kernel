...
DSP_STATUS handle_hibernation_fromDSP(struct WMD_DEV_CONTEXT *pDevContext)
{
	...
	if (timeout == 0) {
		...
		spin_unlock_bh(pDevContext->lock);
		if (DSP_SUCCEEDED(status)) {
			/*
			slam: why 23 does not have this
			may need to reset state pDevContext->dwBrdState =
			prev_state when DSP_FAILED
			*/
#ifdef CONFIG_BRIDGE_DVFS
	...
	for (clkIdx = 0; clkIdx < MBX_PM_MAX_RESOURCES; clkIdx++) {
		if (((pDevContext->uDspPerClks) >> clkIdx) & 0x01) {
			...
			if (BPWR_CLKID[clkIdx] == BPWR_MCBSP1) {
				/* clear MCBSP1_CLKS, on McBSP1 OFF */
				value = __raw_readl(
					pDevContext->sysctrlbase + 0x274);
				value &= ~(1 << 2);
				__raw_writel(
					value, pDevContext->sysctrlbase+0x274);
