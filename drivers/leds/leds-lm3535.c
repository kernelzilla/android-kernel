...
/* ALS Averaging time */
#define ALS_AVERAGING 0x60  // 1600ms, needs 2 ave. periods

/* Zone boundaries */
#ifdef CONFIG_MACH_PITTSBURGH
static unsigned calgary_als_zb[4] = {0x06, 0x5C, 0xDD, 0xDD};
static unsigned calgary_als_zone_max[6] = {0xEB, 0xF0, 0xFA, 0xFF, 0xFF, 0xFF};
static unsigned motus_als_zb[] = {0x03, 0x4A, 0xEE, 0xEF};
static unsigned motus_als_zone_max[] = {0x45, 0x65, 0x7F, 0x79, 0x79, 0x79};
static unsigned morrison_als_zb[] = {0x02, 0xBD, 0xDF, 0xFF};
#elif defined(CONFIG_MACH_MOT)
static unsigned morrison_als_zb[] = {0x02, 0xBD, 0xFE, 0xFF};
#else
static unsigned calgary_als_zb[4] = {0x02, 0x34, 0xB4, 0xB4};
static unsigned calgary_als_zone_max[6] = {0xE3, 0xEB, 0xF6, 0xFC, 0xFC, 0xFC};
static unsigned motus_als_zb[] = {0x03, 0x4A, 0xEE, 0xEF};
static unsigned motus_als_zone_max[] = {0x45, 0x65, 0x7F, 0x79, 0x79, 0x79};
static unsigned morrison_als_zb[] = {0x02, 0xBD, 0xDF, 0xFF};
#endif
static unsigned morrison_als_zone_max[] = {0x45, 0x6B, 0x79, 0x79, 0x79, 0x79};
/* static unsigned morrison_als_zb_old[] = {0x03, 0x7E, 0x77, 0xFF} */
static unsigned als_zb[4];
module_param_array(als_zb, uint, NULL, 0644);
static unsigned als_zone_max[6];
...
static unsigned als_curves = 1;
//module_param(als_curves, uint, 0644);
#ifndef CONFIG_MACH_MOT
#ifdef CONFIG_LM3535_ESD_RECOVERY
...
#ifdef CONFIG_MACH_MOT
unsigned do_trace = TRACE_ALS | TRACE_SUSPEND;
#else
...
/* ALS Coefficients */
static long als_z0[] = {0, 20, 14830, 6373000};
module_param_array(als_z0, long, NULL, 0644);

static long als_z1[] = {0, -65, 36774, 6870186};
module_param_array(als_z1, long, NULL, 0644);

static long als_z2[] = {0, -57, 25530, 9534242};
module_param_array(als_z2, long, NULL, 0644);

static long als_z3[] = {0, -57, 25530, 9534242};
module_param_array(als_z3, long, NULL, 0644);

static long als_z4[] = {0, -57, 25530, 9534242};
module_param_array(als_z4, long, NULL, 0644);

static unsigned long als_denom = 100000;
...
static uint8_t lm3535_convert_value (unsigned value, unsigned zone)
{
    ...
    if (!als_curves) {
        // Old way
        value = value / 2;  // Convert into LM3535 scale
        reg = value * als_zone_max[zone] / LM3535_LED_MAX;
        return reg;
    }
    switch (zone) {
        case 0:
    ...
    const struct i2c_device_id *id)
{
    ...
    if (machine_is_morrison ()) {
        memcpy (als_zone_max, morrison_als_zone_max, sizeof (als_zone_max));
        machine = "morrison";
        memcpy (als_zb, morrison_als_zb, sizeof (als_zb));
#else

    if (machine_is_calgary ()) {
        memcpy (als_zb, calgary_als_zb, sizeof (als_zb));
        memcpy (als_zone_max, calgary_als_zone_max, sizeof (als_zone_max));
	...
	if (client->addr == 0x38) {
		if (i2c_smbus_xfer(client->adapter, client->addr, 0, 0, 0,
			I2C_SMBUS_QUICK, NULL) < 0) {
				printk(KERN_ERR "%s: unable to read address"
					"0x38;switching to 0x36\n", __func__);
						client->addr = 0x36;
#else
...
static int lm3535_setup (struct i2c_client *client)
{
	...
	switch (value) {
	case 0xFF:
		lm3535_data.revision = 0;
		break;
	case 0xF0:
		lm3535_data.revision = 1;
		break;
...
/*    led_classdev_suspend (&lm3535_led_noramp); */
#ifdef CONFIG_LM3535_BUTTON_BL
...
/*    led_classdev_resume (&lm3535_led_noramp); */
#ifdef CONFIG_LM3535_BUTTON_BL
