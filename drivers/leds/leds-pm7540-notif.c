...
/*
 * PM7540 Notification LED Driver for Sesame
 *
...
#define MODULE_NAME "pm7540_notif_leds"
#define DEFAULT_ISINK_LEVEL PM_MPP__I_SINK__LEVEL_20mA
#define PM7540_SOC_LEDS_DEBUG
extern void msm_blink_messaging_led(uint16_t onoff, uint16_t level,
    uint16_t on_ms, uint16_t off_ms);
static char *isink_level_toname (unsigned level);

struct pm7540_notif_data {
    unsigned brightness;
    unsigned blink_on;
    unsigned blink_off;
...
static struct pm7540_notif_data this_data = {
    .default_level = DEFAULT_ISINK_LEVEL,
};
module_param_named (isink_level, this_data.default_level, uint, 0664);

static void pm7540_notif_brightness_set(struct led_classdev *led_cdev,
    enum led_brightness value)
{
    printk (KERN_INFO "%s: %s, %d\n", __FUNCTION__, led_cdev->name, value);
    mutex_lock (&mutex);
    this_data.brightness = value;
    msm_blink_messaging_led(value ? 1 : 0, this_data.default_level, 0, 0);
    mutex_unlock (&mutex);
}

static struct led_classdev this_led = {
    .name                  = "notification",
    .brightness_set        = pm7540_notif_brightness_set,
};

static ssize_t pm7540_notif_blink_show(struct device *dev,
...
static ssize_t pm7540_notif_blink_store(struct device *dev,
                     ...
                     const char *buf, size_t size)
{
    ...
    mutex_lock (&mutex);
    msm_blink_messaging_led(bvalue ? 1 : 0, level, msOn, msOff);
...
static DEVICE_ATTR(blink, 0666, pm7540_notif_blink_show, pm7540_notif_blink_store);
...
static ssize_t pm7540_notif_debug_show(struct device *dev,
                      struct device_attribute *attr, char *buf)
{
...
static ssize_t pm7540_notif_debug_store(struct device *dev,
                     struct device_attribute *attr,
                     const char *buf, size_t size)
{
    struct led_classdev *led_cdev;
...
static DEVICE_ATTR(debug, 0666, pm7540_notif_debug_show, pm7540_notif_debug_store);


static int pm7540_notif_leds_probe(struct platform_device *pdev)
{
    int ret;
...
static int pm7540_notif_leds_remove(struct platform_device *pdev)
{
    printk (KERN_ERR "%s: enter\n", __FUNCTION__);
    led_classdev_unregister (&this_led);
...
static struct platform_driver pm7540_notif_leds_driver = {
    .probe        = pm7540_notif_leds_probe,
    .remove        = pm7540_notif_leds_remove,
    .driver        = {
...
static int __init pm7540_notif_leds_init(void)
{
    int ret;

    printk (KERN_ERR "%s: enter\n", __FUNCTION__);
    ret = platform_driver_register(&pm7540_notif_leds_driver);
    if (ret) {
...
static void __exit pm7540_notif_leds_exit(void)
{
    printk (KERN_ERR "%s: enter\n", __FUNCTION__);
    platform_driver_unregister(&pm7540_notif_leds_driver);

}

module_init(pm7540_notif_leds_init);
module_exit(pm7540_notif_leds_exit);

MODULE_AUTHOR("Jiaping Lv a19234@motorola.com");
MODULE_DESCRIPTION("PM7540 NOTIFICATION LED driver");
MODULE_LICENSE("GPL");
