...
static void led_drvx_do_work(struct work_struct *work)
{
	...
	ldata = container_of(work, struct pm8058_led_data, led_drvx_delayed_work.work);
...
static void pm8058_current_led_brightness_set(struct led_classdev *led_cdev,
			       enum led_brightness brightness)
{
	if (brightness)
		pmic_high_current_led_set_current(PM_KBD_DRV_LED, 100);
	else
		pmic_high_current_led_set_current(PM_KBD_DRV_LED, 0);
}
			       ...
			       enum led_brightness brightness)
{
	...
	pwm_disable(ldata->pwm_led);
	if (brightness) {
		if (ldata->flags & PM8058_LED_DRVX_BLINK_EN) {
...
*/
	...
	} else if (ldata->flags & PM8058_LED_DRVX_BLINK_EN)
		pmic_low_current_led_set_current(ldata->bank - 4, 0);
			       ...
			       enum led_brightness brightness)
{
	...
	ldata = container_of(led_cdev, struct pm8058_led_data, ldev);
	pmic_low_current_led_set_current(ldata->bank - 4, 0);
	cancel_delayed_work_sync(&ldata->led_drvx_delayed_work);
		...
		if (ldata->flags & PM8058_LED_LTU_EN) {
			if (ldata->flags & PM8058_LED_FADE_EN) {
				...
				pm8058_pwm_lut_config(ldata->pwm_led,
						...
						0, 0,   ldata->lut_flag);
				pm8058_pwm_lut_enable(ldata->pwm_led, 1);
				queue_delayed_work(g_led_work_queue, &ldata->led_drvx_delayed_work, msecs_to_jiffies(1000));
				return ;
			}
			pm8058_pwm_lut_enable(ldata->pwm_led, 0);
		}
...
static int pm8058_led_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < pdata->num_leds; i++) {
		...
		case PM8058_LED_CURRENT:
			ldata[i].ldev.brightness_set =
					pm8058_current_led_brightness_set;
			break;
		case PM8058_LED_RGB:
		case PM8058_LED_PWM:
			ldata[i].pwm_led = pwm_request(
						pdata->led_config[i].bank,
						ldata[i].ldev.name);
			ldata[i].ldev.brightness_set =
					pm8058_pwm_led_brightness_set;
			break;
		case PM8058_LED_DRVX:
			INIT_DELAYED_WORK(&ldata[i].led_drvx_delayed_work, led_drvx_do_work);
			ldata[i].pwm_led = pwm_request(
						pdata->led_config[i].bank,
						ldata[i].ldev.name);
			ldata[i].ldev.brightness_set =
	...
	for (i = 0; i < pdata->num_leds; i++) {
		...
		ret = device_create_file(ldata[i].ldev.dev,
					&dev_attr_off_timer);
		if (ret < 0) {
			pr_err("%s: failed on create attr off timer [%d]\n",
				__func__, i);
