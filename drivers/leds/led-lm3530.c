...
static struct lm3530_data *g_als_data;
...
#define FULL_SCALE_CURRENT_MASK 0x1C
#define FULL_SCALE_CURRENT_COUNT 8
int ld_lm3530_adjust_brightness(int perc, int *brightness, enum led_brightness *value)
{
	int v = *value;
	unsigned int fsc_index = ((*brightness & FULL_SCALE_CURRENT_MASK) >> 2);
	int tar_fsc_index = 0;
	/* full scale current (fsc) table x10 (15.5mA = 155) */
	int full_scales[FULL_SCALE_CURRENT_COUNT] =
				{50, 85, 120, 155, 190, 225, 260, 295};
	int fsc_perc = 0;
	/* Compute the desired current (17.1mA = 171) (+5 (0.5) to round up) */
	int tar_current = (((full_scales[fsc_index] * (v*100)/255 * perc)
				/ 1000) + 5) / 10;

	if (perc < 0)
		return -1;

	/* Find target full scale current (fsc) */
	for (tar_fsc_index=fsc_index; tar_fsc_index<FULL_SCALE_CURRENT_COUNT;
							tar_fsc_index++) {
		if (tar_current < full_scales[tar_fsc_index])
			break;
	}

	if (tar_fsc_index == FULL_SCALE_CURRENT_COUNT) {
		pr_err("%s - warning: at max fsc, setting to max brightness\n"
			, __func__);
		tar_fsc_index -= 1;
		v = 255;
	} else {
		/* Compute % change with different fsc (i.e. 81.57% = 8157) */
		fsc_perc = (10000 * full_scales[fsc_index] /
						full_scales[tar_fsc_index]);
		/* Compute new fsc brightness percentage (v/255) */
		v = v * fsc_perc * perc / 1000000;
		/* Catch any rouding errors causing out of bound brightness % */
		if (v > 255)
			v = 255;
	}

	*brightness = *brightness & ~FULL_SCALE_CURRENT_MASK;
	/* Set the brightness register (full scale current) */
	*brightness = *brightness | (tar_fsc_index << 2);
	/* Set the fsc brightness percentage */
	*value = v;
				     ...
				     enum led_brightness value)
{
...
#ifdef CONFIG_PANEL_MAPPHONE
	/* Adjust the display brightness for 3.7" AUO displays */
	if (mapphone_panel_id == MOT_DISP_370_MIPI_480_854_CM &&
	    als_data->als_pdata->brightness_mod_perc) {
		/* increase brightness by percentage indicated in device tree */
		ld_lm3530_adjust_brightness(
			als_data->als_pdata->brightness_mod_perc, &brightness,
			&value);
	}
#endif
	...
	if (value == LED_OFF) {
		...
		/* Set to true for the next time we're enabled */
		als_data->pwm_delay = true;
	} else {
		if (als_data->pwm_delay)
			brightness &= 0xdf;
...
void ld_lm3530_pwm_start(void)
{
	if (g_als_data) {
		g_als_data->pwm_delay = false;
		if (g_als_data->led_on) {
			ld_lm3530_brightness_set(&(g_als_data->led_dev),
				g_als_data->last_requested_brightness);
		}
	}
}
...
static int convert_to_lux(struct lm3530_data *als_data, int zone_value)
{
	...
	if (als_data->als_pdata->scale_factor)
		result = ((current_conv * 100 * divisor) /
				(als_data->als_pdata->scale_factor));
	else
		result = (current_conv * 100) / divisor;
