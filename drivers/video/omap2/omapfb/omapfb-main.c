...
static int omapfb_probe(struct platform_device *pdev)
{
	...
	DBG("mgr->apply'ed\n");
#ifdef CONFIG_FB_OMAP2_MTD_LOGO
	logo_load_status = show_mtd_logo(fbdev->fbs[0]);
#endif
...
#ifdef CONFIG_FB_OMAP2_MTD_LOGO
	/* Update the display only if the logo was successfully read from the
	 * mtd partition. This prevents updating with a blank screen when there
	 * is no mtd logo partition. */
	if (logo_load_status == 0) {
		for (i = 0; i < fbdev->num_displays; i++) {
