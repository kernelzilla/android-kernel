...
static int msm_fb_open_dummy(struct fb_info *info, int user)
{
    return 0;
}
static int msm_fb_release_dummy(struct fb_info *info, int user)
{
    return 0;
}
static int msm_fb_check_var_dummy(struct fb_var_screeninfo *var, struct fb_info *info)
{
    return 0;
}
static int msm_fb_set_par_dummy(struct fb_info *info)
{
    return 0;
}
static int msm_fb_blank_dummy(int blank_mode, struct fb_info *info)
{
    return 0;
}
static int msm_fb_pan_display_dummy(struct fb_var_screeninfo *var,
			      struct fb_info *info)
{
    return 0;
}
static void msm_fb_fillrect_dummy(struct fb_info *info,
			    const struct fb_fillrect *rect)
{
    return 0;
}
static void msm_fb_copyarea_dummy(struct fb_info *info,
			    const struct fb_copyarea *area)
{
    return 0;
}

static void msm_fb_imageblit_dummy(struct fb_info *info, const struct fb_image *image)
{
    return 0;
}
static int msm_fb_ioctl_dummy(struct fb_info *info, unsigned int cmd,
			unsigned long arg)
{
    return 0;
}

static struct fb_ops msm_fb_ops_dummy = {
	.owner = THIS_MODULE,
	.fb_open = msm_fb_open_dummy,
	.fb_release = msm_fb_release_dummy,
	.fb_read = NULL,
	.fb_write = NULL,
	.fb_cursor = NULL,
	.fb_check_var = msm_fb_check_var_dummy,	/* vinfo check */
	.fb_set_par = msm_fb_set_par_dummy,	/* set the video mode according to info->var */
	.fb_setcolreg = NULL,	/* set color register */
	.fb_blank = msm_fb_blank_dummy,	/* blank display */
	.fb_pan_display = msm_fb_pan_display_dummy,	/* pan display */
	.fb_fillrect = msm_fb_fillrect_dummy,	/* Draws a rectangle */
	.fb_copyarea = msm_fb_copyarea_dummy,	/* Copy data from area to another */
	.fb_imageblit = msm_fb_imageblit_dummy,	/* Draws a image to the display */
	.fb_rotate = NULL,
	.fb_sync = NULL,	/* wait for blit idle, optional */
	.fb_ioctl = msm_fb_ioctl_dummy,	/* perform fb specific ioctl (optional) */
	.fb_mmap = NULL,
};
...
#ifdef CONFIG_MACH_SOCIAL
    /* hack, for dummy fb */
    if(panel_info->bl_max == 2)
        fbi->fbops = &msm_fb_ops_dummy;
    else
#endif /* CONFIG_MACH_SOCIAL */
