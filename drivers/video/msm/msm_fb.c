...
#define MOTFB_GET_NEXT_FB_CONTENT 0x7623
#define MOTFB_ENABLE_CABC 0x7630
#define MOTFB_ENABLE_VSYNC 0x7635
#define MOTFB_WRITE_REG 0x7640
#define MOTFB_READ_REG 0x7641
#define MOTFB_ENABLE_PAN 0x7645

static wait_queue_head_t disp_util_wq;
static int motfb_is_waiting_for_frame = 0;
static int en_pan_display = 1;

struct motfb_get_next_fb_content {
short bufNum;
unsigned short numFramesToGet;
unsigned numFramesGot;
unsigned short *contentBuf;
};

struct motfb_enable_cabc {
int enable;
};
...
static int msm_fb_register(struct msm_fb_data_type *mfd)
{
    ...
    printk("msmfb registered_early_suspend\n");
#endif
                  ...
                  struct fb_info *info)
{
    ...
    struct msm_fb_data_type *mfd = (struct msm_fb_data_type *)info->par;
#ifdef CONFIG_MACH_MOT
    static struct msm_fb_panel_data *pdata = NULL;
    static int pan_updt_supported = -1;

    if(!en_pan_display) return 0;
#endif
...
#ifdef CONFIG_MACH_MOT
    if(pan_updt_supported < 0)
    {
        pdata = (struct msm_fb_panel_data *)mfd->pdev->dev.platform_data;
        pan_updt_supported = pdata->pan_updt_supported(mfd->pdev);
    }
...
#ifdef CONFIG_MACH_MOT
    if(motfb_is_waiting_for_frame)
    {
        motfb_is_waiting_for_frame = 0;
        wake_up(&disp_util_wq);
    }

    pdata->enable_panel(mfd->pdev);
#ifdef CONFIG_FB_MSM_ESD
    msm_fb_sent_frame++;
#endif
#endif
...
#ifdef CONFIG_MACH_MOT
    case MOTFB_GET_NEXT_FB_CONTENT:
    {
        unsigned char *pfb;
        struct motfb_get_next_fb_content mfb_get;

        ret = copy_from_user(&mfb_get, argp, sizeof(mfb_get));
        if (ret)
            return ret;

        for(mfb_get.numFramesGot = 0; mfb_get.numFramesGot < mfb_get.numFramesToGet; mfb_get.numFramesGot++)
        {
            motfb_is_waiting_for_frame = 1;
            wait_event_interruptible(disp_util_wq, !motfb_is_waiting_for_frame);

            pfb = (unsigned char *)mfd->fbi->screen_base;
            pfb += (info->var.xoffset +
             info->var.yoffset * info->var.xres_virtual) * 2; // 16-bit = 2 bytes

            if(copy_to_user(&mfb_get.contentBuf[mfb_get.numFramesGot*320*480], pfb, 320*480*2)) return -EFAULT;
        }

        if(copy_to_user(argp, &mfb_get, sizeof(mfb_get))) return -EFAULT;
        break;
    }

    case MOTFB_ENABLE_CABC:
    {
        struct motfb_enable_cabc mfb_cabc;
        break;
    }

    case MOTFB_ENABLE_VSYNC:
    {
        extern void enable_vsync(struct platform_device *, struct motfb_enable_vsync *);
        struct motfb_enable_vsync mfb_vsync;

        ret = copy_from_user(&mfb_vsync, argp, sizeof(mfb_vsync));
        if (ret)
            return ret;

        enable_vsync(mfd->pdev, &mfb_vsync);
        break;
    }

    case MOTFB_WRITE_REG:
    {
        extern void wr_reg(struct platform_device *pdev, struct motfb_write_reg *wreg);

        struct motfb_write_reg mfb_wreg;

        ret = copy_from_user(&mfb_wreg, argp, sizeof(mfb_wreg));
        if (ret)
            return ret;

        wr_reg(mfd->pdev, &mfb_wreg);
        break;
    }

    case MOTFB_READ_REG:
    {
        extern void rd_reg(struct platform_device *pdev, struct motfb_read_reg *rreg);

        struct motfb_read_reg mfb_rreg;

        ret = copy_from_user(&mfb_rreg, argp, sizeof(mfb_rreg));
        if (ret)
            return ret;

        rd_reg(mfd->pdev, &mfb_rreg);

        if(copy_to_user(argp, &mfb_rreg, sizeof(mfb_rreg))) return -EFAULT;
        break;
    }

    case MOTFB_ENABLE_PAN:
        en_pan_display = (int)argp; 
        break;
...
#ifdef CONFIG_MACH_MOT
    init_waitqueue_head(&disp_util_wq);
#else
