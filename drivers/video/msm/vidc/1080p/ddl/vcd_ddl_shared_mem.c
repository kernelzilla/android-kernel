...
#define DDL_MEM_WRITE_32(p_base, offset, val) ddl_mem_write_32(\
	(u32 *) ((u8 *) (p_base)->p_align_virtual_addr + (offset)), (val))
#define DDL_MEM_READ_32(p_base, offset) ddl_mem_read_32(\
	(u32 *) ((u8 *) (p_base)->p_align_virtual_addr + (offset)))

#define DDL_SHARED_MEM_11BIT_RIGHT_SHIFT  11

static void ddl_mem_write_32(u32 *p_addr, u32 data)
{
	*p_addr = data;
}

static u32 ddl_mem_read_32(u32 *p_addr)
{
	return *p_addr;
}

void vidc_sm_get_extended_decode_status(struct ddl_buf_addr_type *p_shared_mem,
	u32 *pn_decode_status)
{
	*pn_decode_status = DDL_MEM_READ_32(p_shared_mem,
...
void vidc_sm_get_frame_tags(struct ddl_buf_addr_type *p_shared_mem,
	u32  *pn_frame_tag_top, u32 *pn_frame_tag_bottom)
{
	*pn_frame_tag_top = DDL_MEM_READ_32(p_shared_mem,
				VIDC_SM_GET_FRAME_TAG_TOP_ADDR);
	*pn_frame_tag_bottom = DDL_MEM_READ_32(p_shared_mem,
					VIDC_SM_GET_FRAME_TAG_BOTTOM_ADDR);
}

void vidc_sm_get_picture_times(struct ddl_buf_addr_type *p_shared_mem,
	u32 *pn_time_top, u32 *pn_time_bottom)
{
	*pn_time_top = DDL_MEM_READ_32(p_shared_mem, VIDC_SM_PIC_TIME_TOP_ADDR);
	*pn_time_bottom = DDL_MEM_READ_32(p_shared_mem,
...
void vidc_sm_get_crop_info(struct ddl_buf_addr_type *p_shared_mem,
	u32 *pn_left, u32 *pn_right, u32 *pn_top, u32 *pn_bottom)
{
	u32 n_info1, n_info2;

	n_info1 = DDL_MEM_READ_32(p_shared_mem, VIDC_SM_CROP_INFO1_ADDR);

	*pn_left = VIDC_GETFIELD(n_info1, VIDC_SM_CROP_INFO1_LEFT_OFFSET_BMSK,
					VIDC_SM_CROP_INFO1_LEFT_OFFSET_SHFT);
	*pn_right = VIDC_GETFIELD(n_info1, VIDC_SM_CROP_INFO1_RIGHT_OFFSET_BMSK,
					VIDC_SM_CROP_INFO1_RIGHT_OFFSET_SHFT);
	n_info2 = DDL_MEM_READ_32(p_shared_mem, VIDC_SM_CROP_INFO2_ADDR);
	*pn_top = VIDC_GETFIELD(n_info2, VIDC_SM_CROP_INFO2_TOP_OFFSET_BMSK,
					VIDC_SM_CROP_INFO2_TOP_OFFSET_SHFT);
	*pn_bottom = VIDC_GETFIELD(n_info2,
					VIDC_SM_CROP_INFO2_BOTTOM_OFFSET_BMSK,
					VIDC_SM_CROP_INFO2_BOTTOM_OFFSET_SHFT);
}

void vidc_sm_get_displayed_picture_frame_type(struct ddl_buf_addr_type
	*p_shared_mem, u32  *p_n_disp_picture_frame_type)
{
	u32 n_disp_pict_frame_type;

	n_disp_pict_frame_type = DDL_MEM_READ_32(p_shared_mem,
					VIDC_SM_DISP_PIC_FRAME_TYPE_ADDR);
	*p_n_disp_picture_frame_type = VIDC_GETFIELD(n_disp_pict_frame_type,
			VIDC_SM_DISP_PIC_FRAME_TYPE_BMSK,
			VIDC_SM_DISP_PIC_FRAME_TYPE_SHFT);
}
void vidc_sm_get_available_luma_dpb_address(struct ddl_buf_addr_type
	*p_shared_mem, u32 *pn_free_luma_dpb_address)
{
	*pn_free_luma_dpb_address = DDL_MEM_READ_32(p_shared_mem,
...
void vidc_sm_set_encoder_param_change(struct ddl_buf_addr_type *p_shared_mem,
	u32 b_bit_rate_chg, u32 b_frame_rate_chg, u32 b_i_period_chg)
{
	u32 n_enc_param_chg;

	n_enc_param_chg = VIDC_SETFIELD((b_bit_rate_chg) ? 1 : 0,
				VIDC_SM_ENC_PARAM_CHANGE_RC_BIT_RATE_SHFT,
				VIDC_SM_ENC_PARAM_CHANGE_RC_BIT_RATE_BMSK) |
				VIDC_SETFIELD((b_frame_rate_chg) ? 1 : 0,
				VIDC_SM_ENC_PARAM_CHANGE_RC_FRAME_RATE_SHFT,
				VIDC_SM_ENC_PARAM_CHANGE_RC_FRAME_RATE_BMSK) |
				VIDC_SETFIELD((b_i_period_chg) ? 1 : 0,
				VIDC_SM_ENC_PARAM_CHANGE_I_PERIOD_SHFT,
				VIDC_SM_ENC_PARAM_CHANGE_I_PERIOD_BMSK);
	DDL_MEM_WRITE_32(p_shared_mem, VIDC_SM_ENC_PARAM_CHANGE_ADDR,
		n_enc_param_chg);
}

void vidc_sm_set_encoder_vop_time(struct ddl_buf_addr_type *p_shared_mem,
	u32 b_vop_time_enable, u32 n_time_resolution, u32 n_frame_delta)
{
	u32 n_vop_time;

	n_vop_time = VIDC_SETFIELD((b_vop_time_enable) ? 1 : 0,
			VIDC_SM_ENC_VOP_TIMING_ENABLE_SHFT ,
			VIDC_SM_ENC_VOP_TIMING_ENABLE_BMSK) |
			VIDC_SETFIELD(n_time_resolution ,
			VIDC_SM_ENC_VOP_TIMING_TIME_RESOLUTION_SHFT,
			VIDC_SM_ENC_VOP_TIMING_TIME_RESOLUTION_BMSK) |
			VIDC_SETFIELD(n_frame_delta,
			VIDC_SM_ENC_VOP_TIMING_FRAME_DELTA_SHFT,
			VIDC_SM_ENC_VOP_TIMING_FRAME_DELTA_BMSK);
	DDL_MEM_WRITE_32(p_shared_mem, VIDC_SM_ENC_VOP_TIMING_ADDR, n_vop_time);
}
...
void vidc_sm_get_h264_encoder_reference_list0(struct ddl_buf_addr_type
	*p_shared_mem, enum VIDC_SM_ref_picture_type *pe_luma_picture0,
	u32 *pn_luma_picture_index0, enum VIDC_SM_ref_picture_type
		*pe_luma_picture1, u32 *pn_luma_picture_index1,
	enum VIDC_SM_ref_picture_type *pe_chroma_picture0,
	u32 *pn_chroma_picture_index0,
	enum VIDC_SM_ref_picture_type *pe_chroma_picture1,
	u32 *pn_chroma_picture_index1)
{
	u32 n_ref_list;

	n_ref_list = DDL_MEM_READ_32(p_shared_mem, VIDC_SM_H264_REF_L0_ADDR);

	*pe_luma_picture0 = (enum VIDC_SM_ref_picture_type)
				VIDC_GETFIELD(n_ref_list,
...
void vidc_sm_get_h264_encoder_reference_list1(struct ddl_buf_addr_type
	*p_shared_mem, enum VIDC_SM_ref_picture_type *pe_luma_picture,
	u32 *pn_luma_picture_index,
	enum VIDC_SM_ref_picture_type *pe_chroma_picture,
	u32 *pn_chroma_picture_index)
{
	u32 n_ref_list;

	n_ref_list = DDL_MEM_READ_32(p_shared_mem, VIDC_SM_H264_REF_L1_ADDR);

	*pe_luma_picture = (enum VIDC_SM_ref_picture_type)
				VIDC_GETFIELD(n_ref_list,
	...
	*pe_chroma_picture = (enum VIDC_SM_ref_picture_type)
				VIDC_GETFIELD(n_ref_list,
				VIDC_SM_H264_REF_L1_CHRO_BTM_FLG_0_BMSK,
				VIDC_SM_H264_REF_L1_CHRO_BTM_FLG_0_SHFT);
	*pn_chroma_picture_index = VIDC_GETFIELD(n_ref_list,
				VIDC_SM_H264_REF_L1_CHRO_REF_0_BMSK,
				VIDC_SM_H264_REF_L1_CHRO_REF_0_SHFT);
}

void vidc_sm_set_allocated_dpb_size(struct ddl_buf_addr_type *p_shared_mem,
		u32 n_y_size, u32 n_c_size)
{
	DDL_MEM_WRITE_32(p_shared_mem, VIDC_SM_ALLOCATED_LUMA_DPB_SIZE_ADDR,
		n_y_size);
	DDL_MEM_WRITE_32(p_shared_mem, VIDC_SM_ALLOCATED_CHROMA_DPB_SIZE_ADDR,
		n_c_size);
}
...
void vidc_sm_get_min_yc_dpb_sizes(struct ddl_buf_addr_type *p_shared_mem,
	u32 *pn_min_luma_dpb_size, u32 *pn_min_chroma_dpb_size)
{
	*pn_min_luma_dpb_size = DDL_MEM_READ_32(p_shared_mem,
		VIDC_SM_MIN_LUMA_DPB_SIZE_ADDR);
	*pn_min_chroma_dpb_size = DDL_MEM_READ_32(p_shared_mem,
...
void vidc_sm_get_metadata_status(struct ddl_buf_addr_type
		*p_shared_mem, u32 *pb_metadata_present)
{
	u32 n_status;

	n_status = DDL_MEM_READ_32(p_shared_mem, VIDC_SM_METADATA_STATUS_ADDR);
	*pb_metadata_present = (u32) VIDC_GETFIELD(n_status,
				VIDC_SM_METADATA_STATUS_STATUS_BMSK,
				VIDC_SM_METADATA_STATUS_STATUS_SHFT);
}

void vidc_sm_get_metadata_display_index(struct ddl_buf_addr_type *p_shared_mem,
	u32 *pn_dixplay_index)
{
	*pn_dixplay_index = DDL_MEM_READ_32(p_shared_mem,
					VIDC_SM_METADATA_DISPLAY_INDEX_ADDR);
}

void vidc_sm_set_metadata_start_address(struct ddl_buf_addr_type *p_shared_mem,
	u32 n_address)
{
	u32 n_address_shift = n_address;

	DDL_MEM_WRITE_32(p_shared_mem, VIDC_SM_EXT_METADATA_START_ADDR_ADDR,
		n_address_shift);
}
...
void vidc_sm_set_extradata_addr(struct ddl_buf_addr_type *p_shared_mem,
	u32 n_extradata_addr)
{
	u32 n_address_shift = n_extradata_addr;

	DDL_MEM_WRITE_32(p_shared_mem, VIDC_SM_EXTRADATA_ADDR_ADDR,
		n_address_shift);
}

void vidc_sm_set_pand_b_frame_qp(struct ddl_buf_addr_type *p_shared_mem,
	u32 n_b_frame_qp, u32 n_p_frame_qp)
{
	u32 nP_B_frame_qp;

	nP_B_frame_qp = VIDC_SETFIELD(n_b_frame_qp,
				VIDC_SM_P_B_FRAME_QP_B_FRAME_QP_SHFT,
				VIDC_SM_P_B_FRAME_QP_B_FRAME_QP_BMASK);
	nP_B_frame_qp |= VIDC_SETFIELD(n_p_frame_qp,
				VIDC_SM_P_B_FRAME_QP_P_FRAME_QP_SHFT,
				VIDC_SM_P_B_FRAME_QP_P_FRAME_QP_BMASK);
	DDL_MEM_WRITE_32(p_shared_mem , VIDC_SM_P_B_FRAME_QP_ADDR,
		nP_B_frame_qp);
}


void vidc_sm_get_profile_info(struct ddl_buf_addr_type *p_shared_mem,
	u32 *pn_disp_profile_info, u32 *pn_disp_level_info)
{
	u32 n_disp_pic_profile;

	n_disp_pic_profile = DDL_MEM_READ_32(p_shared_mem,
				VIDC_SM_DISP_PIC_PROFILE_ADDR);
	*pn_disp_profile_info = VIDC_GETFIELD(n_disp_pic_profile,
			VIDC_SM_DISP_PIC_PROFILE_DISP_PIC_PROFILE_BMASK,
			VIDC_SM_DISP_PIC_PROFILE_DISP_PIC_PROFILE_SHFT);
	*pn_disp_level_info = VIDC_GETFIELD(n_disp_pic_profile,
			VIDC_SM_DISP_PIC_PROFILE_DISP_PIC_LEVEL_BMASK,
			VIDC_SM_DISP_PIC_PROFILE_DISP_PIC_LEVEL_SHFT);
}

void vidc_sm_set_encoder_new_bit_rate(struct ddl_buf_addr_type *p_shared_mem,
	u32 n_new_bit_rate)
{
	DDL_MEM_WRITE_32(p_shared_mem, VIDC_SM_NEW_RC_BIT_RATE_ADDR,
		n_new_bit_rate);
}
...
void vidc_sm_set_encoder_new_i_period(struct ddl_buf_addr_type *p_shared_mem,
	u32 n_new_i_period)
{
	DDL_MEM_WRITE_32(p_shared_mem, VIDC_SM_NEW_I_PERIOD_ADDR,
		n_new_i_period);
}
