...
static u32 ddl_set_dec_buffers(struct ddl_decoder_data_type *p_decoder,
	...
	struct vcd_property_hdr_type *p_property_hdr, void *p_property_value)
{
	...
	if (p_ddl->b_decoding)
		vcd_status = ddl_set_dec_property(p_ddl, p_property_hdr,
				p_property_value);
	else
		vcd_status = ddl_set_enc_property(p_ddl, p_property_hdr,
				p_property_value);
	if (vcd_status)
	...
	struct vcd_property_hdr_type *p_property_hdr, void *p_property_value)
{
	...
	if (p_property_hdr->prop_id == DDL_I_CAPABILITY) {
		if (sizeof(struct ddl_property_capability_type) ==
			...
			p_ddl_capability->b_exclusive = VCD_COMMAND_EXCLUSIVE;
	...
	if (!p_ddl)
		return VCD_ERR_BAD_HANDLE;
	if (p_ddl->b_decoding)
		vcd_status = ddl_get_dec_property(p_ddl, p_property_hdr,
				p_property_value);
	else
		vcd_status = ddl_get_enc_property(p_ddl, p_property_hdr,
				p_property_value);
	if (vcd_status)
	...
	struct vcd_sequence_hdr_type  *p_header)
{
	...
	if ((!p_header) && (!p_decoder->client_frame_size.n_height ||
...
u32 ddl_encoder_ready_to_start(struct ddl_client_context_type *p_ddl)
{
	...
	if (!p_encoder->codec_type.e_codec || !p_encoder->frame_size.n_height ||
...
static u32 ddl_set_dec_property(struct ddl_client_context_type *p_ddl,
	struct vcd_property_hdr_type *p_property_hdr, void *p_property_value)
{
	...
	case DDL_I_DPB_RELEASE:
		if ((sizeof(struct ddl_frame_data_type_tag) ==
			...
			vcd_status = ddl_decoder_dpb_transact(p_decoder,
				(struct ddl_frame_data_type_tag *)
				p_property_value, DDL_DPB_OP_MARK_FREE);
	break;
	case DDL_I_DPB:
	{
		struct ddl_property_dec_pic_buffers_type *p_dpb =
		(struct ddl_property_dec_pic_buffers_type *) p_property_value;
		...
		if ((sizeof(struct ddl_property_dec_pic_buffers_type) ==
			...
			vcd_status = ddl_set_dec_buffers(p_decoder, p_dpb);
	}
	break;
	...
	case DDL_I_INPUT_BUF_REQ:
	{
		struct vcd_buffer_requirement_type *p_buffer_req =
			(struct vcd_buffer_requirement_type *)p_property_value;
		...
		if (sizeof(struct vcd_buffer_requirement_type) ==
			...
			&p_decoder->min_input_buf_req, p_buffer_req))) {
	...
	case DDL_I_OUTPUT_BUF_REQ:
	{
		...
		if (sizeof(struct vcd_buffer_requirement_type) ==
			...
			DDLCLIENT_STATE_IS(p_ddl, DDL_CLIENT_OPEN)) &&
			(ddl_valid_buffer_requirement(
			&p_decoder->min_output_buf_req, p_buffer_req))) {
	...
	case VCD_I_CODEC:
	{
		...
		if (sizeof(struct vcd_property_codec_type) ==
			p_property_hdr->n_size &&
			DDLCLIENT_STATE_IS(p_ddl, DDL_CLIENT_OPEN) &&
			ddl_codec_type_transact(p_ddl, FALSE,
			p_codec->e_codec)) {
			if (p_decoder->codec_type.e_codec != p_codec->e_codec) {
	...
	case VCD_I_FRAME_SIZE:
	{
		...
		(struct vcd_property_frame_size_type *) p_property_value;
		if ((sizeof(struct vcd_property_frame_size_type) ==
			p_property_hdr->n_size) &&
			(DDLCLIENT_STATE_IS(p_ddl, DDL_CLIENT_OPEN)) &&
			(DDL_ALLOW_DEC_FRAMESIZE(p_frame_size->n_width,
			p_frame_size->n_height))) {
			if (p_decoder->client_frame_size.n_height !=
				p_frame_size->n_height ||
				p_decoder->client_frame_size.n_width !=
				p_frame_size->n_width) {
				ddl_calculate_stride(p_frame_size,
					!p_decoder->n_progressive_only);
				p_decoder->client_frame_size = *p_frame_size;
				ddl_set_default_decoder_buffer_req(p_decoder,
					TRUE);
			}
			vcd_status = VCD_S_SUCCESS;
		}
	...
	case VCD_I_BUFFER_FORMAT:
	{
		...
		if (sizeof(struct vcd_property_buffer_format_type) ==
			p_property_hdr->n_size &&
			DDLCLIENT_STATE_IS(p_ddl, DDL_CLIENT_OPEN) &&
			p_tile->e_buffer_format == VCD_BUFFER_FORMAT_TILE_4x2) {
...
static u32 ddl_check_valid_enc_level(struct vcd_property_codec_type *p_codec,
	struct vcd_property_profile_type *p_profile,
	struct vcd_property_level_type *p_level)
{
	u32 b_status = FALSE;

	if (p_codec && p_profile && p_level) {
		switch (p_codec->e_codec) {
		case VCD_CODEC_MPEG4:
			b_status = (p_profile->e_profile ==
				VCD_PROFILE_MPEG4_SP) &&
				(p_level->e_level >= VCD_LEVEL_MPEG4_0) &&
				(p_level->e_level <= VCD_LEVEL_MPEG4_7) &&
				(VCD_LEVEL_MPEG4_3b != p_level->e_level);
			b_status = b_status ||
				((p_profile->e_profile ==
				VCD_PROFILE_MPEG4_ASP) &&
				(p_level->e_level >= VCD_LEVEL_MPEG4_0) &&
				(p_level->e_level <= VCD_LEVEL_MPEG4_5));
		break;
		case VCD_CODEC_H264:
		b_status = (p_level->e_level >= VCD_LEVEL_H264_1) &&
				(p_level->e_level <= VCD_LEVEL_H264_4);
		break;
		case VCD_CODEC_H263:
		b_status = (p_level->e_level >= VCD_LEVEL_H263_10) &&
	...
	void *p_property_value)
{
	...
	case VCD_I_TARGET_BITRATE:
	{
		...
		(struct vcd_property_target_bitrate_type *)p_property_value;
	...
	case VCD_I_FRAME_RATE:
	{
		...
		(struct vcd_property_frame_rate_type *) p_property_value;
	...
	case VCD_I_FRAME_SIZE:
	{
		...
		if ((sizeof(struct vcd_property_frame_size_type) ==
			p_property_hdr->n_size) &&
			(DDL_ALLOW_ENC_FRAMESIZE(p_frame_size->n_width,
			p_frame_size->n_height))) {
			ddl_calculate_stride(p_frame_size, FALSE);
			p_encoder->frame_size = *p_frame_size;
	...
	case VCD_I_CODEC:
	{
		...
		if ((sizeof(struct vcd_property_codec_type) ==
		p_property_hdr->n_size) &&
		(ddl_codec_type_transact(p_ddl, FALSE, p_codec->e_codec))) {
			if (p_codec->e_codec != p_encoder->codec_type.e_codec) {
	...
	{
		struct vcd_property_i_period_type *p_i_period =
			(struct vcd_property_i_period_type *)p_property_value;
		if (sizeof(struct vcd_property_i_period_type) ==
			p_property_hdr->n_size &&
			p_i_period->n_b_frames <= DDL_MAX_NUM_OF_B_FRAME) {
			p_encoder->i_period = *p_i_period;
			p_encoder->client_input_buf_req.n_min_count =
				p_i_period->n_b_frames + 1;
			p_encoder->client_input_buf_req.n_actual_count =
				DDL_MAX(p_encoder->client_input_buf_req.\
				n_actual_count, p_encoder->\
				client_input_buf_req.n_min_count);
			p_encoder->client_output_buf_req.n_min_count =
				p_i_period->n_b_frames + 2;
			p_encoder->client_output_buf_req.n_actual_count =
				DDL_MAX(p_encoder->client_output_buf_req.\
				n_actual_count, p_encoder->\
				client_output_buf_req.n_min_count);
			p_ddl->n_extra_output_buf_count =
				p_i_period->n_b_frames - 1;
			vcd_status = VCD_S_SUCCESS;
		}
	}
	...
	{
		struct vcd_property_profile_type *p_profile =
			(struct vcd_property_profile_type *)p_property_value;
		...
		if ((sizeof(struct vcd_property_profile_type) ==
			...
			p_profile->e_profile == VCD_PROFILE_MPEG4_ASP)) ||
			((p_encoder->codec_type.e_codec == VCD_CODEC_H264) &&
			(p_profile->e_profile >= VCD_PROFILE_H264_BASELINE) &&
			(p_profile->e_profile <= VCD_PROFILE_H264_HIGH)) ||
			((p_encoder->codec_type.e_codec == VCD_CODEC_H263) &&
			(p_profile->e_profile == VCD_PROFILE_H263_BASELINE)))) {
			p_encoder->profile = *p_profile;
			vcd_status = VCD_S_SUCCESS;
			if (p_profile->e_profile == VCD_PROFILE_H264_BASELINE)
	...
	{
		struct vcd_property_level_type *p_level =
			(struct vcd_property_level_type *) p_property_value;

		if ((sizeof(struct vcd_property_level_type) ==
			p_property_hdr->n_size) && (ddl_check_valid_enc_level
			(&p_encoder->codec_type,
			&p_encoder->profile, p_level))) {
	...
	{
		struct vcd_property_multi_slice_type *p_multi_slice =
			(struct vcd_property_multi_slice_type *)
				p_property_value;

		switch (p_multi_slice->e_m_slice_sel) {
		case VCD_MSLICE_OFF:
			vcd_status = VCD_S_SUCCESS;
		break;
		case VCD_MSLICE_BY_GOB:
			if (p_encoder->codec_type.e_codec == VCD_CODEC_H263)
				vcd_status = VCD_S_SUCCESS;
		break;
		case VCD_MSLICE_BY_MB_COUNT:
			if (p_multi_slice->n_m_slice_size >= 1 &&
				(p_multi_slice->n_m_slice_size <=
				DDL_NO_OF_MB(p_encoder->frame_size.n_width,
				p_encoder->frame_size.n_height)))
				vcd_status = VCD_S_SUCCESS;
		break;
		case VCD_MSLICE_BY_BYTE_COUNT:
			if (p_multi_slice->n_m_slice_size >=
		...
		if (sizeof(struct vcd_property_multi_slice_type) ==
			p_property_hdr->n_size && !vcd_status)
			p_encoder->multi_slice = *p_multi_slice;
	}
	break;
	case VCD_I_RATE_CONTROL:
	{
		struct vcd_property_rate_control_type *p_rate_control_type =
			(struct vcd_property_rate_control_type *)
			p_property_value;
		if (sizeof(struct vcd_property_rate_control_type) ==
			p_property_hdr->n_size &&
			p_rate_control_type->e_rate_control >=
			VCD_RATE_CONTROL_OFF &&
			p_rate_control_type->e_rate_control <=
			VCD_RATE_CONTROL_CBR_CFR) {
			p_encoder->rc_type = *p_rate_control_type;
	...
	{
		struct vcd_property_vop_timing_type *p_vop_time =
		...
		if ((sizeof(struct vcd_property_vop_timing_type) ==
			p_property_hdr->n_size) &&
			(p_encoder->frame_rate.n_fps_numerator <=
			p_vop_time->n_vop_time_resolution) &&
			(p_encoder->codec_type.e_codec == VCD_CODEC_MPEG4)) {
			p_encoder->vop_timing = *p_vop_time;
			vcd_status = VCD_S_SUCCESS;
		}
	}
	break;
	case VCD_I_HEADER_EXTENSION:
		if (sizeof(u32) == p_property_hdr->n_size &&
			p_encoder->codec_type.e_codec == VCD_CODEC_MPEG4) {
			p_encoder->n_hdr_ext_control = *(u32 *)p_property_value;
			vcd_status = VCD_S_SUCCESS;
		}
	break;
	case VCD_I_ENTROPY_CTRL:
	{
		struct vcd_property_entropy_control_type *p_entropy_control =
	...
	{
		struct vcd_property_db_config_type *p_db_config =
			(struct vcd_property_db_config_type *) p_property_value;
		if (sizeof(struct vcd_property_db_config_type) ==
			p_property_hdr->n_size  &&
			p_encoder->codec_type.e_codec == VCD_CODEC_H264 &&
			p_db_config->e_db_config >=
			VCD_DB_ALL_BLOCKING_BOUNDARY &&
			p_db_config->e_db_config <=
			VCD_DB_SKIP_SLICE_BOUNDARY) {
			p_encoder->db_control = *p_db_config;
			vcd_status = VCD_S_SUCCESS;
		}
	}
	break;
	case VCD_I_QP_RANGE:
	{
		struct vcd_property_qp_range_type *p_qp =
			(struct vcd_property_qp_range_type *)p_property_value;

		if ((sizeof(struct vcd_property_qp_range_type) ==
			p_property_hdr->n_size) && (p_qp->n_min_qp <=
			p_qp->n_max_qp) && ((p_encoder->codec_type.e_codec ==
			VCD_CODEC_H264 && p_qp->n_max_qp <= DDL_MAX_H264_QP) ||
	...
	case VCD_I_SESSION_QP:
	{
		struct vcd_property_session_qp_type *p_qp =
			(struct vcd_property_session_qp_type *)p_property_value;
	...
	case VCD_I_RC_LEVEL_CONFIG:
	{
		struct vcd_property_rc_level_type *p_rc_level =
			(struct vcd_property_rc_level_type *) p_property_value;
		if (sizeof(struct vcd_property_rc_level_type) ==
			p_property_hdr->n_size &&
			(p_encoder->rc_type.e_rate_control >=
	...
	case VCD_I_FRAME_LEVEL_RC:
	{
		struct vcd_property_frame_level_rc_params_type
			*p_frame_level_rc =
			(struct vcd_property_frame_level_rc_params_type *)
			p_property_value;
		if ((sizeof(struct vcd_property_frame_level_rc_params_type) ==
			p_property_hdr->n_size) &&
			(p_frame_level_rc->n_reaction_coeff) &&
			(p_encoder->rc_level.b_frame_level_rc)) {
			p_encoder->frame_level_rc = *p_frame_level_rc;
			vcd_status = VCD_S_SUCCESS;
		}
	}
	break;
	...
	case VCD_I_INTRA_REFRESH:
	{
		struct vcd_property_intra_refresh_mb_number_type
			*p_intra_refresh_mb_num =
			(struct vcd_property_intra_refresh_mb_number_type *)
			p_property_value;
		u32 n_frame_mb_num = DDL_NO_OF_MB(p_encoder->frame_size.n_width,
					p_encoder->frame_size.n_height);

		if (sizeof(struct vcd_property_intra_refresh_mb_number_type) ==
			p_property_hdr->n_size &&
			p_intra_refresh_mb_num->n_cir_mb_number <=
			n_frame_mb_num) {
			p_encoder->intra_refresh = *p_intra_refresh_mb_num;
			vcd_status = VCD_S_SUCCESS;
		}
		break;
	}
	case VCD_I_BUFFER_FORMAT:
	{
		struct vcd_property_buffer_format_type *p_buffer_format =
			(struct vcd_property_buffer_format_type *)
			p_property_value;

		if (sizeof(struct vcd_property_buffer_format_type) ==
			p_property_hdr->n_size &&
			((p_buffer_format->e_buffer_format ==
			VCD_BUFFER_FORMAT_NV12_16M2KA) ||
			(VCD_BUFFER_FORMAT_TILE_4x2 ==
			p_buffer_format->e_buffer_format))) {
			p_encoder->buf_format = *p_buffer_format;
			vcd_status = VCD_S_SUCCESS;
		}
	}
	break;
	case DDL_I_INPUT_BUF_REQ:
	{
		struct vcd_buffer_requirement_type *p_buffer_req =
			(struct vcd_buffer_requirement_type *)p_property_value;
		if (sizeof(struct vcd_buffer_requirement_type) ==
			p_property_hdr->n_size && (ddl_valid_buffer_requirement(
	...
	case DDL_I_OUTPUT_BUF_REQ:
	{
		...
		if (sizeof(struct vcd_buffer_requirement_type) ==
			...
			p_encoder->client_output_buf_req = *p_buffer_req;
			p_encoder->client_output_buf_req.n_size =
				DDL_ALIGN(p_buffer_req->n_size,
				DDL_KILO_BYTE(4));
			vcd_status = VCD_S_SUCCESS;
		}
	}
...
static u32 ddl_get_dec_property(struct ddl_client_context_type *p_ddl,
	struct vcd_property_hdr_type *p_property_hdr, void *p_property_value)
{
	struct ddl_decoder_data_type *p_decoder = &p_ddl->codec_data.decoder;
	u32 vcd_status = VCD_ERR_ILLEGAL_PARM;

	DDL_MSG_HIGH("p_property_hdr->prop_id: %x \n", p_property_hdr->prop_id);
	switch (p_property_hdr->prop_id) {
	case VCD_I_FRAME_SIZE:
		if (sizeof(struct vcd_property_frame_size_type) ==
			p_property_hdr->n_size) {
	...
	case DDL_I_FRAME_PROC_UNITS:
		if (sizeof(u32) == p_property_hdr->n_size &&
			...
			*(u32 *) p_property_value = DDL_NO_OF_MB(
				p_decoder->client_frame_size.n_width,
				p_decoder->client_frame_size.n_height);
			vcd_status = VCD_S_SUCCESS;
		}
	...
	case DDL_I_DPB_RETRIEVE:
		if (sizeof(struct ddl_frame_data_type_tag) ==
			...
			vcd_status = ddl_decoder_dpb_transact(p_decoder,
				(struct ddl_frame_data_type_tag *)
				p_property_value, DDL_DPB_OP_RETRIEVE);
		}
	break;
	case VCD_I_METADATA_ENABLE:
	case VCD_I_METADATA_HEADER:
...
static u32 ddl_get_enc_property(struct ddl_client_context_type *p_ddl,
	struct vcd_property_hdr_type *p_property_hdr, void *p_property_value)
{
	struct ddl_encoder_data_type *p_encoder = &p_ddl->codec_data.encoder;
	u32 vcd_status = VCD_ERR_ILLEGAL_PARM;
	...
	case VCD_I_FRAME_SIZE:
		if (sizeof(struct vcd_property_frame_size_type) ==
			p_property_hdr->n_size) {
			*(struct vcd_property_frame_size_type *)
				p_property_value = p_encoder->frame_size;
			vcd_status = VCD_S_SUCCESS;
		}
	break;
	case VCD_I_FRAME_RATE:
		if (sizeof(struct vcd_property_frame_rate_type) ==
			...
			*(struct vcd_property_frame_rate_type *)
				p_property_value = p_encoder->frame_rate;
			vcd_status = VCD_S_SUCCESS;
		}
	break;
	case VCD_I_TARGET_BITRATE:
		if (sizeof(struct vcd_property_target_bitrate_type) ==
			...
			*(struct vcd_property_target_bitrate_type *)
				p_property_value = p_encoder->target_bit_rate;
			vcd_status = VCD_S_SUCCESS;
		}
	break;
	case VCD_I_RATE_CONTROL:
	...
	case VCD_I_SEQ_HEADER:
	{
		...
		if (!p_encoder->n_seq_header_length) {
			p_seq_hdr->n_sequence_header_len =
				p_encoder->n_seq_header_length;
			vcd_status = VCD_ERR_NO_SEQ_HDR;
		} else if (sizeof(struct vcd_sequence_hdr_type) ==
			p_property_hdr->n_size &&
			p_encoder->n_seq_header_length <=
			p_seq_hdr->n_sequence_header_len) {
			memcpy(p_seq_hdr->p_sequence_header,
				p_encoder->seq_header.p_align_virtual_addr,
				p_encoder->n_seq_header_length);
			p_seq_hdr->n_sequence_header_len =
				p_encoder->n_seq_header_length;
			vcd_status = VCD_S_SUCCESS;
	...
	case VCD_I_FRAME_LEVEL_RC:
		if (sizeof(struct vcd_property_frame_level_rc_params_type) ==
			p_property_hdr->n_size) {
			*(struct vcd_property_frame_level_rc_params_type *)
			p_property_value = p_encoder->frame_level_rc;
			vcd_status = VCD_S_SUCCESS;
		}
	break;
	case VCD_I_ADAPTIVE_RC:
	...
	case VCD_I_INTRA_REFRESH:
		if (sizeof(struct vcd_property_intra_refresh_mb_number_type) ==
			p_property_hdr->n_size) {
			*(struct vcd_property_intra_refresh_mb_number_type *)
				p_property_value = p_encoder->intra_refresh;
			vcd_status = VCD_S_SUCCESS;
		}
	break;
	case DDL_I_INPUT_BUF_REQ:
		if (sizeof(struct vcd_buffer_requirement_type) ==
			p_property_hdr->n_size) {
			if (p_encoder->input_buf_req.n_size) {
				*(struct vcd_buffer_requirement_type *)
					p_property_value =
...
static u32 ddl_set_enc_dynamic_property(struct ddl_encoder_data_type
	*p_encoder, struct vcd_property_hdr_type *p_property_hdr,
	void *p_property_value)
{
	u32  vcd_status = VCD_ERR_ILLEGAL_PARM;
...
void ddl_set_default_dec_property(struct ddl_client_context_type *p_ddl)
{
	...
	p_decoder->buf_format.e_buffer_format = VCD_BUFFER_FORMAT_TILE_4x2;
	p_decoder->client_frame_size.n_height = VCD_DDL_TEST_DEFAULT_HEIGHT;
	p_decoder->client_frame_size.n_width  = VCD_DDL_TEST_DEFAULT_WIDTH;
	p_decoder->client_frame_size.n_stride = VCD_DDL_TEST_DEFAULT_WIDTH;
	p_decoder->client_frame_size.n_scan_lines = VCD_DDL_TEST_DEFAULT_HEIGHT;
	p_decoder->n_progressive_only = 1;

	p_decoder->n_meta_data_enable_flag = 0;
...
static void ddl_set_default_enc_property(struct ddl_client_context_type *p_ddl)
{
	...
	p_encoder->multi_slice.e_m_slice_sel = VCD_MSLICE_OFF;
	p_ddl->n_b_count = 0;
	...
	p_encoder->db_control.e_db_config         =
		VCD_DB_ALL_BLOCKING_BOUNDARY;
	...
	p_encoder->buf_format.e_buffer_format = VCD_BUFFER_FORMAT_NV12_16M2KA;
	p_encoder->n_hdr_ext_control = 0;
	p_encoder->b_mb_info_enable  = FALSE;
	p_encoder->n_num_references_for_p_frame = DDL_MIN_NUM_REF_FOR_P_FRAME;
	p_encoder->n_meta_data_enable_flag = 0;
	ddl_set_default_encoder_buffer_req(p_encoder);
}
	...
	struct ddl_encoder_data_type *p_encoder)
{
	...
	p_encoder->i_period.n_b_frames = DDL_DEFAULT_NUM_OF_B_FRAME;
}
	...
	struct ddl_encoder_data_type *p_encoder)
{
	...
	if (e_codec == VCD_CODEC_H264) {
		...
		p_encoder->session_qp.n_b_frame_qp = 0x14;
		p_encoder->rc_level.b_frame_level_rc = TRUE;
	...
	case VCD_RATE_CONTROL_CBR_VFR:
		...
		if (e_codec != VCD_CODEC_H264) {
			...
			p_encoder->session_qp.n_b_frame_qp = 0xf;
		}
...
{
	u32 n_y_cb_cr_size, n_y_size;

	n_y_cb_cr_size = ddl_get_yuv_buffer_size(&p_encoder->frame_size,
				&p_encoder->buf_format, FALSE, &n_y_size);
	p_encoder->input_buf_size.n_size_yuv = n_y_cb_cr_size;
	p_encoder->input_buf_size.n_size_y   = n_y_size;
	p_encoder->input_buf_size.n_size_c   = n_y_cb_cr_size - n_y_size;
	memset(&p_encoder->input_buf_req , 0 ,
		sizeof(struct vcd_buffer_requirement_type));
	p_encoder->input_buf_req.n_min_count    = 3;
	...
	if (p_encoder->buf_format.e_buffer_format ==
		VCD_BUFFER_FORMAT_NV12_16M2KA)
		p_encoder->input_buf_req.n_align =
			DDL_LINEAR_BUFFER_ALIGN_BYTES;
	else if (VCD_BUFFER_FORMAT_TILE_4x2 ==
		p_encoder->buf_format.e_buffer_format)
		p_encoder->input_buf_req.n_align = DDL_TILE_BUFFER_ALIGN_BYTES;
	...
	p_encoder->output_buf_req.n_min_count    =
		p_encoder->i_period.n_b_frames + 2;
	...
	if (n_y_cb_cr_size >= VCD_DDL_720P_YUV_BUF_SIZE)
		n_y_cb_cr_size = n_y_cb_cr_size>>1;
	p_encoder->output_buf_req.n_size =
		DDL_ALIGN(n_y_cb_cr_size, DDL_KILO_BYTE(4));
	...
	u32 b_estimate)
{
	...
	if (b_estimate) {
		...
		n_y_cb_cr_size = ddl_get_yuv_buffer_size(p_frame_size,
					...
					(!p_decoder->n_progressive_only), NULL);
	} else {
	...
	if (n_y_cb_cr_size < DDL_KILO_BYTE(1))
		n_y_cb_cr_size = DDL_KILO_BYTE(1);
	else {
		if ((n_y_cb_cr_size & (n_y_cb_cr_size - 1))) {
			u32 n_shift = 0;

			while (n_y_cb_cr_size) {
				n_y_cb_cr_size >>= 1;
				n_shift++;
			}
			n_y_cb_cr_size = 1 << n_shift;
		}
		if (n_y_cb_cr_size > DDL_MEGA_BYTE(4))
			n_y_cb_cr_size = DDL_MEGA_BYTE(4);
	}
...
u32 ddl_get_yuv_buffer_size(struct vcd_property_frame_size_type *p_frame_size,
	struct vcd_property_buffer_format_type *p_buf_format,
	u32 b_interlace, u32 *pn_c_offset)
{
	u32 n_width = p_frame_size->n_stride, n_height =
		p_frame_size->n_scan_lines;
	u32 n_total_memory_size = 0, n_c_offset = 0;

	if (/*(p_buf_format->e_buffer_format == VCD_BUFFER_FORMAT_TILE_1x1) ||*/
		(p_buf_format->e_buffer_format == VCD_BUFFER_FORMAT_TILE_4x2)) {
		u32 n_component_mem_size, n_width_round_up;
		u32 n_height_round_up, n_height_chroma = (n_height >> 1);
		...
		n_height = p_frame_size->n_height;
		n_total_memory_size = n_height * n_width;
		n_c_offset = DDL_ALIGN(n_total_memory_size,
			DDL_LINEAR_MULTIPLY_FACTOR);
		n_total_memory_size = n_c_offset + DDL_ALIGN(
			n_total_memory_size >> 1, DDL_LINEAR_MULTIPLY_FACTOR);
	}
	if (pn_c_offset)
		*pn_c_offset = n_c_offset;
...
void ddl_calculate_stride(struct vcd_property_frame_size_type *p_frame_size,
	u32 b_interlace)
{
	p_frame_size->n_stride = DDL_ALIGN(p_frame_size->n_width,
					DDL_LINEAR_ALIGN_WIDTH);
	if (b_interlace)
		p_frame_size->n_scan_lines = DDL_ALIGN(p_frame_size->n_height,
						DDL_TILE_ALIGN_HEIGHT);
	else
		p_frame_size->n_scan_lines = DDL_ALIGN(p_frame_size->n_height,
...
static u32 ddl_valid_buffer_requirement(struct vcd_buffer_requirement_type
	*original_buf_req, struct vcd_buffer_requirement_type *req_buf_req)
{
	u32 b_status = FALSE;

	if (original_buf_req->n_max_count >= req_buf_req->n_actual_count &&
		original_buf_req->n_min_count <=
		req_buf_req->n_actual_count &&
		!((original_buf_req->n_align - (u32)0x1) &
		req_buf_req->n_align) &&
...
static u32 ddl_decoder_min_num_dpb(struct ddl_decoder_data_type *p_decoder)
{
	...
	case VCD_CODEC_H264:
	{
		u32 n_yuv_size_in_mb = DDL_MIN(DDL_NO_OF_MB(
			p_decoder->client_frame_size.n_height,
			p_decoder->client_frame_size.n_width),
			MAX_FRAME_SIZE_L4PT0_MBS);
		n_min_dpb = DDL_MIN((MAX_DPB_SIZE_L4PT0_MBS /
				n_yuv_size_in_mb), 16);
		n_min_dpb += 2;
		if (n_min_dpb <= 6)
			n_min_dpb += 2;
	...
	struct ddl_property_dec_pic_buffers_type *p_dpb)
{
	...
	for (n_loopc = 0; !vcd_status &&
		...
		if ((!DDL_ADDR_IS_ALIGNED(p_dpb->a_dec_pic_buffers[n_loopc].
			vcd_frm.p_physical,
	...
	p_decoder->dp_buf.a_dec_pic_buffers =
		kmalloc(p_dpb->n_no_of_dec_pic_buf *
			sizeof(struct ddl_frame_data_type_tag), GFP_KERNEL);
...
void ddl_set_initial_default_values(struct ddl_client_context_type *p_ddl)
{
	...
	if (p_ddl->b_decoding) {
		...
		p_encoder->frame_size.n_width = VCD_DDL_TEST_DEFAULT_WIDTH;
		p_encoder->frame_size.n_height = VCD_DDL_TEST_DEFAULT_HEIGHT;
		p_encoder->frame_size.n_scan_lines =
			VCD_DDL_TEST_DEFAULT_HEIGHT;
		p_encoder->frame_size.n_stride = VCD_DDL_TEST_DEFAULT_WIDTH;
