	...
	struct mmc_command *cmd, struct raw_mmc_data *data)
{
	...
	if (reg != CC) {
		...
		printk(KERN_ERR "KPANIC-MMC: %s timeout on CMD%d\n",
			__func__, cmd->opcode);
		goto out;
	}
	...
	timeout = 0;
	reg = OMAP_HSMMC_READ(host->base, PSTATE);
	while (((reg & DLEV_0) == 0) && (timeout++ < MMC_TIMEOUT_COUNT))
		reg = OMAP_HSMMC_READ(host->base, PSTATE);
 ...
 */
static void omap_hsmmc_request(struct raw_omap_hsmmc_host *host,
				struct raw_mmc_request *req)
{
	int err;
...
omap_hsmmc_prepare_data(struct raw_omap_hsmmc_host *host,
			struct raw_mmc_request *req)
{
	DPRINTK(KERN_ERR "KPANIC-MMC: set BLK=%#x\n",
...
static void raw_omap_hsmmc_set_ios(struct raw_omap_hsmmc_host *host)
{
	...
	DPRINTK("KPANIC-MMC: %s power_mode=%d, bus_width=%d, dsor=%d,"
...
int raw_mmc_write(char *buf, sector_t start_sect, sector_t nr_sects,
			unsigned int offset, unsigned int len)
{
	...
	if (kpanic_host != NULL)
		omap_hsmmc_request(kpanic_host, &req);
 ...
 */
int raw_omap_hsmmc_probe_emmc(void)
{
