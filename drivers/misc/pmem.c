...
static void dump(int id)
{
    ...
    for(i = 0; i < pmem[id].num_entries; i++) {
        if (pmem[id].bitmap[i].allocated) {
            alloc_count++;
            order_count += pmem[id].bitmap[i].order;
        }
    }

    //printk("pmem_dump: id:%d num_alloc:%d tot_order:%d\n", id, alloc_count, order_count);
}
#endif
...
#if PMEM_WATCH_USAGE
    printk("pmem_free id:%d index:%08x order:%d\n",
        id,index,PMEM_ORDER(id,index));
#endif 
...
static int pmem_release(struct inode *inode, struct file *file)
{
    ...
    DLOG("current %d(%s) file %p(%ld), inode %p\n",
        current->pid, get_task_comm(currtask_name, current),
        file, file_count(file), inode);
...
static int pmem_open(struct inode *inode, struct file *file)
{
	...
	DLOG("current %u(%s) file %p(%ld), inode %p\n",
        current->pid, get_task_comm(currtask_name, current),
        file, file_count(file), inode);
...
static int pmem_allocate(int id, unsigned long len)
{
...
#if PMEM_WATCH_USAGE
    printk("pmem_allocate attempt id:%d len:%ld\n", id, len);
#endif
	 ...
	 */
	if (best_fit < 0) {
		printk("pmem: no space left to allocate! request len %d\n",len);
		return -1;
	}
...
#if PMEM_WATCH_USAGE
    printk("pmem_allocate id:%d len:%ld name:%s index:%08x\n", id, len, pmem[id].dev.name,best_fit);
    dump(id);
#endif
