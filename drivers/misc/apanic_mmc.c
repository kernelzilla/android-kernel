...
static char mmcpart_dev_name[BDEVNAME_SIZE];
...
static void mmc_panic_notify_add(struct raw_hd_struct *hd)
{
	...
	printk(KERN_ERR "apanic: Bound to mmc block device '%s(%d:%d)'\n",
		mmcpart_dev_name, hd->major, hd->first_minor + hd->partno);
...
static int apanic_write_console_mmc(unsigned int off)
{
	...
	while (!last_chunk) {
		...
		rc2 = raw_mmc_write(ctx->bounce, ctx->mmchd->start_sect,
			ctx->mmchd->nr_sects, off, rc);
		if (rc2 <= 0) {
			printk(KERN_EMERG
			...
			void *ptr)
{
	...
	rc = raw_mmc_write(ctx->bounce, ctx->mmchd->start_sect,
		ctx->mmchd->nr_sects, 0, console_offset);
	if (rc <= 0) {
	...
	printk(KERN_EMERG "apanic: Panic dump sucessfully written\n");
...
static int __init apanic_mmc_init(void)
{
	...
	ret = get_mmcpart_by_name(CONFIG_APANIC_PLABEL, mmcpart_dev_name);
	if (ret < 0) {
		printk(KERN_ERR "apanic: %s partiton not on eMMC\n",
