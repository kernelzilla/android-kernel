...
void scxPublicCryptoPowerManagementTimerCbk(unsigned long aPtr)
{
	...
	/*take the lock on the timer semaphore */

	if (down_trylock(&(pSMComm->sPowerManagementTimerLock)) == 0) {
	/*if one of the HWA is used (by secure or public) the timer function
	 *cuts all the HWA clocks */
	...
	if (down_trylock(&g_SCXLNXDeviceMonitor.sAES1CriticalSection) == 0)
		nHWADisableClock |= RPC_AES_CODE;

	if (down_trylock(&g_SCXLNXDeviceMonitor.sD3D2CriticalSection) == 0)
		nHWADisableClock |= RPC_DES_CODE;

	if (down_trylock(&g_SCXLNXDeviceMonitor.sSHAM1CriticalSection) == 0)
		nHWADisableClock |= RPC_SHA_CODE;

	/*perform the clock cuts if the 3 HWAs were unlocked */
	if (nHWADisableClock == (RPC_AES_CODE | RPC_DES_CODE | RPC_SHA_CODE)) {
		dprintk(KERN_INFO "CBK cut HWA clocks\n");
		scxPublicCryptoDisableClock(PUBLIC_CRYPTO_AES1_CLOCK_BIT);
		scxPublicCryptoDisableClock(PUBLIC_CRYPTO_DES2_CLOCK_BIT);
...
u32 scxPublicCryptoInit(void)
{
...
#ifdef SMODULE_SMC_OMAP3XXX_PUBLIC_DMA

	int nChannel;

	dprintk("scxPublicCryptoInit\n");
	 ...
	 */
	for (nChannel = OMAP3XXX_SMC_DMA_CH_0;
		  nChannel < OMAP3XXX_SMC_DMA_CH_0 + OMAP3XXX_SMC_DMA_CH_NB;
		  nChannel++) {
...
#endif	/*SMODULE_SMC_OMAP3XXX_PUBLIC_DMA */
	...
	g_SCXLNXDeviceMonitor.pDMABuffer =
		(u8 *)internal_get_free_pages(GFP_KERNEL, 4);
...
/*
	dprintk(KERN_INFO
		"scxPublicCryptoUpdate(%p) : \
		HWAID=0x%x, In=%p, Out=%p, Len=%u, InputSh=%x, OutputSh=%x\n",
		pCUSContext, pCUSContext->nHWAID, pCUSParams->pDataBuffer,
	...
	list_for_each_entry(
	   pCUSContext, &(pDeviceContext->ShortcutList), list) {
		...
		if ((pCUSContext->hClientSession == pCommand->hClientSession)
			...
			/*bSuspended of the shortcut context is set to true,
			 *it means that the secure world has suspended the
			 *shortcut to perform an update on its own.
			 *In this case, take the standard path.
			 *This should happen very rarely because the client
			 *and the service should generally communicate
			 *to avoid such a collision */
			if (pCUSContext->bSuspended) {
				dprintk(KERN_INFO
					"shortcut exists but suspended\n");
				...
				if (pCUSContext->nHWAID == RPC_AES_CODE &&
					 ...
					 *shortcut */
					dprintk(KERN_INFO
"shortcut exists but AES key not correct\nhKeyContext=0x%08x vs 0x%08x\n",
					 ...
					 *hD3D2SecureKeyContext and check if it
					 *is equal to hKeyContext. If not, take
					 *the standard path <=> do not shortcut
					 */
					dprintk(KERN_INFO "shortcut exists \
			but DES key not correct hD3D2SecureKey \
			Context = 0x%08x hKeyContext0x%08x\n",
			(u32)g_SCXLNXDeviceMonitor.hD3D2SecureKeyContext,
				...
				CRYPTOKI_UPDATE_PARAMS *pCUSParams)
{
	...
	if (pCurrentCommandMessageElement < pEndOfCommandMessage) {
		...
		if (pCurrentCommandMessageElement != pEndOfCommandMessage) {
			dprintk(KERN_INFO
				"scxPublicCryptoParseCommandMessage: \
	Encrypt/Decrypt operation with data after the output buffer: Error\n");
			return false;
		}
		...
		/*pS2CDataBufferEnd += 5;*/
		pS2CDataBufferEnd = static_befEncoderWriteUntaggedUint32(
			pS2CDataBufferEnd, nResultDataLength);
	...
	pSharedMemoryDescriptor = PDrvCryptoGetSharedMemoryFromBlockHandle(
					pConn, hBlock);
...
void scxPublicCryptoDisableClock(uint32_t vClockBit)
{
	...
	if (vClockBit <= PUBLIC_CRYPTO_AES1_CLOCK_BIT) {
		pClockReg = (u32 *)IO_ADDRESS(PUBLIC_CRYPTO_HW_CLOCK_ADDR);
		pAutoIdleReg =
			(u32 *)IO_ADDRESS(PUBLIC_CRYPTO_HW_AUTOIDLE_ADDR);
	} else {
	  pClockReg = (u32 *)IO_ADDRESS(PUBLIC_CRYPTO_HW_CLOCK1_ADDR);
	  pAutoIdleReg = (u32 *)IO_ADDRESS(PUBLIC_CRYPTO_HW_AUTOIDLE1_ADDR);
	}
