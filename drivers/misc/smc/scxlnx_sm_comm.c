      ...
      u32 *pnFlags)
{
   ...
   /* And no attempt to register the memory */
   *pnBufferAddr = 0;

   if ((*pnBufferAddr != 0)/*In this case (direct access) : - try to access the memory directly
...
static void sclnx_device_flush_shared_mem_cache(SCXLNX_CONN_MONITOR *pConn)
{
   ...
   {
      /* Virtual Address  : pShmemDesc->pBuffer */
      /* Physical Address : pShmemDesc->nBufferPhysAddr */
      /* Size            : pShmemDesc->nAllocatedBufferSize */
      ...
      int bKillable)
{
    ...
    */
      ...
      {
         /* Flush all the shared memories cache. */
   ...
   while (pAnswer != NULL) {
      nError = wait_for_completion_interruptible(&sAnswerStructure.sAnswerEvent);
      ...
      if (unlikely(freezing(current))) {
         ...
         if (bKillable && signal_pending(current))
         {
            ...
            dprintk(KERN_INFO "SCXLNXSMCommSendReceive(%p) : Interrupted by user signal, returning error code %d\n",
                     pSMComm, -EINTR);

            if (pMessage->nMessageType == SCX_MESSAGE_TYPE_CREATE_DEVICE_CONTEXT)
            {
               /* Special case :
                * If we are creating a device context, we cannot destroy until
                * the answer is received.
                */
               dprintk(KERN_INFO "SCXLNXSMCommSendReceive(%p) : Wait for CREATE_DEVICE_CONTEXT Answer\n", pSMComm);
               wait_for_completion(&sAnswerStructure.sAnswerEvent);
               if (pAnswer->nSChannelStatus != S_SUCCESS)
               {
                  dprintk(KERN_INFO "SCXLNXSMCommSendReceive(%p) : No need the destroy context\n", pSMComm);
                  hDeviceContext = 0;
               }
               else
               {
                  hDeviceContext = pAnswer->sBody.sCreateDeviceContextAnswer.hDeviceContext;
               }
            }
            else
            {
               hDeviceContext = pConn->sSharedMemoryMonitor.pDeviceContextDesc->hIdentifier;
            }

            if (hDeviceContext != 0)
            {
               SCX_COMMAND_MESSAGE sMessage;
               SCXLNX_SM_ANSWER_STRUCT sAnswerStructureDestroyContext;

               dprintk(KERN_INFO "SCXLNXSMCommSendReceive(%p) : sending DESTROY_DEVICE_CONTEXT\n", pSMComm);

               sMessage.nMessageType = SCX_MESSAGE_TYPE_DESTROY_DEVICE_CONTEXT;
               sMessage.nOperationID = (u32)&sAnswerStructureDestroyContext;
               sMessage.sBody.sDestroyDeviceContextMessage.hDeviceContext = hDeviceContext;

               /* Reuse pAnswer Structure */
               sAnswerStructureDestroyContext.pAnswer = pAnswer;
               init_completion(&sAnswerStructureDestroyContext.sAnswerEvent);
               ...
               wait_for_completion(&sAnswerStructureDestroyContext.sAnswerEvent);

               dprintk(KERN_INFO "SCXLNXSMCommSendReceive(%p) : DESTROY_DEVICE_CONTEXT received\n", pSMComm);
            }

            if (pMessage->nMessageType != SCX_MESSAGE_TYPE_CREATE_DEVICE_CONTEXT)
            {
               wait_for_completion(&sAnswerStructure.sAnswerEvent);
               dprintk(KERN_INFO "SCXLNXSMCommSendReceive(%p) : Answer to the interrupted command received\n", pSMComm);
            }
...
u32 SCXLNXSMCommRPCHandler(u32 nRPCId, u32 nRPCCommand, u32 nReserved1, u32 nReserved2)
{
   ...
   case RPC_CMD_TRACE:
      spin_lock(&(pSMComm->lock));
      secure_dprintk(KERN_INFO "SMC PA: %s", &(((SCHANNEL_L0_BUFFER_INPUT *)pSMComm->pL0SharedBuffer)->sReserved[RPC_TRACE_OFFSET]));
      spin_unlock(&(pSMComm->lock));
      break;
   ...
   case RPC_CMD_PUBLIC_CRYPTO_INVALIDATE_SESSION:
      /* Only invalidate the current session if it is an AES session */
      if ((pSMComm->pubcrypto.pCurrentSession != NULL) &&
         (pSMComm->pubcrypto.pCurrentSession->nCryptoAlgId == PUBLIC_CRYPTO_ALG_CIPHER_AES))
      {
