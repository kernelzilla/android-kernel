...
static u32 SCXLNXSMCommRPCInit(SCXLNX_SM_COMM_MONITOR *pSMComm)
{
	...
	dprintk(KERN_ERR "SCXLNXSMCommRPCInit(%p)\n", pSMComm);
	...
	if (pSMComm->pPABuffer != NULL) {
		dprintk(KERN_INFO
			...
			pSMComm);
		internal_vunmap(pSMComm->pPABuffer);
		internal_kfree(pSMComm->pPABufferRaw);
	...
	if (pSMComm->pPAInfo != NULL) {
		dprintk(KERN_INFO
			...
			pSMComm);
		internal_vunmap(pSMComm->pPAInfo);
		internal_kfree(pSMComm->pPAInfoRaw);
...
void SCXLNXSMCommStop(SCXLNX_SM_COMM_MONITOR *pSMComm)
{
	...
	if (pSMComm->pBuffer != NULL) {
		dprintk(KERN_INFO
			...
			pSMComm);
		CLEAR_PAGE_LOCKED(virt_to_page(pSMComm->pBufferRaw));
		internal_free_page_vunmap((unsigned long)pSMComm->pBuffer);
		pSMComm->pBufferRaw = NULL;
		pSMComm->pBuffer = NULL;
	}
	...
	if (pSMComm->pL0SharedBuffer != NULL) {
		dprintk(KERN_INFO
			...
			pSMComm);
		CLEAR_PAGE_LOCKED(virt_to_page(pSMComm->pL0SharedBufferRaw));
		internal_free_page_vunmap((unsigned long)pSMComm->
					  pL0SharedBuffer);
		internal_free_page((unsigned long)pSMComm->pL0SharedBufferRaw);
		pSMComm->pL0SharedBufferRaw = NULL;
		pSMComm->pL0SharedBuffer = NULL;
	}
