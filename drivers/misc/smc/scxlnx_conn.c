		...
		SCX_COMMAND_MESSAGE *pMessage)
{
	...
	case SCX_MESSAGE_TYPE_CREATE_DEVICE_CONTEXT:
		...
		if (pConn->nState != SCXLNX_CONN_STATE_NO_DEVICE_CONTEXT) {
			...
			pConn->nState = SCXLNX_CONN_STATE_CREATE_DEVICE_CONTEXT_SENT;
			nValid = 1;
		}
		break;
	...
	case SCX_MESSAGE_TYPE_DESTROY_DEVICE_CONTEXT:
		...
		if (pConn->nState != SCXLNX_CONN_STATE_VALID_DEVICE_CONTEXT) {
			...
			pConn->nState = SCXLNX_CONN_STATE_DESTROY_DEVICE_CONTEXT_SENT;
			nValid = 1;
		}
		break;
 ...
 *the commands CREATE_DEVICE_CONTEXT, REGISTER_SHARED_MEMORY and POWER_MANAGEMENT
 *---------------------------------------------------------------------------- */
		...
		u32 nFullCleanup)
{
	...
	dprintk(KERN_INFO "SCXLNXConnUnmapShmem(%p) : descriptor %p\n",
				pShmemMonitor, pShmemDesc);

	down(&(pShmemMonitor->sharedMemoriesMutex));

	SCXLNXSMCommReleaseDescriptor(
			pShmemDesc,
			1);

	list_del(&(pShmemDesc->list));

	if ((pShmemDesc->nType == SCXLNX_SHMEM_TYPE_REGISTERED_SHMEM) || (nFullCleanup != 0)) {
		/*free descriptor */
		...
		list_add(&(pShmemDesc->list), &(pShmemMonitor->sFreeSharedMemoryList));
	}
...
static int SCXLNXConnMapShmem(
		SCXLNX_SHMEM_MONITOR * pShmemMonitor,
		u32 *pnBufferAddr,
		u32	nFlags, /*flags for read-write access rights on the memory */
		...
		SCXLNX_SHMEM_DESC **ppShmemDesc)
{
	...
	dprintk(KERN_INFO "SCXLNXConnMapShmem(%p) : Buffer=0x%x, Size=%u bytes\n",
	...
	if (list_empty(&(pShmemMonitor->sFreeSharedMemoryList))) {
		if (atomic_read(&(pShmemMonitor->nShmemAllocated)) == SCXLNX_SHMEM_MAX_DESCRIPTORS) {
			printk(KERN_ERR "SMC: ConnMapShmem: Maximum shared memories already registered\n");
			nError = -ENOMEM;
			goto error;
		}
		...
		pShmemDesc = (SCXLNX_SHMEM_DESC *)internal_kmalloc(sizeof(*pShmemDesc), GFP_KERNEL);
		if (pShmemDesc == NULL) {
			printk(KERN_ERR "SMC: ConnMapShmem: Failed to allocate descriptor\n");
			nError = -ENOMEM;
			goto error;
		}
		...
		pShmemDesc = list_entry(pShmemMonitor->sFreeSharedMemoryList.next, SCXLNX_SHMEM_DESC, list);
		list_del(&(pShmemDesc->list));
	}
	...
	nError = SCXLNXSMCommFillDescriptor(
					pShmemDesc,
					pnBufferAddr,
					nBufferSize,
					&nFlags);
	if (nError != 0) {
		dprintk(KERN_ERR "SCXLNXConnMapShmem(%p) : SCXLNXSMCommFillDescriptor failed",
					SCXLNXConnMapShmem);
		goto error;
	}
		...
		u32 nBufferSize)
{
	...
	dprintk(KERN_INFO "SCXLNXConnCheckSharedMem(%p) : Buffer=0x%x, Size=%u\n",
	...
	list_for_each_entry(pShmemDesc, &(pShmemMonitor->sUsedSharedMemoryList), list)
	{
		if (pBuffer == pShmemDesc->pBuffer) {
			if (nBufferSize <= pShmemDesc->nAllocatedBufferSize) {
				u32 nBufferPhysAddr = pShmemDesc->nBufferPhysAddr;
				up(&(pShmemMonitor->sharedMemoriesMutex));
				dprintk(KERN_INFO "SCXLNXConnCheckSharedMem(%p) : Success (PhysAddr=0x%x)\n",
							pShmemMonitor, nBufferPhysAddr);
				return nBufferPhysAddr;
			} else 	{
				up(&(pShmemMonitor->sharedMemoriesMutex));
				dprintk(KERN_ERR "SCXLNXConnCheckSharedMem(%p) : Incorrect size [Allocated=%d]\n",
							pShmemMonitor, pShmemDesc->nAllocatedBufferSize);
				return 0;
			}
		}
	}
...
static void SCXLNXSharedMemoryCleanupList(
		SCXLNX_SHMEM_MONITOR * pSharedMemoryMonitor,
		struct list_head *pList)
{
	while (!list_empty(pList)) {
		SCXLNX_SHMEM_DESC *pShmemDesc;

		pShmemDesc = list_entry(
						pList->next,
						SCXLNX_SHMEM_DESC,
						list);
 ...
 */
void SCXLNXSharedMemoryMonitorCleanup(
		SCXLNX_SHMEM_MONITOR * pSharedMemoryMonitor)
{
	 ...
	 *mutex()ed
	 */
	SCXLNXSharedMemoryCleanupList(
			pSharedMemoryMonitor,
			&pSharedMemoryMonitor->sUsedSharedMemoryList);
	SCXLNXSharedMemoryCleanupList(
			pSharedMemoryMonitor,
			&pSharedMemoryMonitor->sFreeSharedMemoryList);
}
 ...
 */
int SCXLNXSharedMemoryMonitorInitialize(
		SCXLNX_SHMEM_MONITOR * pSharedMemoryMonitor)
{
	int nError;
	 ...
	 */
	for (nSharedMemoryDescriptorIndex = 0; nSharedMemoryDescriptorIndex < 3; nSharedMemoryDescriptorIndex++) {
		SCXLNX_SHMEM_DESC *pShmemDesc = (SCXLNX_SHMEM_DESC *)internal_kmalloc(sizeof(*pShmemDesc), GFP_KERNEL);

		if (pShmemDesc == NULL) {
			printk(KERN_ERR "SMC: SharedMemoryMonitorInitialize: Failed to pre-allocate descriptor %d\n",
		...
		if (nSharedMemoryDescriptorIndex == 0) {
			...
			/*store this descriptor in the device context descriptor pointer */
			pSharedMemoryMonitor->pDeviceContextDesc = pShmemDesc;
		} else {
			/*
			 *This one is used as a registered shared memory descriptor
			 */
		...
		/*
		 * add this preallocated descriptor to the list of free descriptors
		 *Keep the device context specific one at the beginning of the list
		 */
		INIT_LIST_HEAD(&(pShmemDesc->list));
		list_add_tail(&(pShmemDesc->list), &(pSharedMemoryMonitor->sFreeSharedMemoryList));
	}
...
struct vm_area_struct *SCXLNXConnFindVma(struct mm_struct *mm, unsigned long addr, unsigned long size)
{
	...
	if (mm) {
		...
		vma = mm->mmap_cache;
		if (!(vma && vma->vm_end >= (addr+size)  && vma->vm_start <= addr)) {
	...
	u32 nFlags)
{
	...
	if ((nSharedMemorySize == 0) || (((u32)pSharedMemory + nSharedMemorySize) < (u32)pSharedMemory)) {
		return -EINVAL;
	}
	...
	SCX_ANSWER_MESSAGE *pAnswer)
{
	...
	dprintk(KERN_INFO "SCXLNXConnCreateDeviceContext(%p) : MasterHeap={0x%x, %u}\n",
				pConn, pMessage->sBody.sCreateDeviceContextMessage.nSharedMemStartOffset,
	...
	if (!access_ok(VERIFY_WRITE,
			(void *)pMessage->sBody.sCreateDeviceContextMessage.nSharedMemDescriptors[1],
			sizeof(SCX_VERSION_INFORMATION_BUFFER))) {
		dprintk(KERN_ERR "SCXLNXConnCreateDeviceContext(%p) : pVersionBuffer descriptor is not accessible for writing 0x%08x\n",
					pConn, pMessage->sBody.sCreateDeviceContextMessage.nSharedMemDescriptors[1]);
		nError = -EFAULT;
		goto error0;
	}
	pVersionBuffer = (SCX_VERSION_INFORMATION_BUFFER *)pMessage->sBody.sCreateDeviceContextMessage.nSharedMemDescriptors[1];


	/*
	 * As we allocate memory here, make sure that the connection state is valid
	 */
	/*CREATE_DEVICE_CONTEXT only valid while no context created */
	spin_lock(&(pConn->stateLock));
	if (pConn->nState != SCXLNX_CONN_STATE_NO_DEVICE_CONTEXT) {
	...
	if (nError != 0) {
		dprintk(KERN_ERR "SCXLNXConnCreateDeviceContext(%p) : Invalid state\n", pConn);
		goto error0;
	}
	 ...
	 *It will use the first pre-allocated descriptor, which is dedicated to
	 *the device context.
	 */
	nError = SCXLNXConnMapShmem(
						&(pConn->sSharedMemoryMonitor),
						&(pMessage->sBody.sCreateDeviceContextMessage.nSharedMemDescriptors[0]),
						nFlags,
						pMessage->sBody.sCreateDeviceContextMessage.nSharedMemStartOffset,
						pMessage->sBody.sCreateDeviceContextMessage.nSharedMemSize,
						&pShmemDesc);
	if (nError != 0) {
		dprintk(KERN_ERR "SCXLNXConnCreateDeviceContext(%p) : SCXLNXConnMapShmem failed [%d]",
					pConn, nError);
		goto error0;
	}

	dprintk(KERN_INFO "SCXLNXConnCreateDeviceContext(%p) : MasterHeap={0x%x, 0x%x}\n",
	...
	if (pAnswer->nSChannelStatus == S_SUCCESS) {
		 ...
		 */
		pShmemDesc->hIdentifier = pAnswer->sBody.sCreateDeviceContextAnswer.hDeviceContext;
		...
		if (pShmemDesc->nAllocType != SCXLNX_SHMEM_ALLOC_TYPE_REGISTER) {
			pAnswer->sBody.sCreateDeviceContextAnswer.nSharedMemDescriptor = (u32)pShmemDesc->pBuffer;
		} else {
			pAnswer->sBody.sCreateDeviceContextAnswer.nSharedMemDescriptor = 0;
		}
	...
	{
		dprintk(KERN_WARNING "SCXLNXConnCreateDeviceContext(%p) : Failed to copy back the driver description from %p to %p\n",
					pConn, pConn->pDevice->sDriverDescription, pVersionBuffer->sDriverDescription);
		nError = -EFAULT;
		goto error2;
	}
	...
	{
		dprintk(KERN_WARNING "SCXLNXConnCreateDeviceContext(%p) : Failed to copy back the secure world description to %p\n",
	...
	/*successful completion */
	dprintk(KERN_INFO "SCXLNXConnCreateDeviceContext(%p) : hDeviceContext=0x%08x\n",
				pConn, (unsigned int)(pAnswer->sBody.sCreateDeviceContextAnswer.hDeviceContext));
	return 0;
...
error2:
	if (pConn->nState == SCXLNX_CONN_STATE_VALID_DEVICE_CONTEXT) {
		...
		dprintk(KERN_WARNING "SCXLNXConnCreateDeviceContext(%p) : Reverting Device creation\n", pConn);
...
error0:
	if (nError != 0) {
		dprintk(KERN_ERR "SCXLNXConnCreateDeviceContext(%p) : Failed [%d]",
					pConn, nError);
	} else {
		/*
		 *We sent a DeviceCreateContext. The state is now  SCXLNX_CONN_STATE_CREATE_DEVICE_CONTEXT_SENT
		 *It has to be reset if we ever want to send a DeviceCreateContext again
		 */
		spin_lock(&(pConn->stateLock));
		pConn->nState = SCXLNX_CONN_STATE_NO_DEVICE_CONTEXT;
		spin_unlock(&(pConn->stateLock));
		dprintk(KERN_ERR "SCXLNXConnCreateDeviceContext(%p) : Failed [ChannelStatus=0x%08x]",
	...
	SCX_ANSWER_MESSAGE *pAnswer)
{
	...
	dprintk(KERN_DEBUG "SCXLNXConnOpenClientSession(%p)\n", pConn);
	...
	case S_LOGIN_OS_IDENTIFICATION:
		pIdentifier = (SCX_UUID *) &pMessage->sBody.sOpenClientSessionMessage.sLoginData[0];

		/*We need to send an OS identifier. On Linux: this is an UUID derived
			from the executable path, the UID, and EUID of the calling process */
		nError = SCXLNXConnGetCurrentProcessUUID(pIdentifier);
		if (nError != 0) {
			dprintk(KERN_ERR "SCXLNXConnOpenClientSession: error in SCXLNXConnGetCurrentProcessUUID");
			goto exit;
		}
		dprintk(KERN_DEBUG "SCXLNXConnOpenClientSession(%p) : process identity ="
	...
	case S_LOGIN_AUTHENTICATION:
		pHash = &(pMessage->sBody.sOpenClientSessionMessage.sLoginData[0]);

		/*We need to send an OS identifier. On Linux: this is the hash of the
			executable binary */
		nError = SCXLNXConnGetCurrentProcessHash(pHash);
		if (nError != 0) {
			dprintk(KERN_ERR "SCXLNXConnOpenClientSession: error in SCXLNXConnGetCurrentProcessHash");
			goto exit;
		}
		dprintk(KERN_DEBUG "SCXLNXConnOpenClientSession(%p) : process hash ="
	...
	case S_LOGIN_PRIVILEGED:
		...
		if (CURRENT_EUID != 0 && CURRENT_EGID != 0) {
			dprintk(KERN_ERR "SCXLNXConnOpenClientSession: "
					" user %d not allowed to open session with S_LOGIN_PRIVILEGED_n", CURRENT_UID);
			nError = -EACCES;
			goto exit;
		}
	...
	default:
		dprintk(KERN_ERR "SCXLNXConnOpenClientSession: unknown nLoginType(%08X)", pMessage->sBody.sOpenClientSessionMessage.nLoginType);
		nError = -EOPNOTSUPP;
		goto exit;
	}

	pMessage->sBody.sOpenClientSessionMessage.hDeviceContext = pConn->sSharedMemoryMonitor.pDeviceContextDesc->hIdentifier;
...
exit:
	if (nError != 0) {
		...
		dprintk(KERN_ERR "SCXLNXConnOpenClientSession returns nChannelStatus 0x%08X\n", pAnswer->nSChannelStatus);
	}
	...
	SCX_ANSWER_MESSAGE *pAnswer)
{
	...
	pMessage->sBody.sCloseClientSessionMessage.hDeviceContext = pConn->sSharedMemoryMonitor.pDeviceContextDesc->hIdentifier;
	...
	if (nError != 0) {
		...
		dprintk(KERN_ERR "SCXLNXConnCloseClientSession returns nChannelStatus 0x%08X\n",
					pAnswer->nSChannelStatus);
	}
	...
	SCX_ANSWER_MESSAGE *pAnswer)
{
	...
	dprintk(KERN_INFO "SCXLNXConnRegisterSharedMemory(%p) : Block=0x%x, %u bytes\n",
			pConn,
			pMessage->sBody.sRegisterSharedMemoryMessage.nSharedMemDescriptors[0],
			pMessage->sBody.sRegisterSharedMemoryMessage.nSharedMemSize);

	nError = SCXLNXConnValidateSharedMemoryBlockAndFlags(
			(void *)pMessage->sBody.sRegisterSharedMemoryMessage.nSharedMemDescriptors[0],
			pMessage->sBody.sRegisterSharedMemoryMessage.nSharedMemSize,
   ...
   /* SCX_SHMEM_TYPE_DIRECT | SCX_SHMEM_TYPE_DIRECT_FORCE not implemented yet */
   pMessage->sBody.sRegisterSharedMemoryMessage.nFlags &= ~(SCX_SHMEM_TYPE_DIRECT | SCX_SHMEM_TYPE_DIRECT_FORCE);
	...
	nError = SCXLNXConnMapShmem(
		...
		&(pMessage->sBody.sRegisterSharedMemoryMessage.nSharedMemDescriptors[0]),
		pMessage->sBody.sRegisterSharedMemoryMessage.nFlags,
		pMessage->sBody.sRegisterSharedMemoryMessage.nSharedMemStartOffset,
		pMessage->sBody.sRegisterSharedMemoryMessage.nSharedMemSize,
		&pShmemDesc);

	if (nError != 0) {
		dprintk(KERN_ERR "SCXLNXConnRegisterSharedMemory(%p) : SCXLNXConnMapShmem failed [%d]",
					pConn, nError);
		goto error0;
	}

	dprintk(KERN_INFO "SCXLNXConnRegisterSharedMemory(%p) : Block={0x%x, 0x%x}\n",
			pConn, (u32)pShmemDesc->pBuffer,
			pMessage->sBody.sRegisterSharedMemoryMessage.nSharedMemDescriptors[0]);
	...
	pMessage->sBody.sRegisterSharedMemoryMessage.nBlockID = (u32)pShmemDesc;
	...
	if ((nError != 0) || (pAnswer->nSChannelStatus != S_SUCCESS)) {
		dprintk(KERN_ERR "SCXLNXConnRegisterSharedMemory: operation failed. Unmap block\n");
		goto error1;
	}

	/*Store the block handle returned by the Secure World... */
	pShmemDesc->hIdentifier = pAnswer->sBody.sRegisterSharedMemoryAnswer.hBlock;
	/*Return the buffer to map in the user space to the client if required */
	if (pShmemDesc->nAllocType != SCXLNX_SHMEM_ALLOC_TYPE_REGISTER) {
		pAnswer->sBody.sRegisterSharedMemoryAnswer.nSharedMemDescriptor = (u32)pShmemDesc->pBuffer;
	} else {
		pAnswer->sBody.sRegisterSharedMemoryAnswer.nSharedMemDescriptor = 0;
	}


	/*successful completion */
	dprintk(KERN_INFO "SCXLNXConnRegisterSharedMemory(%p) : nBlockID=0x%08x hBlock=0x%08x\n",
...
error0:
	if (nError != 0) {
		...
		dprintk(KERN_ERR "SCXLNXConnRegisterSharedMemory returns nChannelStatus 0x%08X\n", pAnswer->nSChannelStatus);
	}
	...
	SCX_ANSWER_MESSAGE *pAnswer)
{
	...
	SCXLNXConnUnmapShmem(
			&(pConn->sSharedMemoryMonitor),
			(SCXLNX_SHMEM_DESC *)pAnswer->sBody.sReleaseSharedMemoryAnswer.nBlockID,
			0);
...
error:
	if (nError != 0) {
		...
		dprintk(KERN_ERR "SCXLNXConnReleaseSharedMemory returns nChannelStatus 0x%08X\n", pAnswer->nSChannelStatus);
	}
...
/*
 *Invokes a client command to the SM
 */
...
{
	int nError = 0;
	SCX_PUBLIC_CRYPTO_DESC *pPublicCryptoSession = NULL;
	bool bIsSessionCorrupted = false;

	dprintk(KERN_DEBUG "SCXLNXConnInvokeClientCommand(%p)\n", pConn);

	pMessage->sBody.sInvokeClientCommandMessage.hDeviceContext = pConn->sSharedMemoryMonitor.pDeviceContextDesc->hIdentifier;

	dprintk(KERN_INFO "SCXLNXConnInvokeClientCommand: hSessionClient=0x%08x\n",
			  pMessage->sBody.sInvokeClientCommandMessage.hClientSession);

	/*
	 *Lock public crypto and the SM lock, i.e. lock the Public crypto management (with the HWA),
	 *Check if the command must be intercepted, and if yes,
	 *Process the crypto operation in public.
	 * All these operations (including the crypto HWA accesses)are protected
	 *by the global lock.
	 */
	down(&(pConn->pDevice->sm.pubcrypto.HWALock));

	pPublicCryptoSession = scxPublicCryptoCommandMatchAndCheck(
			&(pConn->pDevice->sm.pubcrypto),
			&(pMessage->sBody.sInvokeClientCommandMessage),
			&bIsSessionCorrupted);

	if (pPublicCryptoSession != NULL) {
		/*
		 *This is a command that must be at least pre-processed in public.
		 */
		nError = scxPublicCryptoPreProcessCommand(
				pConn->sSharedMemoryMonitor.pDeviceContextDesc,
				pPublicCryptoSession,
				&(pMessage->sBody.sInvokeClientCommandMessage),
				&(pAnswer->sBody.sInvokeClientCommandAnswer));
		if ((nError == 0) && (!bIsSessionCorrupted)) {
			/*
			 *This is a command that must be fully processed now.
			 *So do it, and then just return (no exchange with the PA).
			 */
			scxPublicCryptoProcessCommand(
					pPublicCryptoSession,
					&(pAnswer->sBody.sInvokeClientCommandAnswer));
		}

		/*
		 *If an error occurred in the pre-processing, or
		 *if the session has not been corrupted (i.e. has been fully processed)
		 */
		if ((nError != 0) || (!bIsSessionCorrupted)) {
			/*Completion (successfully or with error)*/
			pAnswer->nMessageType = pMessage->nMessageType;
			pAnswer->nOperationID = 0;
			pAnswer->nSChannelStatus = S_SUCCESS;

			up(&(pConn->pDevice->sm.pubcrypto.HWALock));
			return 0;
		}

		/*
		 *The crypto operation has been pre-processed.
		 *Forward the command to the PA as usual...
		 */
	}
	up(&(pConn->pDevice->sm.pubcrypto.HWALock));
	...
	/*
	 * Retrieve the public crypto session: it may have been
	 * removed by the call to SCXSMCommSendReceive
	 */
	down(&(pConn->pDevice->sm.pubcrypto.HWALock));
	...
	if (pPublicCryptoSession != NULL) {
		/*
		 *This operation need to be post-processed to update
		 *the user result buffer (if required).
		 */
		scxPublicCryptoPostProcessCommand(
				pPublicCryptoSession,
				&(pAnswer->sBody.sInvokeClientCommandAnswer),
				false, 0);
	}
	...
	dprintk(KERN_INFO "SCXLNXConnInvokeClientCommand(%p) :"
			...
			pConn, pMessage->sBody.sInvokeClientCommandMessage.nClientOperationID);
	return 0;
...
error:
	if (nError != 0) {
		...
		dprintk(KERN_ERR "SCXLNXConnInvokeClientCommand returns nChannelStatus 0x%08X\n", pAnswer->nSChannelStatus);
	}
	...
	SCX_ANSWER_MESSAGE *pAnswer)
{
	...
	pMessage->sBody.sCancelClientOperationMessage.hDeviceContext = pConn->sSharedMemoryMonitor.pDeviceContextDesc->hIdentifier;
	...
	dprintk(KERN_INFO "SCXLNXConnCancelClientCommand(%p) :"
			" nClientOperationID=0x%08x\n",
			pConn, pMessage->sBody.sCancelClientOperationMessage.nClientOperationID);
	return 0;
...
error:
	if (nError != 0) {
		...
		dprintk(KERN_ERR "SCXLNXConnCancelClientCommand returns nChannelStatus 0x%08X\n", pAnswer->nSChannelStatus);
	}
	...
	SCXLNX_CONN_MONITOR * pConn)
{
	 ...
	 *it is guarantied that the first shared memory descriptor describes the device context
	 */
...
error:
	dprintk(KERN_ERR "SCXLNXConnDestroyDeviceContext failed with error %d and nSChannelStatus 0x%x", nError, sAnswer.nSChannelStatus);
		...
		SCXLNX_CONN_MONITOR **ppConn)
{
	 ...
	 */
	pConn = (SCXLNX_CONN_MONITOR *)internal_kmalloc(sizeof(*pConn), GFP_KERNEL);
	if (pConn == NULL) {
		printk(KERN_ERR "SMC: ConnOpen: Out of memory for connection descriptor !\n");
		nError = -ENOMEM;
		goto error;
	}
	 ...
	 *Initialize the shared memory monitor.
	 */
	nError = SCXLNXSharedMemoryMonitorInitialize(&(pConn->sSharedMemoryMonitor));
	if (nError != 0) {
		goto error;
	}
		...
		SCXLNX_CONN_MONITOR *pConn)
{
	...
	if (nState == SCXLNX_CONN_STATE_VALID_DEVICE_CONTEXT) {
		/*A DestroyDeviceContext operation was not performed. Do it now. */
