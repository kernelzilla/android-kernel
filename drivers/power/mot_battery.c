...
/* Local constants */
enum {
   MOT_BATTERY_NUMBER_OF_PROPERTIES = sizeof(mot_battery_props) / sizeof(mot_battery_props[0]),
...
typedef struct {
   ...
   get_battery_property       get;       /* function to get new value for the property */
   char*                      name;      /* name of the file in debugfs to set the value of the property */
} mot_battery_property;


/* Battery properties.  All the properties from the mot_battery_props[] must be listed here.
   The order or the properties can be arbitrary */
mot_battery_property mot_battery_data[MOT_BATTERY_NUMBER_OF_PROPERTIES] = {
   { POWER_SUPPLY_PROP_STATUS,      MOT_BATTERY_STATUS_INIT_VAL,     false, NULL, "status"      },   
   { POWER_SUPPLY_PROP_HEALTH,      MOT_BATTERY_HEALTH_INIT_VAL,     true,  NULL, "health"      },   
   { POWER_SUPPLY_PROP_PRESENT,     MOT_BATTERY_PRESENT_INIT_VAL,    false, NULL, "present"     },   
   { POWER_SUPPLY_PROP_TECHNOLOGY,  MOT_BATTERY_TECHNOLOGY_INIT_VAL, true,  NULL, "technology"  },  
   { POWER_SUPPLY_PROP_VOLTAGE_NOW, MOT_BATTERY_VOLTAGE_INIT_VAL,    false, NULL, "voltage"     },  
   { POWER_SUPPLY_PROP_CAPACITY,    MOT_BATTERY_CAPACITY_INIT_VAL,   false, NULL, "capacity"    },  
   { POWER_SUPPLY_PROP_TEMP,        MOT_BATTERY_TEMP_INIT_VAL,       false, NULL, "temperature" }    
};
...
static int psp_to_index(enum power_supply_property psp)
{
   ...
   for (i = 0; i < MOT_BATTERY_NUMBER_OF_PROPERTIES && mot_battery_data[i].psp != psp; i++);
...
{
   int  *status   = &mot_battery_data[psp_to_index(POWER_SUPPLY_PROP_STATUS)].val;
   int  *capacity = &mot_battery_data[psp_to_index(POWER_SUPPLY_PROP_CAPACITY)].val;
   bool  changed  = true;
   
   /* Stay FULL until DISCHARGING starts.  Become FULL if CHARGING and CAPACITY is 100% */
   
   if (new_status == POWER_SUPPLY_STATUS_CHARGING && *status != POWER_SUPPLY_STATUS_FULL) {
      *status = (*capacity == 100 ? POWER_SUPPLY_STATUS_FULL : POWER_SUPPLY_STATUS_CHARGING);
   } else if (new_status != POWER_SUPPLY_STATUS_CHARGING ) {
      *status = new_status;
   } else { /* new_status == POWER_SUPPLY_STATUS_CHARGING && *status == POWER_SUPPLY_STATUS_FULL */
      changed = false;
   }
...
static bool update_capacity(int new_capacity)
{
   ...
   /* Become FULL if CHARGING and CAPACITY is 100%.  Latch 100% CAPACITY while FULL */

   if (new_capacity == 0) {
      /* Ensure that phone will perform a low battery shutdown */
      *status   = POWER_SUPPLY_STATUS_DISCHARGING;
   } else if (new_capacity == 100 && *status == POWER_SUPPLY_STATUS_CHARGING) {
      *status   = POWER_SUPPLY_STATUS_FULL;
   } else if (new_capacity < 100 && *status == POWER_SUPPLY_STATUS_FULL) {
      changed   = false; /* stay at full capacity after charging was complete while charger is present */
   }
...
/* sysfs */
static int mot_battery_get_property(struct power_supply *psy, enum power_supply_property psp, union power_supply_propval *val)
{     
   int                   ret = 0;
   ...
   if (i != MOT_BATTERY_NUMBER_OF_PROPERTIES) {
      ...
      if (!mot_battery_data[i].no_update) {
         ...
         if (get && get(&data) && sanity_check(psp, data)) {
            ...
            /* Prevent charging when battery temperature cannot be obtained */            
            pr_err("prevening battery charging since battery temp cannot be obtained\n");
...
static bool sanity_check(enum power_supply_property psp, int val)
{
   ...
   switch(psp) {   
      case POWER_SUPPLY_PROP_VOLTAGE_NOW:
         ret = (val >= MOT_BATTERY_VOLTAGE_MIN  && val <= MOT_BATTERY_VOLTAGE_MAX);
         break;
      case POWER_SUPPLY_PROP_CAPACITY:
         ret = (val >= MOT_BATTERY_CAPACITY_MIN && val <= MOT_BATTERY_CAPACITY_MAX);
         break;
      case POWER_SUPPLY_PROP_TEMP:
         ret = (val >= MOT_BATTERY_TEMP_MIN     && val <= MOT_BATTERY_TEMP_MAX);
         break;
      default:
         ret = true;
         break;
...
{     
   int  i       = psp_to_index(psp);     
   bool changed = (i != MOT_BATTERY_NUMBER_OF_PROPERTIES && !mot_battery_data[i].no_update && sanity_check(psp, val) && set_property(i, val)); 
...
/* 
   Debugfs interface to test how system works with different values of the battery properties 
   Once the propety value is set through the debugfs, updtes from the drivers will be discarded 
*/
...
static int mot_battery_debug_set(void *data, u64 val)
{
   ...
   mot_battery_data[i].no_update = true; /* set the flag to prevent updates other than through debugfs */
...
DEFINE_SIMPLE_ATTRIBUTE(mot_battery_fops, mot_battery_debug_get, mot_battery_debug_set, "%llu\n");
...
static int __init mot_battery_debug_init(void)
{
   ...
   if (!IS_ERR(dent)) {
      for (i = 0; i < MOT_BATTERY_NUMBER_OF_PROPERTIES; i++) {
         debugfs_create_file(mot_battery_data[i].name, 0666, dent, (void *) i, &mot_battery_fops);
      }
   } else {
...
{
   /* Send uevent.  No need to use power_supply_changed() because LED's are not used
...
static int __init mot_battery_init(void)
{
   ...
   if (!batt_info.motorola_battery) {   
      ...
      mot_battery_data[status].no_update = true; /* to discard updates from a driver */     
      
      mot_battery_data[health].val       = POWER_SUPPLY_STATUS_UNKNOWN;      
      mot_battery_data[health].no_update = true; /* to discard updates from a driver */       
   }
...
fs_initcall(mot_battery_init); /* need to be called before USB driver and after power supply driver */
