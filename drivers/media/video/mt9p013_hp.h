/*
 * drivers/media/video/mt9p013.h
 *
 * Register definitions for the MT9P013 camera sensor.
 *
 * Author:
...
#define CONFIG_MT9P013_I2C_MIPI_BASE_ADDR

#if defined(CONFIG_MT9P013_I2C_MIPI_BASE_ADDR)
    #define MT9P013_I2C_ADDR		0x36  /* MIPI */
#else
    #define MT9P013_I2C_ADDR		0x10  /* SMIA */
#endif
...
#define MT9P013_MFR_ID		0x0006

/* MT9P013 has 8/16/32 registers */
#define I2C_MT9P013_8BIT		1
#define I2C_MT9P013_16BIT		2
#define I2C_MT9P013_32BIT		4

/* terminating token for reg list */
#define MT9P013_TOK_TERM		0xFF

/* delay token for reg list */
#define MT9P013_TOK_DELAY		100

/* Sensor specific GPIO signals */
#define MT9P013_RESET_GPIO  	98
#define MT9P013_STANDBY_GPIO	58
...
#define MT9P013_REG_TERM		0xFF
/* terminating list entry for val */
#define MT9P013_VAL_TERM		0xFF

#define MT9P013_CLKRC			0x11
...
/*
 * The nominal xclk input frequency of the MT9P013 is 12MHz, maximum
 * frequency is 64MHz, and minimum frequency is 2MHz.
 */
#define MT9P013_XCLK_MIN   2000000
#define MT9P013_XCLK_MAX   64000000
#define MT9P013_XCLK_NOM_1 12000000
#define MT9P013_XCLK_NOM_2 24000000

#define MT9P013_USE_XCLKA	0
#define MT9P013_USE_XCLKB	1


/* FPS Capabilities */
#define MT9P013_MIN_FPS		11
#define MT9P013_DEF_FPS		15
#define MT9P013_MAX_FPS		30
...
/* Analog gain values */
#define MT9P013_MIN_ANALOG_GAIN	0x002D
#define MT9P013_MAX_ANALOG_GAIN	0x01FF

#define MT9P013_MIN_LINEAR_GAIN	((u16)(1.4 * 256))
#define MT9P013_MAX_LINEAR_GAIN	((u16)(15.875 * 256))

#define MT9P013_MAX_FRAME_LENGTH_LINES 0xFFFF
...
/**
 * struct mt9p013_reg - mt9p013 register format
 * @length: length of the register
 * @reg: 16-bit offset to register
 * @val: 8/16/32-bit register value
 *
 * Define a structure for MT9P013 register initialization values
 */
struct mt9p013_reg {
	u16	length;
	u16	reg;
...
enum mt9p013_frame_type {
	MT9P013_FRAME_5MP_10FPS = 0,
	MT9P013_FRAME_3MP_10FPS,
	MT9P013_FRAME_1296_30FPS,
	MT9P013_FRAME_648_30FPS,
	MT9P013_FRAME_648_120FPS,
	MT9P013_FRAME_216_30FPS
};

enum mt9p013_pixel_format {
	MT9P013_RAWBAYER10
};

enum mt9p013_orientation {
	MT9P013_NO_HORZ_FLIP_OR_VERT_FLIP = 0,
	MT9P013_HORZ_FLIP_ONLY,
	MT9P013_VERT_FLIP_ONLY,
	MT9P013_HORZ_FLIP_AND_VERT_FLIP
};
 ...
 */
struct mt9p013_capture_size {
	unsigned long width;
	unsigned long height;
};

/**
 * struct mt9p013_platform_data - platform data values and access functions
...
struct mt9p013_platform_data {
	...
	const struct mt9p013_reg *default_regs;
 ...
 */
struct mt9p013_exposure_settings {
	u16	coarse_int_tm;
	u16	fine_int_tm;
	u16	fine_correction;
	u16	analog_gain;
...
/**
 * struct struct mt9p013_sensor_settings - struct for storage of
 * sensor settings.
 */
struct mt9p013_sensor_settings {
	struct mt9p013_clk_settings clk;
	struct mt9p013_frame_settings frame;
	struct mt9p013_exposure_settings exposure;
};

/**
 * struct struct mt9p013_clock_freq - struct for storage of sensor
 * clock frequencies
 */
struct mt9p013_clock_freq {
	u32 x_clk;
	u32 pll_clk;
	u32 vt_pix_clk;
	u32 op_pix_clk;
...
/*
 * Array of image sizes supported by MT9P013.  These must be ordered from
 * smallest image size to largest.
 */
const static struct mt9p013_capture_size mt9p013_sizes[] = {
