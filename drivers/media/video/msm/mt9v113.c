				  ...
				  unsigned char *txdata, int length)
{
	...
	if (i2c_transfer(mt9v113_client->adapter, msg, 1) < 0) {
		pr_info("mt9v113_i2c_txdata failed\n");
		return -EIO;
	}
#endif
			      ...
			      unsigned char *rxdata, int length)
{
	...
	if (i2c_transfer(mt9v113_client->adapter, msgs, 2) < 0) {
		pr_info("mt9v113_i2c_rxdata failed!\n");
		return -EIO;
	}
#endif
...
static inline int resume(void)
{
	...
	/* write 0x30E0 to 0x0001 */
	rc = mt9v113_i2c_write(mt9v113_client->addr, 0x30E0, 0x0001,
	WORD_LEN);
	if (rc < 0) {
		pr_err("%s: Enter Active mode fail\n", __func__);
		return rc;
...
#if 0
static int mt9v113_pwd(const struct msm_camera_sensor_info *info)
{
	int rc = 0;
...
static int mt9v113_reg_init(void)
{
	...
	for (k = 0; k < CHECK_STATE_TIME; k++) {  /* retry 100 times */
		mt9v113_i2c_read_w(mt9v113_client->addr,
			      0x0014, &check_value);
		if ((check_value & 0x8000)) /* check state of 0x0014 */
			break;
		msleep(1);
	}
	if (k == CHECK_STATE_TIME)
	...
	rc = mt9v113_i2c_read_w(mt9v113_client->addr, 0x3210, &check_value);
	if (rc < 0)
	  return rc;

	pr_info("%s: mt9v113: 0x3210 reg value = 0x%x\n", __func__,
		check_value);
	...
	rc = mt9v113_i2c_write(mt9v113_client->addr, 0x3210, check_value,
		WORD_LEN);
	if (rc < 0) {
		pr_err("%s: Enter Active mode fail\n", __func__);
		return rc;
...
#if 0
static int mt9v113_init_done(const struct msm_camera_sensor_info *info)
{
	int rc = 0;
#if 0 /* un-use now */
	rc = gpio_request(info->sensor_pwd, "mt9v113");
...
static int mt9v113_set_sensor_mode(int mode)
{
	...
	case SENSOR_GET_EXP:
		pr_err("%s: SENSOR_GET_EXP not support\n", __func__);
		return 0;
#if 0
...
static int mt9v113_set_front_camera_mode(enum frontcam_t frontcam_value)
{
	...
	rc = mt9v113_i2c_write(mt9v113_client->addr, 0x098C, 0x2103, WORD_LEN);
	rc = mt9v113_i2c_write(mt9v113_client->addr, 0x0990, 0x0006, WORD_LEN);
		if (rc < 0)
			return -EIO;
	}
...
int mt9v113_sensor_open_init(struct msm_camera_sensor_info *data)
{
	...
	pr_info("mt9v113: mt9v113_sensor_open_init: switch clk\n");
	...
	if (rc < 0) {
		pr_info("mt9v113_sensor_init failed!\n");
		goto init_fail;
	}
	...
	rc = mt9v113_reg_init();
	if (rc < 0) {
		pr_info("mt9v113_sensor_reg_init failed!\n");
		goto init_fail;
	}
#else
...
int mt9v113_sensor_release(void)
{
	...
	rc = gpio_request(mt9v113_ctrl->sensordata->sensor_pwd, "mt9v113");
	if (!rc)
		gpio_direction_output(mt9v113_ctrl->sensordata->sensor_pwd, 1);
	else
		pr_info("GPIO(%d) request faile",
			mt9v113_ctrl->sensordata->sensor_pwd);
	gpio_free(mt9v113_ctrl->sensordata->sensor_pwd);
...
static int mt9v113_sysfs_init(void)
{
	...
	if (ret) {
		pr_info("mt9v113_sysfs_init: sysfs_create_file htcwc failed\n");
		kobject_del(android_mt9v113);
	}

	ret = sysfs_create_file(android_mt9v113, &dev_attr_node.attr);
	if (ret) {
		pr_info("mt9v113_sysfs_init: dev_attr_node failed\n");
		kobject_del(android_mt9v113);
	}
				...
				struct msm_sensor_ctrl *s)
{
	...
	pr_info("mt9v113 s->node %d\n", s->node);
	sensor_probe_node = s->node;

	/*switch clk source*/
	pr_info("mt9v113: mt9v113_sensor_probe switch clk\n");
	...
	if (!rc) {
		...
		pr_err("GPIO(%d) request faile", info->sensor_reset);
	...
	msleep(10);
	/*
	pr_info("JACKY: %s: mt9v113_init_done()\n", __func__);
	mt9v113_init_done(info);
	*/
	pr_info("JACKY: %s: mt9v113_sysfs_init()\n", __func__);
	mt9v113_sysfs_init();
