		...
		void (*getevent)(void *ptr, size_t len))
{
	...
	CDBG("%s:VFE_ADSP event:msg_id=%d\n", __func__, rp->evt_msg.msg_id);
...
static int vfe_7x_enable(struct camera_enable_cmd *enable)
{
	...
	if (!strcmp(enable->name, "QCAMTASK")) {
		printk(KERN_ERR "enabling QCAMTASK module with id =%d\n", qcam_mod->id);
		rc = msm_adsp_enable(qcam_mod);
		printk(KERN_ERR "QCAMTASK enable rc =%d\n", rc);
	} else if (!strcmp(enable->name, "VFETASK")) {
		printk(KERN_ERR "enabling VFETATSK module with id =%d\n", vfe_mod->id);
		rc = msm_adsp_enable(vfe_mod);
		printk(KERN_ERR "VFETASK enable rc =%d\n", rc);
	}
