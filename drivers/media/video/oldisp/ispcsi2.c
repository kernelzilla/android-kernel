...
int isp_csi2_complexio_power(enum isp_csi2_power_cmds power_cmd)
{
	...
	do {
		...
		/* FIXME: Powerstate for first time is failing, although it
		 * works anyway... Strange. So overriding this by now... */
		power_cmd = current_state;
...
/* FIXME: I'm receiving these errors on my Zoom board only. Perhaps a HW issue?
 * Disabling it by now...
 */
/*
...
int isp_csi2_reset(void)
{
	...
	if (cpu_is_omap3630()) {
		reg =  omap_readl(ISPCSI2_COMPLEXIO_CFG1);
		reg |= ISPCSI2_COMPLEXIO_CFG1_RESET_CTRL_DEASSERTED;
		omap_writel(reg, ISPCSI2_COMPLEXIO_CFG1);
	}
    ...
    if (i == 0) {
		printk(KERN_ERR "CSI2: Reset for CSI2_96M_FCLK domain failed!\n");
		return -EBUSY;
	}
