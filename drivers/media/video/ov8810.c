...
static struct ov8810_sensor_settings sensor_settings[] = {
	...
	*/
	...
	/* 320 x 240 - 120fps */
	{
		...
		.frame = {
			.frame_len_lines_min = 284,
			.line_len_pck_min = 2016,
			.x_addr_start = 320,
			.x_addr_end = 2975,
			.y_addr_start = 224,
			.y_addr_end = 2239,
	...
	/* 408 x 306 - 116fps */
		/* 2-lane, RAW 10, PCLK = 97.2MHz, MIPI_CLK = 74.3MHz,
							...
							struct vcontrol *lvc)
{
	...
	if (linear_gain_Q8 < sensor->exposure.min_linear_gain) {
		DPRINTK_OV8810("Gain %d less than regal limit %d\n",
	...
	if (linear_gain_Q8 > sensor->exposure.max_linear_gain) {
		DPRINTK_OV8810("Gain %d greater than regal limit %d\n",
			...
			struct v4l2_fract *fper, enum image_size_ov isize)
{
	...
	line_len_pck = (line_time_q8 * ((sensor->freq.pclk * skip_factor) /
		10000)) / 100;
...
static int ov8810_configure(struct v4l2_int_device *s)
{
    ...
    /* set internal core voltage to 1.63V */
    ov8810_write_reg(client, 0x30A8, 0x2F);
...
const struct v4l2_fract ov8810_frameintervals[] = {
	...
	{ .numerator = 1, .denominator = 30 },  /* 13 - SIZE_1_5M &
							SIZE_500K max fps*/
	{ .numerator = 1, .denominator = 120 },  /* 14 - SIZE_125K max fps */
};
