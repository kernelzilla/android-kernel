...
out:
	...
	// printk(KERN_INFO "BLUETOOTH: msm_serial_hs [grabbing a 500ms wakelock]\n");
...
static irqreturn_t msm_hs_isr(int irq, void *dev)
{
	...
	if (isr_status & UARTDM_ISR_RXLEV_BMSK) {
	  // printk(KERN_INFO "BLUETOOTH: msm_serial_hs [grabbing DMA wakelock]\n");
...
static int msm_hs_startup(struct uart_port *uport)
{
	...
	printk(KERN_INFO "BLUETOOTH: msm_serial_hs startup\n");
	...
	ret = request_irq(uport->irq, msm_hs_isr, IRQF_TRIGGER_HIGH,"msm_hs_uart", msm_uport);
	if (unlikely(ret))
		return ret;
	if (use_low_power_wakeup(msm_uport)) {
...
static void msm_hs_shutdown(struct uart_port *uport)
{
	...
	printk(KERN_INFO "BLUETOOTH: msm_serial_hs shutdown\n");
