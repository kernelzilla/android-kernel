...
int iIpcW;
int iLogR;
unsigned long aIpcR[IPC_DBG_ARRAY_SIZE];
unsigned long aIpcW[IPC_DBG_ARRAY_SIZE];
unsigned long aLogR[IPC_DBG_ARRAY_SIZE];
...
void usb_ipc_data_disconnect(struct usb_interface *intf)
{
	...
	unsigned int dma_ccr_r, dma_clnk_ctrl_r, dma_cicr_r, dma_csr_r;
	unsigned int dma_csdp_r, dma_cen_r, dma_cfn_r, dma_cssa_r, dma_cdsa_r;
	unsigned int dma_csei_r, dma_csfi_r, dma_cdei_r, dma_cdfi_r, dma_csac_r;
	unsigned int dma_cdac_r, dma_ccen_r, dma_ccfn_r, dma_color_r;
	unsigned int dma_ccr_w, dma_clnk_ctrl_w, dma_cicr_w, dma_csr_w;
	unsigned int dma_csdp_w, dma_cen_w, dma_cfn_w, dma_cssa_w, dma_cdsa_w;
	unsigned int dma_csei_w, dma_csfi_w, dma_cdei_w, dma_cdfi_w, dma_csac_w;
	unsigned int dma_cdac_w, dma_ccen_w, dma_ccfn_w, dma_color_w;
  ...
  usb_unlink_urb (&usb_ipc_data_param.write_urb);
	...
	dma_ccr_r = omap_readl(0x48056080 + 0x60 * dma_ch);
	dma_clnk_ctrl_r = omap_readl(0x48056084 + 0x60 * dma_ch);
	dma_cicr_r = omap_readl(0x48056088 + 0x60 * dma_ch);
	dma_csr_r = omap_readl(0x4805608c + 0x60 * dma_ch);
	dma_csdp_r = omap_readl(0x48056090 + 0x60 * dma_ch);
	dma_cen_r = omap_readl(0x48056094 + 0x60 * dma_ch);
	dma_cfn_r = omap_readl(0x48056098 + 0x60 * dma_ch);
	dma_cssa_r = omap_readl(0x4805609c + 0x60 * dma_ch);
	dma_cdsa_r = omap_readl(0x480560a0 + 0x60 * dma_ch);
	dma_csei_r = omap_readl(0x480560a4 + 0x60 * dma_ch);
	dma_csfi_r = omap_readl(0x480560a8 + 0x60 * dma_ch);
	dma_cdei_r = omap_readl(0x480560ac + 0x60 * dma_ch);
	dma_cdfi_r = omap_readl(0x480560b0 + 0x60 * dma_ch);
	dma_csac_r = omap_readl(0x480560b4 + 0x60 * dma_ch);
	dma_cdac_r = omap_readl(0x480560b8 + 0x60 * dma_ch);
	dma_ccen_r = omap_readl(0x480560bc + 0x60 * dma_ch);
	dma_ccfn_r = omap_readl(0x480560c0 + 0x60 * dma_ch);
	dma_color_r = omap_readl(0x480560c4 + 0x60 * dma_ch);

	dma_ch = (unsigned int)ipc_memcpy_node2buf.dma_ch;
	dma_ccr_w = omap_readl(0x48056080 + 0x60 * dma_ch);
	dma_clnk_ctrl_w = omap_readl(0x48056084 + 0x60 * dma_ch);
	dma_cicr_w = omap_readl(0x48056088 + 0x60 * dma_ch);
	dma_csr_w = omap_readl(0x4805608c + 0x60 * dma_ch);
	dma_csdp_w = omap_readl(0x48056090 + 0x60 * dma_ch);
	dma_cen_w = omap_readl(0x48056094 + 0x60 * dma_ch);
	dma_cfn_w = omap_readl(0x48056098 + 0x60 * dma_ch);
	dma_cssa_w = omap_readl(0x4805609c + 0x60 * dma_ch);
	dma_cdsa_w = omap_readl(0x480560a0 + 0x60 * dma_ch);
	dma_csei_w = omap_readl(0x480560a4 + 0x60 * dma_ch);
	dma_csfi_w = omap_readl(0x480560a8 + 0x60 * dma_ch);
	dma_cdei_w = omap_readl(0x480560ac + 0x60 * dma_ch);
	dma_cdfi_w = omap_readl(0x480560b0 + 0x60 * dma_ch);
	dma_csac_w = omap_readl(0x480560b4 + 0x60 * dma_ch);
	dma_cdac_w = omap_readl(0x480560b8 + 0x60 * dma_ch);
	dma_ccen_w = omap_readl(0x480560bc + 0x60 * dma_ch);
	dma_ccfn_w = omap_readl(0x480560c0 + 0x60 * dma_ch);
	dma_color_w = omap_readl(0x480560c4 + 0x60 * dma_ch);
	...
	for (i = 0; i < IPC_DBG_ARRAY_SIZE; i++) {
		printk(KERN_INFO "\t0x%08x\t0x%08x\t0x%08x\n",
			(unsigned int)aIpcR[iIpcR++],
			(unsigned int)aLogR[iLogR++],
			(unsigned int)aIpcW[iIpcW++]);
		if (iIpcR >= IPC_DBG_ARRAY_SIZE)
			iIpcR = 0;
		if (iLogR >= IPC_DBG_ARRAY_SIZE)
			iLogR = 0;
		if (iIpcW >= IPC_DBG_ARRAY_SIZE)
			iIpcW = 0;
	}
	...
	printk(KERN_INFO "\nDMA IPC-READ Channel %d Register Dump:\n"
		...
		ipc_memcpy_buf2node.dma_ch,
		dma_ccr_r, dma_clnk_ctrl_r, dma_cicr_r, dma_csr_r, dma_csdp_r,
		dma_cen_r, dma_cfn_r, dma_cssa_r, dma_cdsa_r, dma_csei_r,
		dma_csfi_r, dma_cdei_r, dma_cdfi_r, dma_csac_r, dma_cdac_r,
		dma_ccen_r, dma_ccfn_r, dma_color_r);
	printk(KERN_INFO "\nDMA IPC-WRITE Channel %d Register Dump:\n"
		"dma_ccr = 0x%08x \tdma_clnk_ctrl = 0x%08x\n"
		"dma_cicr = 0x%08x\tdma_csr = 0x%08x\n"
		"dma_csdp = 0x%08x\tdma_cen = 0x%08x\n"
		"dma_cfn = 0x%08x \tdma_cssa = 0x%08x\n"
		...
		"dma_ccfn = 0x%08x\tdma_color = 0x%08x\n",
		ipc_memcpy_node2buf.dma_ch,
		dma_ccr_w, dma_clnk_ctrl_w, dma_cicr_w, dma_csr_w, dma_csdp_w,
		dma_cen_w, dma_cfn_w, dma_cssa_w, dma_cdsa_w, dma_csei_w,
		dma_csfi_w, dma_cdei_w, dma_cdfi_w, dma_csac_w, dma_cdac_w,
		dma_ccen_w, dma_ccfn_w, dma_color_w);
	printk(KERN_INFO "\nINTC Status and Mask register Dump:\n"
...
static int __init usb_ipc_init(void)
{
  int result;
	...
	iIpcW = 0;
	iLogR = 0;
	memset((void *)aIpcR, 0xff, IPC_DBG_ARRAY_SIZE * sizeof(unsigned long));
	memset((void *)aIpcW, 0xff, IPC_DBG_ARRAY_SIZE * sizeof(unsigned long));
	memset((void *)aLogR, 0xff, IPC_DBG_ARRAY_SIZE * sizeof(unsigned long));
#endif /* CONFIG_IPC_USBHOST_DBG */
