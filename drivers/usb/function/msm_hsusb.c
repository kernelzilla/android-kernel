...
static unsigned charger_attached_at_boot = 0;
...
static int usb_suspend_phy(struct usb_info *ui)
{
	...
	if (!(readl(USB_PORTSC) & PORTSC_PHCD)) {
...
#ifdef CONFIG_MACH_MOT
		/* only reset in the case when we are not online and suspended */
		if(ui->usb_state != USB_STATE_SUSPENDED) {
			...
			pr_err("since USB state is suspended, do not reset usb hw\n");
		}
#endif
		return -1;
	}
			...
			if ((flags & USB_FLAG_START) ||
				...
				if (usb_vbus_is_on(ui)) {
...
#ifdef CONFIG_MACH_MOT
					/* we rely on interrupts from the host when doing charger
					 * charger detection, to protect against slow insertions */
					enable_irq(ui->irq);
#endif
					usb_chg_set_type(ui);
#ifdef CONFIG_POWER_SUPPLY
					...
					if (ui->chg_type == CHG_WALL) {
						...
						msleep(500);
#ifndef CONFIG_MACH_MOT							
						// FIXME - workaround for battery temp monitoring during charging
						usb_lpm_enter(ui);
#endif						
					}
		...
		case USB_STATE_ONLINE:
			...
			if ((flags & USB_FLAG_RESUME) ||
              ...
              (void) set_battery_property(POWER_SUPPLY_PROP_STATUS, POWER_SUPPLY_STATUS_CHARGING);       
					...
					ui->usb_state = USB_STATE_DEFAULT;
#ifdef CONFIG_MACH_MOT
					/* FIXME - only charge after full enumeration */
					/* workaround for 100mA then 500mA charging bug */
...
#ifndef CONFIG_MACH_MOT
#else
	writel((readl(USB_OTGSC) & ~(OTGSC_IDIS | OTGSC_BSVIS)) | OTGSC_BSVIE, USB_OTGSC);
#endif 
	if (readl(USB_PORTSC) & PORTSC_PHCD) {
		disable_irq(ui->irq);
#ifndef CONFIG_MACH_MOT
...
no_gpios:
	...
	if(power_supply_register(&pdev->dev, ui->psy_usb))
		printk(KERN_ERR "%s error in power_supply_register for psy_usb\n", __func__);
	...
	if(power_supply_register(&pdev->dev, ui->psy_ac))
		printk(KERN_ERR "%s error in power_supply_register for psy_ac\n", __func__);
#endif
	usb_debugfs_init(ui);
			...
			union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
		...
		if(psy->type == POWER_SUPPLY_TYPE_USB)
			...
			printk("%s called for unknown type\n", __func__);
			ret = -EINVAL;
		}
		spin_unlock_irqrestore(&ui->lock, flags);
		break;
