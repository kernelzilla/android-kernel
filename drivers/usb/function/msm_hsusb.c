...
#if defined(CONFIG_KERNEL_MOTOROLA)
#define CDROM_EP0_SWITCH_DELAY	(msecs_to_jiffies(4000)) /* 4-sec delay */
#endif /* defined(CONFIG_KERNEL_MOTOROLA) */
...
static int adb_enabled;
static int adb_checked;
...
static unsigned char str_ms_os_desc[] = {18,
				USB_DT_STRING,
				/* Signature field (14 bytes) */
				0x4D, 0, 0x53, 0, 0x46, 0, 0x54, 0,
				0x31, 0, 0x30, 0, 0x30, 0,
				0xFE,		/* Vendor code */
				0x00};		/* Pad field */
#endif
...
{
   unsigned short mot_pid = MOT_PID1;

   switch(wIndex)
   {
      case MOT_USB_CONFIG_34:
#if defined(CONFIG_MACH_SOCIAL)
      case MOT_USB_CONFIG_39:
#endif /* defined(CONFIG_MACH_SOCIAL) */
      mot_pid = MOT_PID1;
      break;
      ...
      queue_delayed_work(usb_work, &ui->work, CDROM_EP0_SWITCH_DELAY);
}
#endif /* defined(CONFIG_KERNEL_MOTOROLA) */
...
#if defined(CONFIG_KERNEL_MOTOROLA)
        /* If desired mode is composite/Phone tools mode , prepare
           HW to enumerate in CDROM. After receiving a switch from
           Motorola PC driver, phone will re-enumerate in composite */
        if ( ui->flags != USB_FLAG_SWITCH ){
           if (pid == MOT_PID1)
                pid = MOT_PID8;
        }
...
void usb_function_enable(const char *function, int enable)
{
	...
	if (!strcmp(function, "adb"))
		adb_enabled = enable;
	if (!strcmp(function, "mtp")) {
		if (functions_mask & (1<<usb_get_function_index("adb"))) {
			if (enable) {
				/* mtp + ADB + Diag */
				usb_switch_composition(MOT_PID6);
			} else {
				/* composite w/ ADB */
				usb_switch_composition(MOT_PID);
			}
		} else {
			if (enable)
				usb_switch_composition(MOT_PID3); /*mtp only*/
			else
				usb_switch_composition(MOT_PID1); /*composite*/
		}
		return;
	}
#endif /* defined(CONFIG_MACH_SOCIAL) */
	...
	else
	{
		...
		if((functions_mask & ui->composition->functions) != ui->composition->functions) {
			printk("default pid not ready\n");
			return;
		}
...
#if defined(CONFIG_MACH_SOCIAL)
static int is_adb_enable()
{
#define   BUFF_SIZE   128

	char   buff[BUFF_SIZE];
	struct   file   *fp;
	if ((0 == adb_checked) && (0 == adb_enabled)) {
		fp = filp_open("//default.prop", O_RDONLY, 0);
		if (IS_ERR(fp)) {
			printk(KERN_INFO "Read default.prop  File   Failed.");
			return   adb_enabled;
		} else {
		...
		if (NULL == strstr(buff, "persist.service.adb.enable=1")) {
			adb_enabled = 0;
			printk(KERN_INFO "adb disabled in default.prop\n");
		} else {
			adb_enabled = 1;
			printk(KERN_INFO "adb enabled in default.prop\n");
		}
	}
	return adb_enabled;
}
	...
	for (;;) {
		...
		case USB_STATE_ONLINE:
...
#if defined(CONFIG_KERNEL_MOTOROLA)
#if defined(CONFIG_MACH_SOCIAL)
				/*If phone is not in an ADB-enabled mode
				  before disconnection, prepare subsequent
				  enumeration in CDROM mode. */
				if (!(ui->composition->functions &
					(1<<usb_get_function_index("adb")))) {
					usb_uninit(ui);
					usb_set_composition(MOT_PID8);
					printk(KERN_INFO "call usb_set_composition at %s \n",
						__LINE__);
					usb_configure_device_descriptor(ui);
				}
#else
				 /*If phone is in composite mode
					before disconnection, prepare HW for
					subsequent enumeration in CDROM mode. */
				if (pid == MOT_PID1) {
					usb_uninit(ui);
					usb_set_composition(MOT_PID8);
					usb_configure_device_descriptor(ui);
			...
			if (flags & USB_FLAG_SWITCH) {
				/*Retain SWITCH flag until it is reset */
				/* if adb is enable don't do switch */
				if (is_adb_enable() == 0) {
					ui->flags = USB_FLAG_SWITCH;
					usb_switch_composition(pid);
				} else {
...
#define	USB_REMOTE_WAKEUP_SUPPORT	1
	...
	if (type == USB_DT_STRING) {
		...
		buffer[0] = 0;
#if defined(CONFIG_KERNEL_MOTOROLA)
		if ((id == STRING_MS_OS_DESCRIPTOR) &&
			(ui->composition->functions &
			1<<usb_get_function_index("mtp"))) {
			/*
			 * Explicitly handle MS OS Descriptor
       * requests for compositions with MTP function
      */
			memcpy(buffer, str_ms_os_desc, str_ms_os_desc[0]);
			req->length = buffer[0];
			return 0;
		}
#endif
