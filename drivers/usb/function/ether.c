...
static int ether_configured=0;
static void ether_blan_send_uevent (int config);
...
#if defined(CONFIG_KERNEL_MOTOROLA)
        /* Send connect/disconnect uevents*/
        ether_blan_send_uevent(ether_configured);
#endif /* defined(CONFIG_KERNEL_MOTOROLA) */
}
...
static void ether_unbind(void *_ctxt)
{
	...
	spin_lock_irqsave(&unbind_lock, flags);
	if(ctxt->in) {
	...
	spin_unlock_irqrestore(&unbind_lock, flags);
...
static void ether_configure(int configured, void *_ctxt)
{
	...
	ether_configured=configured;
	pr_info("ether_configured %d\n", ether_configured);
	...
	if (configured) {
		...
		eifc.router_ip = 0x0;
                ...
                /*Update 'usb0' device IP params. (NGP-L requirement)*/
               eifc.ether_config_dev = ctxt->dev;
               schedule_work(&eifc.ether_config_wq);
#endif /* defined(CONFIG_KERNEL_MOTOROLA) */
	}
...
static void ether_blan_send_uevent (int config)
{
    ...
    sprintf (event_string, "USB_CONNECT=%d", config);
    ret = kobject_uevent_env (&blan_enable_device.this_device->kobj,
        KOBJ_CHANGE, envp);
    if (ret) {
        printk (KERN_ERR "%s: kobject_uevent_env failed: %d\n",
            __FUNCTION__, ret);
    } else {
        printk (KERN_INFO "%s: kobject_uevent_env %s success\n",
