...
static int omap_ehci_bus_suspend(struct usb_hcd *hcd)
{
	...
	if (!ehci_clocks->suspended) {
		...
		if (res == 0) {
			...
			if ((omap_readl(OMAP_UHH_DEBUG_CSR) &
				(1 << OMAP_UHH_DEBUG_CSR_OHCI_GLOBALSUSPEND)))
				omap_readl(OHCI_HC_CONTROL);
			if ((omap_readl(OMAP_UHH_DEBUG_CSR) &
				(1 << OMAP_UHH_DEBUG_CSR_OHCI_GLOBALSUSPEND)))
				printk(KERN_ERR
					"ehci-omap: ohci resume failed!\n");
			ehci_bus_resume(hcd);
			ret = -EBUSY;
			goto end;
		}
		...
		if (status & INTR_MASK) {
			/* pending irq, resume */
			ret = -EBUSY;
			LOG_USBHOST_ACTIVITY(aUsbHostDbg, iUsbHostDbg, 0x4E);
			goto resume;
		}
...
resume:
	...
	if ((omap_readl(OMAP_UHH_DEBUG_CSR) &
			(1 << OMAP_UHH_DEBUG_CSR_OHCI_GLOBALSUSPEND)))
		printk(KERN_ERR "ehci-omap: ohci resume failed!\n");
	spin_unlock_irqrestore(&usbtll_clock_lock, flags);
	ehci_bus_resume(hcd);
