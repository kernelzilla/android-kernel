		...
		while ((descriptor = *descriptors++) != NULL) {
			...
			if (intf->bDescriptorType == USB_DT_INTERFACE) {
				...
				if (intf->bInterfaceClass == USB_CLASS_COMM &&
					...
					struct usb_cdc_union_desc *desc = (struct usb_cdc_union_desc *)
						*(descriptors+1);
					/* set ecm master and slave interface number */
					if (desc && desc->bDescriptorType == USB_DT_CS_INTERFACE &&
						desc->bDescriptorSubType == USB_CDC_UNION_TYPE) {
						desc->bMasterInterface0 = interfaceCount;
						desc->bSlaveInterface0 = interfaceCount+1;
					}
