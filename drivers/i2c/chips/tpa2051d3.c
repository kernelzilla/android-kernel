...
err:
	...
	case TPA2051_SET_MODE:
		...
		if (modeid > tpa2051_mode_cnt || modeid <= 0) {
                ...
                if (*(config_data + modeid * MODE_CMD_LEM + 1)
						== SPKR_OUTPUT)
                        memcpy(SPK_AMP_ON, config_data + modeid *
			MODE_CMD_LEM + 2, sizeof(SPK_AMP_ON));
                else if (*(config_data + modeid * MODE_CMD_LEM + 1)
						== HEADSET_OUTPUT)
                        memcpy(HEADSET_AMP_ON, config_data + modeid *
				MODE_CMD_LEM + 2, sizeof(HEADSET_AMP_ON));
                else if (*(config_data + modeid * MODE_CMD_LEM + 1)
						== DUAL_OUTPUT)
                        memcpy(RING_AMP_ON, config_data + modeid *
				MODE_CMD_LEM + 2, sizeof(RING_AMP_ON));
                else
                        return -EINVAL;
