...
enum {
   ...
   MAX_NUM_TRIES                = 3       /* max number of times to read batt temp */
};
...
static bool bq27505_get_capacity(int *capacity)
{
   ...
   if (bq27505_low_battery_shutdown()) {   
      ...
      *capacity = 0;
      ...
      if (data == 0) {
         *capacity = 1; /* keep the phone alive until threshold voltage is reached */
      } else {
         *capacity = data;
      }      
   } else {
...
static bool bq27505_get_temp(int* temp)
{
   ...
   for (i = 0; i < MAX_NUM_TRIES && !ret; i++)  {
      ...
      if (!ret) {
         pr_err("fuel gauge, cannot get battery temp, attempt %d\n", (i+1));
      }
   }
...
{
   return i2c_smbus_write_word_data(bq27505_client,ENTER_ROM_MODE_REG, ENTER_ROM_MODE_DATA);
}
...
static void wait_init_complete(void)
{
   ...
   do {
      ...
      if (bq27505_standard_command(CONTROL, &data) && (data & 0x0080)) {
         complete = true;
      }      
...
static int bq27505_ioctl(struct inode *inode, struct file *f, unsigned int cmd, unsigned long arg)
{
                              ...
                              char *buf, size_t size)
{
   ...
   i2c_smbus_read_i2c_block_data(bq27505_client, BLOCK_DATA, sizeof(manuf_buf), manuf_buf);
...
static int bq27505_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
   bq27505_client = client;

   INIT_WORK(&work, bq27505_work_func);

   /* SOC_INT is asserted low for 2 msec.  Rising Edge interrupt to catch the end of the pulse */
   if (client->irq) {
      (void) request_irq(client->irq, bq7505_irq_handler, IRQF_TRIGGER_RISING, "bq27505", &work);
   }
...
static struct bq27505_debugfs_file debugfs_files[] = {
   ...
   { "control",                          { CONTROL,                         bq27505_standard_command } },                             
   { "at_rate",                          { AT_RATE,                         bq27505_standard_command } },                           
   { "at_rate_time_to_empty",            { AT_RATE_TIME_TO_EMPTY,           bq27505_standard_command } },  
   { "temperature",                      { TEMPERATURE,                     bq27505_standard_command } },   
   { "voltage",                          { VOLTAGE,                         bq27505_standard_command } },                             
   { "flags",                            { FLAGS,                           bq27505_standard_command } },   
   { "nominal_available_capacity",       { NOMINAL_AVAILABLE_CAPACITY,      bq27505_standard_command } },   
   { "full_available_capacity",          { FULL_AVAILABLE_CAPACITY,         bq27505_standard_command } },   
   { "remaining_capacity",               { REMAINING_CAPACITY,              bq27505_standard_command } },   
   { "full_charge_capacity",             { FULL_CHARGE_CAPACITY,            bq27505_standard_command } },   
   { "average_current",                  { AVERAGE_CURRENT,                 bq27505_standard_command } },  
   { "time_to_empty",                    { TIME_TO_EMPTY,                   bq27505_standard_command } },   
   { "time_to_full",                     { TIME_TO_FULL,                    bq27505_standard_command } },   
   { "standby_current",                  { STANDBY_CURRENT,                 bq27505_standard_command } },  
   { "standby_time_to_empty",            { STANDBY_TIME_TO_EMPTY,           bq27505_standard_command } },   
   { "max_load_current",                 { MAX_LOAD_CURRENT,                bq27505_standard_command } },  
   { "max_load_time_to_empty",           { MAX_LOAD_CURRENT,                bq27505_standard_command } },  
   { "available_energy",                 { AVAILABLE_ENERGY,                bq27505_standard_command } },   
   { "average_power",                    { AVERAGE_POWER,                   bq27505_standard_command } },   
   { "time_to_empty_at_constant_power",  { TIME_TO_EMPTY_AT_CONSTANT_POWER, bq27505_standard_command } },   
   { "state_of_health",                  { STATE_OF_HEALTH,                 bq27505_standard_command } },   
   { "state_of_charge",                  { STATE_OF_CHARGE,                 bq27505_standard_command } },   
   { "normalized_impedance",             { NORMALIZED_IMPEDANCE,            bq27505_standard_command } },   
   { "instant_current",                  { INSTANT_CURRENT,                 bq27505_standard_command } },   
   { "control_status",                   { CONTROL_STATUS,                  bq27505_control_subcommand } },                       
   { "device_type",                      { DEVICE_TYPE,                     bq27505_control_subcommand } }, 
   { "fw_version",                       { FW_VERSION,                      bq27505_control_subcommand } },     
   { "hw_version",                       { HW_VERSION,                      bq27505_control_subcommand } }, 
   { "data_flash_checksum",              { DATA_FLASH_CHECKSUM,             bq27505_control_subcommand } }, 
   { "write_temperature",                { WRITE_TEMPERATURE,               bq27505_control_subcommand } }, 
   { "prev_command",                     { PREV_COMMAND,                    bq27505_control_subcommand } },          
   { "chem_id",                          { CHEM_ID,                         bq27505_control_subcommand } }, 
   { "compute_board_offset",             { COMPUTE_BOARD_OFFSET,            bq27505_control_subcommand } },     
   { "compute_coulomb_counter_offset",   { COMPUTE_COULOMB_COUNTER_OFFSET,  bq27505_control_subcommand } }, 
   { "write_coulomb_counter_offset",     { WRITE_COULOMB_COUNTER_OFFSET,    bq27505_control_subcommand } }, 
   { "ocv_cmd",                          { OCV_CMD,                         bq27505_control_subcommand } },    
   { "bat_insert",                       { BAT_INSERT,                      bq27505_control_subcommand } },     
   { "bat_remove",                       { BAT_REMOVE,                      bq27505_control_subcommand } },   
   { "set_hibernate",                    { SET_HIBERNATE,                   bq27505_control_subcommand } },    
   { "clear_hibernate",                  { CLEAR_HIBERNATE,                 bq27505_control_subcommand } },  
   { "set_sleep_plus",                   { SET_SLEEP_PLUS,                  bq27505_control_subcommand } },   
   { "clear_sleep_plus",                 { CLEAR_SLEEP_PLUS,                bq27505_control_subcommand } }, 
   { "seal",                             { SEAL,                            bq27505_control_subcommand } },   
   { "enable_impedance_track",           { ENABLE_IMPEDANCE_TRACK,          bq27505_control_subcommand } }, 
   { "disable_impedance_track",          { DISABLE_IMPEDANCE_TRACK,         bq27505_control_subcommand } },  
   { "calibration_mode",                 { CALIBRATION_MODE,                bq27505_control_subcommand } },  
   { "reset",                            { RESET,                           bq27505_control_subcommand } }, 
};
...
static int bq27505_debug_get(void *data, u64 *val)
{
   ...
   struct bq27505_debugfs_command* command = (struct bq27505_debugfs_command*) data;
...
DEFINE_SIMPLE_ATTRIBUTE(bq27505_debug_rom_mode_fops, bq27505_debug_rom_mode, NULL, "%llu\n");
...
static int __init bq27505_debug_init(void)
{
   ...
   if (!IS_ERR(dent)) {
      for (i = 0; i < ARRAY_SIZE(debugfs_files); i++) {
         debugfs_create_file(debugfs_files[i].name, 0444, dent, &debugfs_files[i].command, &bq27505_debug_fops);
      }

      debugfs_create_file( "rom_mode", 0444, dent, NULL, &bq27505_debug_rom_mode_fops);
