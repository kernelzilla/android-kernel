...
#if DEBUG		
		printk(KERN_INFO "%s:atomic_read--m_flag\n", __FUNCTION__);
#endif
...
#if DEBUG		
		printk(KERN_INFO "%s:atomic_read--t_flag\n", __FUNCTION__);
#endif
...
#if DEBUG		
		printk(KERN_INFO "%s:atomic_read--mv_flag\n", __FUNCTION__);
#endif
...
#if DEBUG
        printk(KERN_INFO "%s, wake up return\n", __FUNCTION__); 
#endif
...
static int akm_aot_open(struct inode *inode, struct file *file)
{
	int ret = -1;
#if DEBUG
	printk(KERN_INFO "%s:count=%d,flag=%d,reserve=%d\n", __FUNCTION__,atomic_read(&open_count),atomic_read(&open_flag),atomic_read(&reserve_open_flag));
#endif
	if (atomic_cmpxchg(&open_count, 0, 1) == 0) {
		if (atomic_cmpxchg(&open_flag, 0, 1) == 0) {
			atomic_set(&reserve_open_flag, 1);
#if DEBUG			
			printk(KERN_INFO "%s:wake_up open_wq\n", __FUNCTION__);
#endif
			wake_up(&open_wq);
			ret = 0;
		}
...
#if DEBUG
	printk("KERN_INFO %s:count=%d,flag=%d,reserve=%d\n", __FUNCTION__,
	atomic_read(&open_count),atomic_read(&open_flag),atomic_read(&reserve_open_flag));
#endif
	   ...
	   unsigned long arg)
{
	...
	int ret = -1, status;
	short mode, value[12], delay,switch_flag = 0; /* step_count,*/
//	char *pbuffer = 0;
#if DEBUG
	printk(KERN_INFO "%s\n", __FUNCTION__);
#endif
...
#endif
		break;
	case ECS_IOCTL_GET_REPORT_SWITCH:
		switch_flag = (atomic_read(&m_flag) |atomic_read(&a_flag) |atomic_read(&t_flag) |atomic_read(&mv_flag));
#if DEBUG
		printk("ECS_IOCTL_GET_REPORT_SWITCH %x status=%d\n", cmd, status);
#endif
		break;
	default:
#if DEBUG
...
//		break;
	case ECS_IOCTL_GET_DELAY:
		...
		break;
	case ECS_IOCTL_GET_REPORT_SWITCH:
		if (copy_to_user(argp, &switch_flag, sizeof(switch_flag)))
			return -EFAULT;
		break;		
	default:
		break;
...
//	init_waitqueue_head(&data_ready_wq);
	...
	/* As default, does not report any information  */
	atomic_set(&m_flag, 0);
	atomic_set(&a_flag, 0);
	atomic_set(&t_flag, 0);
