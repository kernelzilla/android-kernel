//################################################################################
//#
//#   FILE NAME       : adp5588_keypad.c
//#   ORIGINATOR      : Vladimir Karpovich
//#   DATE OF ORIGIN  : 14OCT2008
//#
//#---------------------------------- PURPOSE ------------------------------------
...
/*keypad driver should use regular suspend/resume */
...
#else
#define    ADP5588_I2C_ADDRESS   ADP5588_I2C_ADDR
#define    ADP5588_KEYPAD_RESET_GPIO  QWRTY_RST_N 
#endif
...
#define ADP5588_CONFIG_KP_GPIO1       0xff     /*  Row0 to Row7  are KP Matrix */
#define ADP5588_CONFIG_KP_GPIO1_M     0xff     /*  Row0 to Row7  are KP Matrix */
#define ADP5588_CONFIG_KP_GPIO2       0xff     /*  Col0 to Col7  are KP Matrix */
#define ADP5588_CONFIG_GPIO_PULL2     0xff     /*  GPI  on Col7 */
#define ADP5588_CONFIG_KP_GPIO1_M_LOCK 0x28     /*  Row3 and Row5  only  are KP Matrix */
#endif
...
static ADP5588_cfg adp5588_init_seq[] =
{
   ...
   { ADP5588_REG_GPIO_PULL2,         ADP5588_CONFIG_GPIO_PULL2},     /*  GPI  on Col7 */
#ifdef CONFIG_MACH_MOT
   ...
   { ADP5588_REG_GPIO_INT_LVL2,      ADP5588_CONFIG_INT_LVL2  },   /*  GPI  on Col7 ( RINGER/ ( RINGER/SILENT KEY ) INT is LOW  */
   {ADP5588_REG_INT_STAT,            0x5},    /* Clear  Interrupt */
#else

   { ADP5588_REG_DEBOUNCE_DIS_2,     0xFF },             /* Set Debouce */
   { ADP5588_REG_INT_STAT,            0x01 },            /* Clear  Interrupt */
#endif
   { ADP5588_REG_KP_GPIO1,               ADP5588_CONFIG_KP_GPIO1,},      /*  Row0 to Row7  are KP Matrix*/
   { ADP5588_REG_KP_GPIO2,               ADP5588_CONFIG_KP_GPIO2,},      /*  Col0 to Col7  are KP Matrix */
...
static ADP5588_cfg adp5588_init_seq_motus[] =
{
    ...
    {ADP5588_REG_INT_STAT,            0x5 },           /* Clear  Interrupt */
#else
    {ADP5588_REG_INT_STAT,            0 },             /* Clear  Interrupt */
#endif
    /* Must end with 0 reg */
    { 0,          0x00 },
};
...
static void switch_event(struct input_handle *handle, unsigned int type,
				unsigned int code, int value)
{

	if (type == EV_SW && code == SW_TABLET_MODE) {
		printk(KERN_DEBUG "%s(%s): Event.  Type: %d, Code: %d,"
			"Value: %d   KEYPAD  : %s \n", __FILE__, __func__,
			 ...
			 const struct input_device_id *id)
{
	...
	if (error)
		...
		printk(KERN_DEBUG "%s(%s): Connected device: \"%s\", %s\n"
			"sw state  %d ", __FILE__, __func__, dev->name,
				dev->phys, sw_value);
...
#else
	return adp5588_write_register(client, ADP5588_REG_INT_STAT, 0x01);
#endif
}
...
int adp5588_enable( uint8_t value)
{
	if ( adp5588_data.use_irq)   
	{
                    if(value)
				enable_irq(adp5588_data.client->irq);
		   else
		   	disable_irq(adp5588_data.client->irq);
	}
	return 1;
}

EXPORT_SYMBOL(adp5588_enable);
...
#else
	if ((rc = adp5588_read_register(adp5588_data.client,
		ADP5588_REG_GPIO_DAT_STAT3 , &mask)) < 0)
#endif
         return  rc;
...
/* read scancodes until queue is empty */
   do 
   {
	...
	if (scan_code != ADP5588_KEY_FIFO_EMPTY) {
		...
		dev_dbg(&client->dev, "ADP5588 got scancode %d, "
		...
		/* got RINGER/SILENCE  switch event */
			...
			input_report_switch(kp->input_dev, SW_MUTE,
				adp5588_data.ringer_switch);
		} else {
		/* got regular key event */
			input_report_key(kp->input_dev, kp->last_key,
				kp->last_key_state);
...
static irqreturn_t adp5588_irq_handler(int irq, void *dev_id)
{
	...
	schedule_work(&kp->work);
#else
        if (!atomic_read(&kp->suspended))
        {
	   schedule_work(&kp->work);
        }
        else
        {
           atomic_set(&kp->suspended, SUSP_IRQ);
        }
#endif
	return IRQ_HANDLED;
}
...
static int adp5588_config(struct i2c_client *client)
{
   ...
   {
      cfg = adp5588_init_seq_motus;
   }
   else 
   {
      cfg = adp5588_init_seq;
   }
...
/* Performed by board file, as it should be */
	...
	printk("ADP5588 Keypad Dev ID : 0x%x  \n",adp5588_data.dev_id);
   ...
   for (i=0; i<=80; i++)
   {
      set_bit(i, adp5588_data.input_dev->keybit);
   }

   clear_bit(0, adp5588_data.input_dev->keybit);
#endif
#ifdef CONFIG_MACH_MOT
...
/* test only :  Add GPIO keys to be able report them for testing */
	...
	printk(KERN_INFO "ADP5588 : Start keypad %s in %s mode\n",
           adp5588_data.input_dev->name,
           adp5588_data.use_irq ? "interrupt" : "polling");
#ifdef CONFIG_MACH_MOT
	if (!adp5588_data.device_type) {
		if (adp5588_read_register(client, ADP5588_REG_GPIO_DAT_STAT2, &reg) < 0) {
			adp5588_data.ringer_switch = 1;
			dev_err(&client->dev, " %s(%s):Can't read Ringer/Silent"
				"switch state.Set it to ON\n",
					__FILE__, __func__);
	} else
	...
	{
	   /* No switch on Motus , set to default */
	   adp5588_data.ringer_switch = 1;

	input_report_switch(adp5588_data.input_dev, SW_MUTE,
		adp5588_data.ringer_switch);
	...
	input_report_switch(adp5588_data.input_dev,
		SW_LID, !gpio_get_value(ADP5588_KEYPAD_FLIP_DETECT));
	input_sync(adp5588_data.input_dev);
#endif
 ...
 /* it is MOTUS  .  start lock switch handler */
	...
	if (err < 0)
	 ...
	 if (err) {
		dev_err(&client->dev, " %s(%s): Unable to register"
			"key lock  input handler e\n",
				__FILE__, __func__);
		goto err_input_register_device_failed;
	}
	}
...
#endif
#ifndef CONFIG_MACH_MOT
       atomic_set(&adp5588_data.suspended, NOT_SUSP);
#endif
#ifdef  CONFIG_HAS_EARLYSUSPEND
#ifdef CONFIG_MACH_MOT
	adp5588_data.early_suspend.level = EARLY_SUSPEND_LEVEL_BLANK_SCREEN + 1;
#else
	adp5588_data.early_suspend.level = EARLY_SUSPEND_LEVEL_BLANK_SCREEN + 3,
#endif
...
#ifndef CONFIG_MACH_MOT
#define TEST_SLIDER_DETECT_GPIO SLIDE_DETECT
#endif
#ifdef CONFIG_MACH_MOT
...
static int adp5588_debug_get(void *data,u64 *val)
{
	...
	*val = TEST_SCANCODE_SET(adp5588_data.last_key, adp5588_data.last_key_state,
		gpio_get_value(TEST_SLIDER_DETECT_GPIO),
			adp5588_data.ringer_switch);
#endif
#ifdef CONFIG_MACH_MOT
	*val = TEST_SCANCODE_SET(adp5588_data.last_key,
		adp5588_data.last_key_state, gpio_get_value(MORRISON_SLIDER_DETECT_GPIO),
			adp5588_data.ringer_switch);
#endif
	return 0;
}
...
static int adp5588_resume(struct i2c_client *client)
{
   ...
   struct adp5588_keypad_data *kp = i2c_get_clientdata(client);
#ifndef CONFIG_MACH_MOT
   if (atomic_read(&kp->suspended) == SUSP_IRQ)
   {
     schedule_work(&kp->work);
   }
   atomic_set(&kp->suspended, NOT_SUSP);
#endif
#ifdef CONFIG_MACH_MOT
