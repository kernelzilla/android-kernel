...
static struct mode_cfg all_modes[]={ 
	...
	{0xc7, 0xd0, 0x86, 0x04, 0x05, 0x89, 0x04, 0x05}, // 10Hz
	{0x27, 0xd0, 0x86, 0x04, 0x19, 0x89, 0x04, 0x19}, // 50Hz
	{0x2f, 0xd0, 0x86, 0x04, 0x32, 0x89, 0x04, 0x32}, //100Hz
	{0x37, 0xd0, 0x86, 0x04, 0xc8, 0x89, 0x04, 0xc8}, //400Hz
#else
	{0xc7, 0xd0, 0x86, 0x12, 0x05, 0x89, 0x16, 0x05}, // 10Hz
	{0x27, 0xd0, 0x86, 0x12, 0x19, 0x89, 0x16, 0x19}, // 50Hz
	{0x2f, 0xd0, 0x86, 0x12, 0x32, 0x89, 0x16, 0x32}, //100Hz
	{0x37, 0xd0, 0x86, 0x12, 0xc8, 0x89, 0x16, 0xc8}, //400Hz
#endif
#endif
...
static int accel_axis2val(u8 lsb, u8 msb)
{
	...
	//dprintk ("lsb=%02x msb=%02x value=%08x(%d)", lsb, msb, uni.uval, uni.ival);
...
#endif
{
...
#else
	u8 buf[6]={SUB_AUTO_INCREMENTED (ACCEL_I2C_PORT_XOUT_L)};
#endif
#ifdef CONFIG_MACH_MOT
...
#else
	ret1 = i2c_master_recv (accel_info.i2c, buf, 6);
#endif
    if (ret1 < 0) {
#ifdef CONFIG_MACH_MOT
	mutex_unlock(&accel_info.mlock);
	...
	default:
	 ...
	 case SWAPPED_YZ : 
		...
		z = accel_axis2val (buf[2], buf[3]);
		y = accel_axis2val (buf[4], buf[5]); 
					break;
	 case SWAPPED_XZ :
		z = accel_axis2val (buf[0], buf[1]);
		y = accel_axis2val (buf[2], buf[3]);
		x = accel_axis2val (buf[4], buf[5]); 
					break;
	 case SWAPPED_XY_INV : 
		y = -accel_axis2val (buf[0], buf[1]);
		x = -accel_axis2val (buf[2], buf[3]);
...
int accel_i2c_read_axis(int axis)
{
	...
	if (accel_param_swapped) {
		if (axis == AXIS_X) {
			buf[0] = SUB_AUTO_INCREMENTED (ACCEL_I2C_PORT_XOUT_L);
			if (accel_param_swapped == SWAPPED_XY) { 
				update = &accel_info.y_axis;	/* read X and swap with Y */
			}else

			if (accel_param_swapped == SWAPPED_XZ) { 
				update = &accel_info.z_axis;	/* read X and swap with Z */
			}
		}else

		if (axis == AXIS_Y) {
			buf[0] = SUB_AUTO_INCREMENTED (ACCEL_I2C_PORT_YOUT_L);
			if (accel_param_swapped == SWAPPED_XY) {
				update = &accel_info.x_axis;	/* read Y and swap with X */
			}else

			if (accel_param_swapped == SWAPPED_YZ) {
				update = &accel_info.z_axis;	/* read Y and swap with Z */
			}
		}else
		
		if (axis == AXIS_Z) {
			buf[0] = SUB_AUTO_INCREMENTED (ACCEL_I2C_PORT_ZOUT_L);
			if (accel_param_swapped == SWAPPED_XZ) {
				update = &accel_info.x_axis;	/* read Z and swap with X */
			}else

			if (accel_param_swapped == SWAPPED_YZ) {
				update = &accel_info.y_axis;	/* read Z and swap with Y */
			}
		}
	}else 
		switch (axis) {
		 case AXIS_X : 
			buf[0] = SUB_AUTO_INCREMENTED (ACCEL_I2C_PORT_XOUT_L); 
			update = &accel_info.x_axis;
					break;
		 case AXIS_Y : 
			buf[0] = SUB_AUTO_INCREMENTED (ACCEL_I2C_PORT_YOUT_L); 
			update = &accel_info.y_axis;
					break;
		 case AXIS_Z : 
			buf[0] = SUB_AUTO_INCREMENTED (ACCEL_I2C_PORT_ZOUT_L); 
			update = &accel_info.z_axis;
					break;
		 default: return -EFAULT;
		}
 ...
 *
 */
static void accel_feed_input(int *axes)
{
	struct input_dev *idev = accel_info.idev;
	
	if (axes) {
		input_report_abs (idev, ABS_X, *axes);
		input_report_abs (idev, ABS_Y, *(axes+1));
		input_report_abs (idev, ABS_Z, *(axes+2));
		input_sync (idev);
	}else {
		mutex_lock (&accel_info.mlock);
		input_report_abs (idev, ABS_X, accel_info.x_axis);
		input_report_abs (idev, ABS_Y, accel_info.y_axis);
		input_report_abs (idev, ABS_Z, accel_info.z_axis);
		input_sync (idev);
		mutex_unlock (&accel_info.mlock);
	}
}
...
#else
	status = (u8) i2c_smbus_read_byte_data (accel_info.i2c, 
        ACCEL_I2C_PORT_STATUS);
	ret = accel_i2c_read_axis_data (axes);
#endif
    if (ret < 0) {
        return;
    }
...
//	up (&accel_info.sema2);
    ...
    input_sync (accel_info.idev);
#ifndef CONFIG_MACH_MOT
    printk_data ("status=0x%02X, x=%4d y=%4d z=%4d", 
        status, axes[0], axes[1], axes[2]);
#endif
}
