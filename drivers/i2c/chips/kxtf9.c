...
//static atomic_t current acc x,y,z values;
static atomic_t acc_xyz_x;
static atomic_t acc_xyz_y;
static atomic_t acc_xyz_z;
...
static void kxtf9_device_power_off(struct kxtf9_data *tf9)
{
	...
	u8 buf[2] = { CTRL_REG1, PC1_OFF };
        ...
        err = kxtf9_i2c_write(tf9, buf, 1);
	if (err < 0)
	dev_err(&tf9->client->dev, "kxtf9 power off failed 0x%x,0x%x: %d\n",
		buf[0], buf[1], err);  
//vqv	
...
static void kxtf9_irq_work_func(struct work_struct *work)
{
	...
	if (gpio_get_value(tf9->pdata->gpio)) {
		...
		if ((status & TPS) > 0) {
			...
			printk(KERN_ERR "kxtf9_irq_work_func: TILT_POS_CUR=%x, TILT_POS_PRE=%x \n", buf[0], buf[1]); //HZ
			if (err < 0) {
				dev_err(&tf9->client->dev,
					"tilt read error: %d\n", err);
			} else {
				...
				/*** DEBUG OUTPUT - REMOVE ***/
				dev_info(&tf9->client->dev, "IRQ TILT kxtf9_resolve_dir(buf[0]) = %x\n", kxtf9_resolve_dir(tf9, buf[0]));
...
static int kxtf9_get_acceleration_data(struct kxtf9_data *tf9, int *xyz)
{
	...
	//printk("x:%d y:%d z:%d\n", xyz[0], xyz[1], xyz[2]);
       #endif
	/*** DEBUG OUTPUT - REMOVE ***/
//HZ	dev_info(&tf9->client->dev, "x:%d y:%d z:%d\n", xyz[0], xyz[1], xyz[2]);
	/*** <end> DEBUG OUTPUT - REMOVE ***/
...
static int kxtf9_misc_read(struct file *file, char __user *buffer, size_t bytes, loff_t *offset)
{
    accel_event_t acc_ev;
	...
	if (kxtf9_get_acceleration_data(tf9, acc_ev.values))
		return -EFAULT;

	if (copy_to_user(buffer, &acc_ev, nread))
		return -EFAULT;
				...
				unsigned int cmd, unsigned long arg)
{
	...
	case KXTF9_IOCTL_SET_DELAY:
		...
		if (interval < 0)
			return -EINVAL;
		tf9->pdata->poll_interval =
		    max(interval, tf9->pdata->min_interval);
	...
	case KXTF9_IOCTL_SET_WAKE_ENABLE:
		...
		break;
 	case KXTF9_IOCTL_GET_CURRENT_XYZ:
		acc_xyz[0] = atomic_read(&acc_xyz_x);
		acc_xyz[1] = atomic_read(&acc_xyz_y);
		acc_xyz[2] = atomic_read(&acc_xyz_z);
		if (copy_to_user(argp, &acc_xyz, sizeof(acc_xyz)))
			return -EINVAL;
		break;
	default:
		return -EINVAL;
...
static void kxtf9_store_current_value(int *xyz)
{
	atomic_set(&acc_xyz_x, xyz[0]);
	atomic_set(&acc_xyz_y, xyz[1]);
	atomic_set(&acc_xyz_z, xyz[2]);	
}
...
static void kxtf9_input_work_func(struct work_struct *work)
{
	...
	if (kxtf9_get_acceleration_data(tf9, xyz) == 0)
	{
		   ...
		   kxtf9_store_current_value(xyz);
		   #if DEBUG
	            printk( "kxtf9 drive xyz report: x=%d, y=%d, z=%d\n", xyz[0], xyz[1], xyz[2]);
                 #endif
	}
	schedule_delayed_work(&tf9->input_work,
			      msecs_to_jiffies(tf9->pdata->poll_interval));
...
static int kxtf9_validate_pdata(struct kxtf9_data *tf9)
{
	...
	tf9->pdata->poll_interval = max(tf9->pdata->poll_interval,
					tf9->pdata->min_interval);
...
//vqv
	gpio_request(tf9->irq, "kxtf9 accelerometer int");
	gpio_direction_input(tf9->irq);
