...
static int __devinit i2c_gpio_probe(struct platform_device *pdev)
{
	...
	ret = -ENOMEM;
//	if((ret = i2c_gpio_configure_gpio())) return ret;
...
err_add_bus:
	printk(KERN_ERR "%s: err_add_bus\n", __func__);
	gpio_free(pdata->scl_pin);
err_request_scl:
	printk(KERN_ERR "%s: err_request_scl\n", __func__);
	gpio_free(pdata->sda_pin);
err_request_sda:
	printk(KERN_ERR "%s: err_request_sda\n", __func__);
	kfree(bit_data);
err_alloc_bit_data:
	printk(KERN_ERR "%s: err_alloc_bit_data\n", __func__);
	kfree(adap);
err_alloc_adap:
	printk(KERN_ERR "%s: err_alloc_adap\n", __func__);
	if((ret = i2c_gpio_unconfigure_gpio())) return ret;
    return ret;
}
...
static int __init i2c_gpio_init(void)
{
    ...
    printk(KERN_ERR "i2c-gpio: platform_driver_register\n");
