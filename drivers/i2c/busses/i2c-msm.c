...
#define SBA5008_BUSY
#define SBA5008_LOCK
// this one is causing excessive i2c errors
//#define SBA5008_REM
...
#else
		printk(KERN_ERR "%s: IRQ %d but nothing to do!\n", __func__, irq);
#endif
...
msm_i2c_poll_notbusy(struct msm_i2c_dev *dev)
{
	...
	do {
	    ...
	    gpio_tlmm_config(GPIO_CFG(60, 0, GPIO_OUTPUT, GPIO_NO_PULL, GPIO_16MA), GPIO_ENABLE);
	    gpio_tlmm_config(GPIO_CFG(61, 0, GPIO_OUTPUT, GPIO_NO_PULL, GPIO_16MA), GPIO_ENABLE);
	    udelay(50);

	    gpio_configure(60, GPIOF_DRIVE_OUTPUT|GPIOF_OUTPUT_HIGH);
	    gpio_configure(61, GPIOF_DRIVE_OUTPUT|GPIOF_OUTPUT_HIGH);
	    udelay(50);
...
wait_for_int:
		...
		if (dev->err) {
            ...
            if (! (dev->msg->flags & I2C_M_NAK_LIKELY)) 
                dev_err(dev->dev,
                        "Error during data xfer (%d) at addr 0x%x flags 0x%x len %d \n",
                        dev->err, msgs->addr,msgs->flags,msgs->len);
#else
			dev_err(dev->dev,
