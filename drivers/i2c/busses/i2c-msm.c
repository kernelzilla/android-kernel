...
#if defined(CONFIG_KERNEL_MOTOROLA)
	dev_err(dev->dev, "Poll write-ready timed out\n");
	if (!dev->busy)
		dev->busy = jiffies_to_msecs(jiffies);
#endif /* defined(CONFIG_KERNEL_MOTOROLA) */
	return -ETIMEDOUT;
}
...
#if defined(CONFIG_KERNEL_MOTOROLA)
	dev_err(dev->dev, "Poll not-busy timed out\n");
	if (!dev->busy)
		dev->busy = jiffies_to_msecs(jiffies);
#endif /* defined(CONFIG_KERNEL_MOTOROLA) */
	return -ETIMEDOUT;
...
msm_i2c_recover_bus_busy(struct msm_i2c_dev *dev, struct i2c_adapter *adap)
{
...
#if defined(CONFIG_KERNEL_MOTOROLA)
	{
		dev_info(dev->dev, "Bus busy cleared\n");
		dev->busy = 0;
#endif /* defined(CONFIG_KERNEL_MOTOROLA) */
		return 0;
#if defined(CONFIG_KERNEL_MOTOROLA)
...
#if defined(CONFIG_KERNEL_MOTOROLA)
	dev_err(dev->dev, "Attempting to clear bus busy via clock cycles\n");
#endif /* defined(CONFIG_KERNEL_MOTOROLA) */
	for (i = 0; i < 9; i++) {
	...
	dev_err(dev->dev, "Bus still busy, status %x, intf %x\n",
		 status, readl(dev->base + I2C_INTERFACE_SELECT));
#if defined(CONFIG_KERNEL_MOTOROLA)
	if (dev->busy && ((jiffies_to_msecs(jiffies) - dev->busy)
			>= BUS_BUSY_PANIC_MS)) 
		panic("Unable to clear I2C bus busy for too long!\n");
#endif /* defined(CONFIG_KERNEL_MOTOROLA) */
	enable_irq(dev->irq);
	return -EBUSY;
}
