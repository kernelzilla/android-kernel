...
//static	ktime_t	timsPress[NUMBER_OF_FINGERS];
		...
		while ( !complete )
		{
			...
			else 
			{
				...
				case Q51001211009AK08_KEY_MSG:
					KEY08_PRINTK("%s: Processing key message. ",__FUNCTION__ );
/*
 * since there are only 3 keys and they are pre-defined. Just check if one of this keys is pressed. Ignore everything else
 */
					if ( buf[2] & 0x80 )
					{
						/* Back key */
						input_report_key(ts->input_dev, 158, 1);
						lastKeyPressed = 158;
					}
					if ( buf[3] & 0x08 )
					{
						/* Home key */
						input_report_key(ts->input_dev, 102, 1);
						lastKeyPressed = 102;
					}
					if ( buf[4] & 0x80 )
					{
						/* Menu key */
						input_report_key(ts->input_dev, 139, 1);
						lastKeyPressed = 139;
					}
					if ( lastKeyPressed != 0 && buf[2] == 0 && buf[3] == 0 && buf[4] == 0 )
					{
						input_report_key(ts->input_dev, lastKeyPressed, 0 );
						lastKeyPressed = 0;
					}
...
/*
			lpValue = key08_ts_read_register(tsGl, (int)Q51001211009AK08_LP_MODE_ADDR);
*/
		}
		lastKeyPressed = 0;
...
//	struct key08_ts_data *ts = i2c_get_clientdata(client);
	...
	// Read all the data from the device, so the device can enable interrupt again, unless we are in bootloader mode
	bytes2Read = 6;
	if ( ts->mode == KEY08_MODE_NORMAL )
	{
...
// 			if ( buf[0] == 0xff )
			...
			{
				KEY08_PRINTK("key08_ts_resume: Got all data.\n");
				ret = 0;
			}
			else
			{
				KEY08_PRINTK("%s: reading %d.\n",__FUNCTION__, numNeededReads++);
			}
		}
	} while ( ret != 0 );
