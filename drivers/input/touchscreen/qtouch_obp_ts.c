...
static int qtouch_hw_init(struct qtouch_ts_data *ts)
{
	...
	if (obj && obj->entry.num_inst > 0) {
		...
		for (i = 0; i < obj->entry.num_inst; i++) {
			if (ts->pdata->flags & QTOUCH_USE_MULTITOUCH)
				memcpy(&cfg, &ts->pdata->multi_touch_cfg[i],
				       sizeof(cfg));
			else
				memset(&cfg, 0, sizeof(cfg));
			ret = qtouch_write_addr(ts,
			                        obj->entry.addr + (obj->entry.size * i),
			                        &cfg,
	...
	if (obj && obj->entry.num_inst > 0) {
		for (i = 0; i < obj->entry.num_inst; i++) {
			ret = qtouch_write_addr(ts,
			                        obj->entry.addr + (obj->entry.size * i),
						&ts->pdata->grip_suppression_cfg[i],
						min(sizeof
						    (ts->pdata->grip_suppression_cfg[i]),
						    obj->entry.size));
			if (ret != 0) {
		...
		if (msg != NULL) {
			for (object = 7; object < QTM_OBP_MAX_OBJECT_NUM; object++) {
				...
				if (size != 0) {
					...
					for (instance = 0;
					     instance < ts->obj_tbl[object].entry.num_inst;
					     instance++) {

						qtouch_read_addr(ts,
						                 ts->obj_tbl[object].entry.addr
						                 + (instance * ts->obj_tbl[object].entry.size),
						                 (void *)data_buff,
						                 size);

						msg_location = sprintf(msg, "%s: Object %d, Instance %d:",
						                       __func__, object, instance);
			      ...
			      void *_msg)
{
	...
	if (finger >= ts->pdata->multi_touch_cfg[0].num_touch)
		return 0;
	...
	for (i = 0; i < ts->pdata->multi_touch_cfg[0].num_touch; i++) {
			     ...
			     struct qtm_obj_message *msg)
{
	...
	case QTM_OBJ_TOUCH_MULTI:
		if (msg->report_id < (obj->report_id_min + obj->entry.num_rids))
			...
			pr_info("%s: No handler defined for second touch screen\n", __func__);
		break;
