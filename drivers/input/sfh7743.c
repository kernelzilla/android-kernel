 ...
 * Motorola 2009-June-03 - Not allow proximity enabling from framework for Motus P2 boards
 * Motorola 2009-June-23 - Not allow proximity enabling from framework for Motus P2 boards hwid < 2F
*/
...
*/
      ...
      case SFH7743_IOCTL_GET_STATUS:
		error = gpio_get_value(sfh7743_info.gpio_en);
		break;
      default:
...
static int	sfh7743_probe(struct platform_device *pdev)
{
   ...
   if (error == E_OK)
   {
      sfh7743_log(KERN_INFO "sfh7743_probe: initialized gpio_int[%d] irq[%d] gpio_en[%d]\n",
         sfh7743_info.gpio_intr, sfh7743_info.irq, sfh7743_info.gpio_en);
   }
   else
   {
...
static ssize_t sfh7743_set_mode(struct device *dev, struct device_attribute *att, const char *buf, size_t count)
{
	...
	{
            if(machine_is_motus() &&  (mot_hw_rev < 0x3B0))
            {
                sfh7743_log(KERN_ERR "PROXIMITY SENSOR NOT ENABLED FOR MOTUS P2 BOARDS (< 2F)\n");
                return 0;
           }
                sfh7743_enable(true);
